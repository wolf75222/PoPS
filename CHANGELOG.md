...
- **Spec 5 sec.12.5 (#43): AMR/MPI profiling counters** -- the `AmrRuntime` times `regrid` / `fill_boundary` / `average_down` into an `AmrSystem`-owned `Profiler` (+ `mpi_reductions` / `mpi_messages` under MPI np>1, `enabled()`-guarded, MockImpl-safe); `AmrSystem` gains `enable_profiling` / `profile_report` + a typed `profile()` context, and `PerformanceSummary.by_amr_mpi()` surfaces them (declared-unavailable on a host / non-AMR build). New `python/tests/test_spec5_amr_mpi_counters.py` (epic ADC-479).
- **Spec 5 sec.15 (#23/#27): the time Program requires typed operators + terms (one public path)** -- the public `P.call` now requires an operator HANDLE (a bare string operator name is refused, naming `m.rate(...)`/`m.field_operator(...)`/`m.source_term(...)`) and the public `P.rhs` requires the typed `terms=[Flux(), source]` list (the legacy `flux=`/`sources=`/`fluxes=` and a bare `P.rhs` are refused, naming `terms=`); no opt-in flag (`Program(strict_typed=)` / `POPS_STRICT_TYPED` removed). The byte-identical string/`flux=`/`sources=` builders survive only as the internal `Program._call` / `_rhs_legacy` that the `pops.lib.time` macros and the typed front doors lower through (same `_ir_hash`). New `python/tests/test_spec5_typed_program.py` (epic ADC-479).
...
- **Spec 5 C1-System (#479): named elliptic fields lower on the uniform System** -- a `Case` with a named non-Poisson field validates (the `_POISSON_FIELD_NAMES` whitelist reject is removed) and `CompiledModel` carries `elliptic_field_names` (populated from `m._elliptic_fields`); at install, `_install_solver` routes a solver selection for a field a block's model DECLARES through the shared system elliptic solver (`set_poisson`) and rejects an undeclared field name (a typo) naming the declared set. The AMR named-field path stays deferred (ADC-428). New `python/tests/test_unified_install_named_field_system.py` (epic ADC-479).
- **Spec 5 C3 (#479): multi-block Cases lower on a Uniform layout** -- `Case.validate` no longer rejects more than one block, and `pops.compile` resolves EACH block's physics and carries the `{block: model}` table on the handle (`_block_models`) so `pops.bind` installs each block with its own model; multi-block on an AMR layout stays a HONEST deferral (the whole-system AMR multi-block seam is not wired). New `python/tests/test_case_multiblock_uniform.py` / `test_case_multiblock_amr.py` (epic ADC-479).
