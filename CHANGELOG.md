...
- **Spec 5 sec.11 (#32): `layout=AMR` end-to-end** -- `pops.compile` no longer defers a single-block AMR assembly; it routes to `target="amr_system"` and `pops.bind` builds an `AmrSystem` from an `AmrSystemConfig` derived from the AMR layout (n / L / periodic / `regrid_every` / patches) + flows the typed refinement and Poisson. `layout=Uniform` is byte-identical; the `.so` emit is Kokkos-gated (ROMEO). New `python/tests/test_spec5_amr_e2e.py` (epic ADC-479).
