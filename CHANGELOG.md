...
- Share uniform/AMR field-provider installation and Case/Program immutable-container copying;
  reject corrupt sibling native variants before signing another dimension.
...
- Uniform checkpoint payload v7 and AMR payload v8 persist one release-versioned rank-generic
  ...
  allocation or mutation, with no scalar `nx`/`ny` compatibility route. Uniform v7 additionally
  authenticates the semantic embedded-boundary expression, mode and thresholds independently of
  MPI rank ownership and local materialization before any restart mutation.
...
- Add an explicit offline-only migration for the byte-exact frozen Uniform-v2 checkpoint fixture.
  Migration requires a complete authenticated current-v7 authority plus a reviewed mapping that pins
...
### Removed
- ADC-752 Retire the duplicate Euler-specific HLLC/Roe providers, route tokens, builders and
  component-count/layout inference. `HLLC()` and `Roe()` now each resolve to one capability-driven
...
- **ADC-590: remove the implicit Euler fallback from the generic HLLC/Roe routes** -- the native Euler brick (`include/pops/physics/fluids/euler.hpp`) now provides the HLLC/Roe capabilities (`contact_speed`, `hllc_star_state`, `roe_dissipation`) and takes the generic `HLLCFlux`/`RoeFlux` path. The temporary explicit Euler route experiment shipped with this change was subsequently retired by ADC-752; current code has one capability-driven route per public provider.
