...
- **Spec 5 sec.12.1 print-readability `__str__` for `pops.Spatial` / `pops.FiniteVolume`** (ADC-505, epic ADC-479, criteria #40-41): `print(obj)` on a finite-volume scheme now gives a short, deterministic one-line summary (`Spatial(limiter=minmod, flux=rusanov, recon=conservative)`, appending `positivity_floor` / `wave_speed_cache` only when set) instead of the default `<...Spatial object at 0x...>` that leaked a memory address. The audit confirmed the other headline user objects already print a short summary: the mesh layouts `Uniform` / `AMR`, the elliptic solver `GeometricMG`, the Krylov solvers, `codegen.Optimization`, `params.RuntimeParam`, `output.OutputPolicy`, and `diagnostics.Norm` / `Integral` inherit the inert `Descriptor.__str__`, and `fields.FieldProblem` / `PoissonProblem` carry their own. New `tests/architecture/test_print_readability.py` builds each object and asserts `str(obj)` is short (< 800 chars), deterministic, free of any raw `array(` / `ndarray` dump, and never the default object repr. `__repr__` is unchanged for debug; the change is additive and inert.
