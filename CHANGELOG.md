...
- **Clean-rebuild docs uninstall the renamed `pops` package, not just `adc-cpp`** (ADC-520) -- the "Update or clean rebuild" block in `docs/sphinx/getting-started/installation.md` ran `pip uninstall -y adc-cpp` (the pre-rename name only), so on a current tree it left the installed `pops` wheel in place and the clean rebuild reinstalled over a stale copy. It now uninstalls both names (`pip uninstall -y pops adc-cpp`).
