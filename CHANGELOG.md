...
### Changed
- **Spec 5 no-transitional-reject (C7): CFL on a compiled time Program is wired, not rejected at install** (epic ADC-479): `System::step_cfl` already routes the per-block CFL `dt` through an installed compiled Program (so `sim.run(cfl=0.5)` advances it), but a stale Python guard rejected `cadence.cfl` at install. The `_install_compiled` cadence seam no longer raises on a numeric `cfl`; the value is applied at runtime by `System.run(cfl=)` / `step_cfl(cfl)`. `pops.CompiledTime(cfl=<number>)` now constructs (a self-computed `cfl="program"` sub-program stays deferred), and the docstrings say where the cfl is applied. Updated `python/tests/test_unified_install.py`.
- **Spec 5 no-transitional-reject (C5a, ADC-510): runtime params on a compiled time Program are a loud reject, not a silent drop** (epic ADC-479): a runtime param that no native instance consumes was silently dropped on a compiled whole-system Program (no per-block param carrier; `ProgramContext` is deferred to ADC-510). The unified install now raises a clear `NotImplementedError` naming the deferred feature and redirecting to native instances or a const param; a param a native instance DOES declare still routes through `set_block_params` unchanged. Updated `python/tests/test_unified_install.py`.
- **Spec 5 no-transitional-reject (C2, ADC-508): an AMR whole-system time Program is rejected EARLY at compile, not at bind** (epic ADC-479): `pops.compile(layout=AMR, time=Program)` used to compile the whole-system Program and then die at `pops.bind` (a compile-succeeds-then-bind-fails seam). `pops.compile` now raises `NotImplementedError` BEFORE any `.so` is built, redirecting to the wired per-block AMR path (`pops.physics.Model.compile(backend="production", target="amr_system")` + `pops.AmrSystem.add_equation`); the deferred whole-system AMR install seam is ADC-508. The bind-time guard remains as a defensive backstop. Updated `python/tests/test_problem_orchestration.py`.
...
- **Spec 5 no-decorative-API (C4, ADC-509): the general `pops.output` policy surface is removed** (epic ADC-479): `OutputPolicy` / `CheckpointPolicy` (with `HDF5` / `Plotfile` formats and `AllLevels` / `CoarseOnly` / `SelectedLevels` level selection) and `Case.output(...)` had ZERO codegen and ZERO C++ runtime wiring, so the assembly only ever rejected at validate -- a decorative API. They are deleted (`python/pops/output/policies.py` / `formats.py` removed, `pops.output.__all__` emptied, `Case.output` / `Case._outputs` / the validate reject / the `n_outputs` / `outputs` introspection gone). The general output/checkpoint runtime is tracked by ADC-509. The WIRED narrower `pops.mesh.amr.AMROutput` / `pops.mesh.amr.CheckpointPolicy` (sec.8.11) are untouched. New architecture guard in `tests/architecture/test_spec5_structure.py`; updated `python/tests/test_params_output_external.py` and `python/tests/test_problem_orchestration.py`.
