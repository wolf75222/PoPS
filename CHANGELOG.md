...
- **ADC-621: accelerate the PR gate -- import-closure test selection + a header-build cache guard** -- the `gate-python` selector now picks impacted tests from the ACTUAL static import graph instead of a name-token area heuristic: a new stdlib-only `scripts/ci_import_closure.py` AST-walks `python/pops/**` (every import at any depth, so the pervasive function-scope lazy imports and relative imports are captured, plus the ancestor packages whose `__init__` runs on import), reverse-closes a changed pops module to the tests that transitively import it, and closes the bare-sibling cross-test edges both ways (a shared helper drags its dependents in). `scripts/ci_select_tests.py` `plan_python` wires it in with every conservative rule intact (broad/bindings/CMake -> all; a new or unparseable pops file fails safe to all; the >75% and unknown-path nets stay) and now emits the firing rule (`python_why`) to the job summary. On the build side, `gate-python-build` compiles the module with `POPS_HEAVY_TU_POOL=2` (the two ~4 GB `-O3` heavy TUs fit the 16 GB Serial runner, halving the header-PR critical path that pool=1 serialized to ~37 min) and gains an eviction guard: a `pops-module-*` cache keyed on every `.so`-affecting input (`include/**`, `python/bindings/**`, the CMake/preset files, the pinned Kokkos version+commit, and the gcc major) reuses the compiled `_pops.so` on a pure-Python PR (exact hit + `cpp` unchanged), re-syncing the checkout's Python sources over it and skipping the cmake build; a near-miss or any C++ change rebuilds (no restore-keys, so a stale `.so` is never served), and push/nightly stay a full build. Routing and the `gate` aggregation are unchanged. New source-only `tests/python/architecture/test_ci_impacted_selection.py`. CI-only; no change to the shipped library or test coverage (the full suite still runs on master push + nightly).
- **ADC-593: generate the per-route pybind seam TUs from one combination manifest** -- the ~20 hand-written binding files that carried one `pops::detail::build_block_*` / `build_amr_block_*` / `build_amr_compiled_*` leaf per `(transport, flux)` combination (`python/bindings/system/{isothermal,compressible}/*`, `amr/block/**`, `amr/compiled/**`) are no longer tracked source: the 19 leaves are GENERATED at configure time from a single declarative manifest (`python/bindings/seam_combinations.cmake`) into `${build}/generated_seams/` via `configure_file` over six shape templates (`python/bindings/templates/*.cpp.in`), byte-equivalent in symbols/semantics to the deleted files. `python/CMakeLists.txt` and `tests/CMakeLists.txt` both `foreach` the manifest (the `_pops` module and the `pops_runtime_{system,amr}` OBJECT libs share the same generated leaves), and the build-memory mitigation is unchanged (one leaf per TU, still serialized by the Ninja `JOB_POOL`). Adding a Riemann or reconstruction is now ONE manifest row plus the numerics template in the headers, never a new hand-written pybind file. The two thin riemann DISPATCHERS (`amr_block_compressible.cpp` / `amr_compiled_compressible.cpp`) and the unique polar visitor body stay hand-written. `init_system.cpp` / `init_amr.cpp` are reorganized into concern-grouped internal helpers (assembly / program / checkpoint / physics / stepping / data) with no change to any `.def` name, docstring, arg, or registration order. New permanent audit `docs/design/pybind-binding-audit.md`, source gate `tests/python/architecture/test_pybind_seam_manifest.py`, and runtime sibling `tests/python/unit/runtime/test_seam_combinations.py`.
