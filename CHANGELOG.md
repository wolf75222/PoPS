...
- ADC-633 Compiled condensed-Schur time Program (std.condensed_schur, theta=1) on the AMR hierarchy: the condensed-Schur operators are context-generic (templated on the runtime facade, per-level assembly through AmrProgramContext::grid_context / schur_target / schur_source, matrix-free coefficiented apply, reconstruct, energy), so a flat hierarchy is bit-identical to the uniform Program (the emitted matrix-free BiCGStab runs on level 0 through ctx.solve_linear_schur) and a refined hierarchy solves the tensor Schur elliptic by the composite FAC (CompositeFacPoisson in a new amr_schur.hpp tail header), matching the native source-stage route; the vestigial AMR Schur deferral stubs are removed and the Spec 6 sec.20 clean_schur_program.amr.mono cell flips to green.
