...
- **Spec 5 sec.15 (#23/#27): strict de-stringing** -- opt-in `Program(strict_typed=True)` + `POPS_STRICT_TYPED` rejects `P.call("name")` and `P.rhs(flux=/sources=)`, naming the typed handle / `terms=` alternatives; byte-identical IR, default-off unchanged. New `python/tests/test_spec5_strict_typed.py` (epic ADC-479).
