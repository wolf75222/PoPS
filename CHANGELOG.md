...
- ADC-633 Compiled condensed-Schur time Program (std.condensed_schur, theta=1) on the AMR hierarchy: the condensed-Schur operators are context-generic (templated on the runtime facade, per-level assembly through AmrProgramContext::grid_context / schur_target / schur_source, matrix-free coefficiented apply, reconstruct, energy), so a flat hierarchy is bit-identical to the uniform Program (the emitted matrix-free BiCGStab runs on level 0 through ctx.solve_linear_matfree) and a refined hierarchy solves the tensor Schur elliptic by the composite FAC (CompositeFacPoisson in a new amr_schur.hpp tail header), matching the native source-stage route; the vestigial AMR Schur deferral stubs are removed and the Spec 6 sec.20 clean_schur_program.amr.mono cell flips to green.
...
- ADC-645 Expose the frozen tunables: typed composite-FAC backend overrides (`GeometricMG(fac=CompositeFAC(...))`) independent from the final `CompositeHierarchySolve` hierarchy policy and carried into `CompositeFacOptions` without silent fallback; CondensedSchur(n_precond_vcycles=) on the cartesian and AMR condensed source stages and CondensedSchur(polar_precond='radial_line'|'jacobi') on the polar stage (cross-geometry misuse refuses); Richardson(omega=) baked at emit; per-descriptor Krylov rel_tol supplying P.solve_linear's tol when the call site leaves it default; wave_speeds_from_jacobian(eig_max_iter=, im_tol=) baked into the emitted real_eig_minmax / Roe |A| gate (cache-key participating, fd_eps rule); System/AmrSystem.step_cfl(speed_floor=); and WENO5(epsilon=) threaded to the native Weno5 stencil on the uniform cartesian path (polar/disc/AMR refuse a non-default epsilon loud). Every knob at its default is bit-identical (omitted attrs, native-default trailing args); kWenoEpsilon and kCflSpeedFloor are reclassified public_knob.
...
- ADC-644 DirectSmallGrid(threshold) default changed from 100 to None ("governed by min_coarse").
  ...
  0, exactly today's behaviour, and a positive int now enables the real coarsening ceiling.
...
- ADC-632 Split the System implementation monolith (now `src/runtime/system/system.cpp`) by
...
- **ADC-595: typed `CouplingOperator` contract for inter-species couplings** -- the runtime coupled-source representation (`CoupledSourceProgram`, an opaque bytecode POD) is now wrapped by a typed contract in the new `include/pops/coupling/source/coupling_operator.hpp` (`ConservationContract`, `FrequencyBound`, `CouplingOperator`, `CouplingOperatorView`). `System::add_coupling_operator` / `AmrSystem::add_coupling_operator` register a coupling with a DECLARED conservation contract (which roles it CONSERVES versus CREATES) that is validated at registration on the host (fail-loud, `validate_coupling_contract`) against the actual output terms before the program is lowered through the SAME `add_coupled_source` path (bit-identical numerics), and record the contracts for a read-only `coupled_operators()` view (a raw `add_coupled_source` registers an "unchecked" entry). The new inspect metadata is a plain `Impl` member the `SystemProgramDriver` never reads (no MockImpl impact). One-time AOT `.so` regeneration is expected (adding the header shifts `POPS_HEADER_SIG`). New `tests/cpp/unit/runtime/test_coupled_source.cpp` missing-block case and the Python coverage below.
- **ADC-561: the short named-value API `T.value(name, expr)`** -- naming an intermediate SSA value no longer needs the heavy `T.define(U.stage("rhs_star"), expr)`. `python/pops/time/_program/solve.py` gains `Program.value(name, expr)` which lowers to the EXACT `program.define(name, expr)` path (an affine combination materializes via `linear_combine`, a `rate(U) == <expr>` equation keeps its right-hand side, any other Value is named in place), so `T.value("rhs_star", U.n + T.dt * R_n)` produces the byte-identical IR as the long `T.define` form and appears in `ir_nodes()` under its name. `U.stage(k)` stays the temporal-VERSION handle only (its docstring now points at `T.value` for free intermediates) and `T.define(U.next, value)` remains the commit door; passing a version handle to `T.value` is refused pointing at `T.define`, and the SSA invariants (single definition, no redefine, use-before-define) are unchanged. New `tests/python/unit/time/test_named_value.py`.
- **ADC-554: `pops.lib.time` macros and the manual Program are one IR route** -- a scheme macro is now a function that BUILDS an inspectable `Program`. The new `@program_macro` wrapper (`python/pops/lib/time/_helpers.py`) makes every macro dispatch on its first argument: a live `Program` keeps the historical in-place builder (byte-identical IR, unchanged result), while the block name alone builds a fresh `Program` and RETURNS it, so `isinstance(pops.lib.time.forward_euler("plasma"), Program)` (and every other macro) holds; the two forms lower through the SAME builder into the same logical IR. `Program.ir_nodes()` (`python/pops/time/_program/inspection.py`) exposes the generated nodes as a structured, array-free list (the machine-readable counterpart of `dump_operator_ir`). The `CompiledTime` public bypass is blocked: `System.add_block` / `System.add_equation` refuse `time=CompiledTime(...)` with a structured error pointing at the Case time scheme and the cadence path (`_reject_compiled_time_route`), while `CompiledTime` stays importable and constructible in its real `cadence=` role. New `tests/python/unit/time/test_macro_program_route.py` (macro-returns-Program, `ir_nodes` inspection, macro-vs-manual IR parity, the CompiledTime refusal).
- **ADC-560: `pops.model.OperatorHandle` is callable for tableau-style Programs** -- a handle now builds IR when called inside a Program: `R(U, f)` finds the Program from its Value arguments and delegates to the internal `P._call(name, ...)`, the SAME lowering the public `P.call(handle, ...)` uses, so `fields_n = fields(U.n); R_n = explicit_rate(U.n, fields_n); C_n = implicit_operator(fields_n)` reads like the tableau. The facade is byte-identical to `P.call` / `T.call` (same `_ir_hash`, same signature type-checks, same errors) and does ZERO numerics; a call outside a Program (no Value argument) is refused with a message naming the explicit `P.call`. `python/pops/physics/board_handles.py`'s `CallableOperator` is folded into an `OperatorHandle` subtype (kept one release), keeping its board self-binding of the model's fresh module and resolving its kind/signature lazily so `inspect()` works. New `tests/python/unit/codegen/test_operator_callable.py` (callable-vs-`P.call` IR parity across rate/source/linear, identical signature errors, no-numerics, one-handle fold).
- **ADC-559: `m.operator(...)` reads as a mathematical construction** -- the operator families are made explicit. `python/pops/model/operators.py` gains an `OPERATOR_FAMILIES` table and `operator_family(kind)` folding every operator kind into a readable family (`rate` / `field_solve` / `local_linear_map` / `matrix_free_map` / `projection` / `coupled_rate` / `diagnostic` / `residual`). The canonical `pops.model.OperatorHandle` now carries `signature` and `category` alongside `name` / `kind` and exposes `inspect()` (a structured `{name, kind, category, signature}` view); identity stays `(name, kind)` so a handle remains a dict key. Three mathematical facade declarers -- `m.rate(...)` -> `(U, Fields) -> Rate(U)`, `m.field_solve(...)` -> `U -> Fields`, `m.local_linear_map(...)` -> `Fields -> LocalLinearOperator(U, U)` -- funnel into the ONE typed registry (`rate_operator` / `elliptic_field` / `linear_source`; no parallel facade registry, module hash unchanged) and return the enriched handle. A bare unregistered math object (a board `local_linear_operator`) still raises when called directly. New `tests/python/unit/codegen/test_operator_families.py`.
- **ADC-607: memoize the AMR copy schedules and coarse-parent box lookups** -- `parallel_copy` no longer rebuilds a `BoxHash` and re-enumerates its redistribution job list on every call. The schedule is now memoized per layout pair by `CopyScheduleCache` (`include/pops/mesh/layout/copy_schedule.hpp`), mirroring the intra-level `HaloScheduleCache` (ADC-260): the cache lives mutable on the destination `MultiFab` (auto-dropped when regrid move-assigns a fresh dst) but each entry is keyed on a src-layout fingerprint (src `BoxArray` + `DistributionMapping`), because the schedule depends on BOTH layouts. Jobs carry global box indices resolved at replay and run in the same deterministic order as the inline enumeration, so the cached path is bit-identical to a per-call rebuild. The replicated-parent coarse-fine ghost fill (`mf_fill_fine_ghosts_mb`) replaces its per-cell `O(boxes)` `mf_find_box` linear scan with a precomputed dense cell -> local-box lookup (`MfBoxLookup`); the parent valid boxes are disjoint, so first-hit == only-hit and the lookup returns the identical box index. The regrid profiler exposes `tag_density` (the dense `TagBox` fill fraction, kept dense on purpose since a sparse grid would degrade Berger-Rigoutsos), `box_hash_rebuilds`, and `copy_cache_hits`/`copy_cache_misses` counters, surfaced through `pops.runtime.profile` (`by_amr_mpi`). New `tests/cpp/unit/mesh/test_copy_schedule_cache.cpp` (build count == 1 over K copies, cached result bit-identical to a cleared rebuild, src-layout invalidation, np=1/2/4).
- **ADC-588: typed `FieldContext` and `AuxLayout` for field solves** -- the "which aux component carries this output" contract, until now an unwritten convention (`phi`=0, `grad_x`=1, `grad_y`=2, then `B_z`/`T_e`/model-named), is now a typed descriptor. New host-only `include/pops/runtime/context/aux_layout.hpp` (`AuxChannel` handle->component, `AuxLayout` manifest with `component_of` naming the missing output and the known set, `default_poisson_layout()`) WRAPS the fixed component truth of `include/pops/core/state/state.hpp` without redefining any `kAux*` constant, and `include/pops/runtime/context/field_context.hpp` (`FieldContext`) is a validity token recording the (field problem, block, stage) that produced a solve so a stage-k context cannot be silently read as stage-k'. On the Python side `P.solve_fields(...)` now tags its returned value with a real `pops.time.field_context.FieldContext` (previously a docstring-only concept) carrying the problem/block/stage-source and the output handles; the default solve keeps byte-identical IR (`attrs == {}`) so the compiled-Program cache key and trajectories are unchanged (the context is build-time metadata, never serialized). New `tests/cpp/unit/runtime/test_field_context.cpp` and `tests/python/unit/time/test_field_context.py`.
- **ADC-596: native `FieldProblemRegistry` unifying the default and named field problems** -- the shared Poisson (the `ell_` / `mg_` solve) and each named elliptic field (`NamedField`) were two parallel bookkeeping paths validated late per backend; they are now described ONCE by a native `include/pops/runtime/system/field_problem_registry.hpp` (`FieldProblemEntry`: id, `EquationKind`, output `AuxLayout`, `FieldBoundaryKind`, `EllipticSolverKind`, Uniform/AMR route support; `FieldProblemRegistry` with `register_problem`/`find`/`at`/`validate`/`validate_all`). Both the Uniform `SystemFieldSolver` and the AMR `AmrRuntime` `register_named_field` seams now mirror their fields into the registry (via `named_field_entry`, seeding the default `default_poisson_entry()`), and expose it through `field_problem_registry()`. The typed `validate(id, route)` refuses an illegal combination before bind (FFT on AMR, FFT with a Dirichlet potential, an entry on an unsupported route, or a problem with no output) with a message naming the field problem and the offending axis, and `field_problem_routes` (in `module_capabilities.hpp`) surfaces each entry as a `field_problem:<id>` report row plus a static `elliptic:field_problem_registry` capability row. The registry owns no solver: the lazy `ensure_named_elliptic` build and `assemble_poisson_rhs` stay the numerical truth, the `NamedField` phi/grad components become the derived low-level view of an entry's `AuxLayout`, so trajectories are bit-identical. New `tests/cpp/unit/runtime/test_field_problem_registry.cpp` and `tests/cpp/integration/runtime/test_field_registry_uniform_amr.cpp`.
- **ADC-587: ProgramContext is a generic runtime facade; the condensed-Schur / Lorentz kernels move to a native operator module** -- the aux-aware Schur functors and the seven Schur/Lorentz seam methods (`assemble_schur_coeffs`, `apply_laplacian_coeff`, `schur_explicit_flux`, `assemble_schur_rhs`, `schur_reconstruct`, `schur_energy`, `geometric_mg_precond_apply`) leave `include/pops/runtime/program/program_context.hpp` VERBATIM (zero numerical change) for the new module `include/pops/coupling/schur/program/` (`schur_program_kernels.hpp` + `condensed_schur_operator.hpp`, free functions taking a `ProgramContext&`, plus a persistent `GeometricMgPreconditioner` holding the V-cycle cache). The facade now carries no Schur/Lorentz/electrostatic token and drops the `schur_condensation.hpp` / `geometric_mg.hpp` / `lorentz_eliminator.hpp` includes; it gains a generic `grid_context()` accessor. The codegen lowers the Schur ops to the module free functions and emits the `coupling/schur/program` include ONLY when a Schur op is in the IR, so a Schur-free Program's generated `.so` no longer pulls in `coupling/schur/**`. A structured `pops.model.schur_route_manifest(program)` makes the condensed-Schur route and its `theta == 1` bit-exact limitation machine-visible. New `tests/python/architecture/test_no_schur_header_leak.py` (source-parse token/include hygiene + Schur-free vs condensed-Schur emit), `tests/cpp/unit/elliptic/test_condensed_schur_operator.cpp` (native parity of the extracted kernels, bit-identical to the pre-split path) and `tests/cpp/unit/runtime/test_program_context_schur_free.cpp` (the trimmed facade builds standalone). The condensed-Schur IR hash and the native `pops.CondensedSchur` stepper are unchanged.
- **ADC-555: refinement belongs to the layout mental model, not a separate `problem.amr` route** -- `case.amr.refine(criterion, regrid=, nesting=, patches=)` was already a thin authoring shim writing straight onto the case's `AMR` layout descriptor (never a second engine); this confirms and locks the full chain (refine/regrid/nesting/patches all land on `case.layout`, each independently settable across calls, a bogus role refused before any layout mutation) with new `tests/python/unit/mesh/test_case_amr_refine_chaining.py`. Closes the remaining gap in `compiled.inspect_amr()`: a `CompiledModel` produced by the AMR route now reports the ACTUAL compiled layout (its refine/regrid/... tags) by default instead of silently falling back to the generic native envelope, since `pops.compile`'s AMR path already attaches the layout as `compiled._layout` but `inspect_amr()` ignored it unless passed explicitly; an explicit `layout=` argument still overrides. `docs/ALGORITHMS.md`'s "which scheme when" table now points at `layout=AMR(refine=Refine.on(...))` instead of the internal `AmrSystem.set_refinement` vocabulary.
- **ADC-589: `layout=AMR(...)` + typed refine policies are the only AMR configuration path** -- a `Uniform(...)` layout that carries an active refinement criterion (`refine=Refine.on(...)` / `TagUnion(...)`) is now refused by `Case.validate` by default (a single-level layout has no level to refine onto, so a criterion is never silently ignored); the new `pops.mesh.amr.IgnoreAMRCriteria()` marker descriptor is the explicit escape (`Uniform(mesh, refine=..., ignore_amr=IgnoreAMRCriteria())`). `sim.amr.inspect()` now returns a unified `pops.runtime.amr.RuntimeInspection` composing `{hierarchy, patches, regrid, limitations}` in one call (the four things the AMR route must answer: config + live hierarchy, the live patch census, the regrid cadence/criteria, and the non-available native-route limitations), replacing the bare `HierarchySnapshot` it returned before; `hierarchy_snapshot()` / `patch_table()` / `explain_regrid()` stay reachable standalone and unchanged. New `tests/python/unit/mesh/test_uniform_refine_refusal.py`; extended `tests/python/architecture/test_no_legacy_runtime_routes.py` and `tests/python/integration/amr/test_amr_runtime_inspect.py`.
- **ADC-592: explicit runtime freeze lifecycle after `pops.bind`** -- the runtime lifecycle is now explicit (`assembling` -> `bound` -> `running`): `pops.bind` freezes the composition, so every structural setter (blocks, field problems, AMR layout, source stage, refinement, solver routes, aux layout, installed Program) refuses with a precise bind-vocabulary error once bound, while runtime data / params / checkpoint / diagnostics stay mutable. New native `System::mark_bound()` / `lifecycle_state()` (and the AmrSystem counterparts) plus a Python-layer guard that stays effective under a prebuilt `.so`; a `BoundSnapshot` manifest of WHAT was bound (stable `snapshot_hash`) surfaced by `sim.inspect()`; and a compile-time snapshot authority so `pops.bind` lowers from the frozen artifact and a Case mutated after `pops.compile` is rejected loudly. New `tests/python/integration/runtime/test_freeze_lifecycle.py`.
- **ADC-552: typed wave-speed providers replace the string `capability("wave_speeds")` lookup** -- a new `pops.numerics.riemann.waves.WaveSpeedProvider` (typed descriptor, Spec 5 sec.6) names the SOURCE of a model's signed wave speeds without ambiguity: the factories `ExplicitPair()` / `FromJacobian(eig=, blocks=)` / `FromPressure()` / `Einfeldt()` / `Davis()` (signed pair, HLL-usable) and `MaxWaveSpeed()` (the unsigned Rusanov majorant, refused by HLL), each declaring its `requirements()` / `capabilities()` / `describe()`. `HLL(waves=<provider>)` now validates the contract (a bare string is rejected pointing at the typed factories, a non-signed provider is refused with a precise message) and folds the provider into the descriptor `options["waves"]` / `requirements`; the provider rides through `Spatial.waves_provider` to the shared install guard (`pops.numerics.riemann.waves.check_hll_waves` -> `pops.runtime.routes.check_wave_speed_provider`), which cross-checks the requested provider against the compiled model's actual source (`provider_of`) and refuses a mismatch (documented honest limitation: a bare `CompiledModel` whose source kind is unrecorded is accepted when `has_wave_speeds`). Typed capability handles expose it as `m.capabilities.wave_speeds` / `compiled.capabilities.wave_speeds` (a model that declares none raises a precise error, never a silent `None`). `compiled.requirements()` gains the additive `wave_speed_provider` key and `pops.inspect_capabilities()` gains `wave_speeds:*` rows; the moment-hierarchy `pops.moments.speeds.ExactSpeeds` stays the separate strategy axis. New `tests/python/unit/physics/test_wave_speed_providers.py`.
- **ADC-600: quarantine the host/prototype hot-path routes off the compile/bind target surface** -- every `_BACKEND_CAPS` row (`python/pops/codegen/compile_emit.py`) and the `doctor.capabilities()` backend rows now carry a `tier` (`production` | `prototype` | `internal`), and the typed backend descriptors gain a `.tier` property (`JIT` -> `prototype`, `AOT` -> `internal`, `Production` -> `production`), so a report names plainly whether a route is the production native loader, a host prototype (`add_dynamic_block`), or an internal host-marshalled harness (`add_compiled_block`). `compile_native` now refuses early with an explicit sentence that the production native route is REQUIRED and prototype/host routes are NOT a fallback; the C++ `dynamic_model.hpp` / `native_loader.hpp` adder docs are marked INTERNAL/PROTOTYPE (never a fallback). New source-only `tests/python/architecture/test_prototype_route_quarantine.py` (target surface free of the host/prototype routes, production-only refusal string kept, `pops.experimental` off the root, every caps row carries a legal tier) and `tests/python/unit/runtime/test_prototype_quarantine.py` (runtime refusal + descriptor tiers + experimental isolation + no Python-callback token in a compiled Program step).
- **ADC-586: a declarative native brick catalog replaces the model-factory tag dispatch** -- the builtin native bricks (transports `exb`/`compressible`/`isothermal`, sources `none`/`potential`/`gravity`/`magnetic`/`potential_magnetic`, elliptics `charge`/`background`/`gravity`) are now described in one light table, `include/pops/runtime/builders/factory/brick_catalog.hpp` (`kBrickCatalog`: id, category, typed route index, native entry, ModelSpec constructor params, n_vars, polar_ok, requirements, capabilities, summary, plus `catalog_entry`/`catalog_csv`/`brick_catalog_json`), mirrored by `python/pops/runtime/brick_catalog.py` (`brick_catalog()`/`resolve()`, re-exported on `pops.runtime`). The header static_asserts every column against BOTH single sources (`model_registry.hpp` and `route_ids.hpp`), so a brick edited in one table fails the build until every mirror agrees. `model_factory.hpp`'s `dispatch_transport`/`dispatch_source`/`dispatch_elliptic` now parse the tag ONCE into its typed route (`parse_*_route`, route_ids.hpp) and `switch` on the enum instead of a public string if-chain (the alias source spellings are resolved by `parse_source_route`, so the switch has one case per canonical brick); the historical rejection messages and the constructed brick values stay byte-identical. `build_module_manifest` adds a `native_catalog` component (registry version + canonical ids) so a generated artifact can reference the native bricks by id. New `tests/test_brick_catalog.cpp`, `tests/python/architecture/test_brick_catalog_parity.py` (source-only mirror parity plus the acceptance gate refusing a new hand-coded brick-token list in an error-message string) and `tests/python/unit/runtime/test_brick_catalog.py`.
- **ADC-585: Module / Operator manifests replace ModelSpec as the central model representation** -- the operator-first `pops.model.Module` now serialises to a self-describing, JSON-ready `ModuleManifest` (`Module.manifest()` / `pops.model.build_module_manifest`): the state/field spaces, params, aux, per-direction eigenvalue presence, and a typed `OperatorRegistryManifest` (each operator by its stable integer id in registration order, with its signature/inputs/output space or `RateBundle` names, capabilities, requirements and lowering route), plus the native route-registry components (`version`/`hash`/`signature` from `pops.runtime.routes`) and an ABI-requirements slot the compile seam binds. `compile_problem` attaches the resolved model's manifest to the `CompiledProblem` handle (`module_manifest`, cache-hit and fresh paths, abi_key bound from the handle) and `compiled.inspect()` surfaces it under the report's `module_manifest` key. The legacy flat `ModelSpec` POD is quarantined off the `pops` root (`pops.ModelSpec` removed) to `pops.runtime.ModelSpec` for the native `add_block` bridge only; a new source-only `tests/python/architecture/test_modelspec_quarantine.py` gates the operator-first / codegen surface free of it.
- **ADC-599: the compiled-artifact cache key and manifests now carry the typed route registry** -- every cache key (model .so, Program .so, external library .so) gains a route-registry / report-vocabulary component (`_registry_cache_key`: `ROUTE_REGISTRY_VERSION` + `route_registry_hash()` + `CAPABILITY_VOCAB_VERSION` from `pops.runtime.routes`), so a native-catalog change (a route added/removed/re-tokenized, a native entry renamed) is a cache MISS instead of a silent reuse; runtime-only params keep NOT recompiling (locked by `tests/python/unit/codegen/test_cache_key_routes.py`). Generated artifacts embed their build-time registry signature (`pops_compiled_route_manifest` / `pops_program_route_manifest`), and the loaders refuse a stale artifact BEFORE install with the first mismatching family named (`pops::verify_route_manifest`, `route_ids.hpp`; old artifacts without the symbol stay loadable, append-only). `compiled.inspect()` exposes the participating components under `options["cache_key"]["route_registry"]` (version, hash, signature, capability vocabulary version).
- **ADC-584: typed native route IDs replace behavior strings as the selection mechanism** -- every algorithmic choice (Riemann flux, limiter, reconstructed variables, time treatment, splitting, field solver, Poisson bc/rhs, wall, layout, transport/source/elliptic bricks, condensed source stage) now has a typed route: `include/pops/runtime/config/route_ids.hpp` (one `enum class` + `RouteInfo` table per family, `parse_*_route` refusing an unknown token with the family, the requested token and the valid set -- never a default -- and `route_token` as the single typed-ID-to-string adapter, static_asserted row-for-row against the historical `kLimiters`/`kRiemanns`/`kTransports`/`kSources`/`kElliptics` registries) mirrored by `python/pops/runtime/routes.py` (`Route`, a `str` subclass so the wire crossing stays byte-identical, carrying `id`/`native_entry`/`requirements`/`limitations`; `pops.runtime.route_manifest()` is the structured inspection). The Python lowering layer now carries Routes end to end (`Spatial.limiter/.flux/.recon`, `Explicit/IMEX/IMEXRK.kind`, `Split/Strang.scheme`, `CondensedSchur.kind`) with `.routes()` inspection on the spatial/time bricks; `System.set_poisson` pre-validates `rhs`/`solver`/`bc`/`wall` routes before the C++ boundary; the flux-guard refusals cite the requested route and its native entry point. The silent `add_compiled_model` time fallback (an unknown token ran SSPRK2) is now a typed refusal. Parity of the two registries is locked source-only by `tests/python/architecture/test_route_registry_parity.py`; `tests/test_route_ids.cpp` covers round-trips, refusals and aliases (`lorentz`/`potential_lorentz`/`ssprk2` parse-only spellings).
...
- **`scripts/uninstall_pops.sh`: one-command teardown, the inverse of `setup_env.sh` + `build_python.sh`** -- a bare run removes the in-tree build artifacts (`build/cp3*/` wheel cache, `.pops_cache`, `*.egg-info`) and deletes the `pops` conda env (its pinned toolchain and `POPS_*` vars included), prompting once before the env delete; `--keep-env` removes only the module (`pip uninstall pops`) plus the artifacts, `--ccache` also clears the shared `~/.cache/adc-ccache`, and `--yes` skips the prompt for CI / scripted use. Documented in `installation.md` ("Update or clean rebuild").
- **Spec 6 sec.11 (ADC-503): AMR multi-block via `pops.Case(layout=AMR(...))` -> `compile`/`bind`** -- `pops.compile` no longer defers a multi-block AMR assembly. An AMR layout (single OR multi block) now compiles EACH block's resolved physics to a `target="amr_system"` production `CompiledModel` (the native AMR `.so` loader, `add_native_block`) and carries the `{block: CompiledModel}` table on the handle (`_block_compiled_models`); there is no whole-system time Program on AMR (`AmrSystem` has no `install_program` seam), so the AMR route does not call `compile_problem` and does not require `time=` (the native blocks carry their own per-block time policy). `pops.bind` installs the AMR route through the NATIVE path (`_install_compiled(compiled=None, instances=...)`, each instance carrying its own `CompiledModel`), reusing the existing `AmrSystemConfig` derivation, Poisson flow and typed refinement. A multi-block refine on a non-density subject now forwards to `set_refinement(threshold, variable=...)` (the union-of-tags engine resolves it per block); the single-block component-0-only behaviour and the `Uniform` route (`compile_problem`) are byte-identical. The `.so` emit + multi-block AMR run are Kokkos-gated (ROMEO); the new `tests/python/unit/runtime/test_problem_orchestration.py` / `test_case_multiblock_amr.py` / `test_spec5_amr_e2e.py` cases validate the routing under monkeypatch.
- **Spec 6: AMR checkpoint/restart for multi-block and MPI np>1** -- `AmrSystem.checkpoint`/`restart` no longer reject multi-block or `n_ranks() > 1`; both routes are now bit-identical (frozen hierarchy, `regrid_every == 0`). The C++ gains append-only collective per-level accessors `AmrCouplerMP::level_state_global`/`level_potential_global` (mono-block) and per-block `AmrRuntime::block_level_state`/`block_level_state_global`/`set_block_level_state` + shared `level_potential`/`level_potential_global`/`set_level_potential` (multi-block), surfaced on `AmrSystem` (`level_state_global`, `level_potential_global`, `block_n_vars`, `block_level_state`, `block_level_state_global`, `set_block_level_state`) and bound in Python. The checkpoint format becomes v2 (per-block `state_<block>_<k>`, shared `phi_<k>`); v1 (mono-block single-rank) still restores. Under np>1 the `_global` accessors `all_reduce_sum` the distributed fabs onto rank 0, which alone writes the single file (mirror of `System.checkpoint`); the multi-block shared hierarchy is the deterministic frozen central patch, reproduced by replaying the composition (no `set_hierarchy` on the runtime). The single-rank multi-block path is locally validated; the np>1 gather is CI-MPI/ROMEO-gated. New `test_amr_checkpoint_multiblock_bit_identical` in `tests/python/integration/io/test_amr_checkpoint.py` (epic ADC-479, ADC-509).
- **Spec 6 sec.11 (ADC-508): the compiled time-Program install seam on `AmrSystem`** -- `AmrSystem::install_program(so_path)` now loads a generated `problem.so` on the AMR hierarchy (dlopen + ABI-key check + section-24 block-instance / solver requirement validation + name-based block binding + runtime-param seeding, mirroring `System::install_program`), backed by the AMR-owned `set_program_cadence` / `seed_program_params` / `set_program_params` / `program_params` carriers and an `install_program_step` macro-step hook that `AmrSystem::step` routes through (global substeps/stride cadence). The codegen emits a `pops_install_program_amr` loader export when a Program is compiled with `target='amr_system'` (`compile_problem(..., target='amr_system')`), and `pops.bind` now installs a compiled Program on `AmrSystem` and routes `params=` / `cadence=` to the new seams (the three former `_install_compiled` `NotImplementedError` rejects are real routes). The per-level Lie/Strang macro-step driver (`AmrProgramContext`) is WIRED and CPU-validated: the generated `pops_install_program_amr` constructs an `AmrProgramContext` and installs the synchronous per-level macro-step, bit-identical to the same Program on `System` for a single-level AMR Program (the must-pass parity gate, evolved coarse density matches to 0 ulp on Kokkos CPU); the CUDA/GPU run is the ROMEO step. New `tests/python/integration/amr/test_amr_install_program.py`; updated `tests/python/integration/runtime/test_install_validation.py` (epic ADC-511, ADC-508).
- **ADC-508: the `AmrProgramContext` per-level driver -- a compiled time Program over the AMR hierarchy** -- the new header-only `include/pops/runtime/program/amr_program_context.hpp` is a DUCK-TYPED structural mirror of `ProgramContext`: the SAME lowered Program body (which references only the variable `ctx`, never the type) now compiles against it. The codegen `_emit_amr_install` replaces its fail-loud throw with a real install -- `pops_install_program_amr` constructs an `AmrProgramContext` over the `AmrSystem` and installs a SYNCHRONOUS per-level macro-step: the identical body wrapped in a per-level loop (`ctx.set_level(k)`), then `ctx.couple_levels()` (fine->coarse `average_down`). A single coarse system Poisson per macro-step (OncePerStep, level-0-guarded `solve_fields`) is injected coarse->fine; the head-of-step union-tags regrid runs at the engine cadence. The driver wraps the `AmrRuntime` engine (append-only public seam ops: `level_state` / `level_geom` / `level_scalar_field` / `level_rhs_into` / `level_neg_div_flux_into` / `level_source_into` / `level_max_speed` / `level_hmin` / `average_down_level` / `regrid_if_due`, plus a per-block `level_rhs` / `level_neg_div_flux` / `level_source` closure built from the SAME device-clean `BlockRhsEval` System uses), reached through new `AmrSystem` accessors (`engine` / `profiler_handle` / `record_program_diagnostic` / `program_diagnostic[s]`, and `macro_step` is now `POPS_EXPORT`). A single-block AMR Program builds a COARSE-ONLY hierarchy (a `make_shared_amr_layout` opt-in, untouched for the native multi-block path), so a single-level AMR Program is BIT-IDENTICAL to the same Program on `System` (the must-pass parity gate: the evolved coarse density matches to 0 ulp). v1 is SYNCHRONOUS (same dt every level + `average_down`); Berger-Oliger subcycling, conservative reflux, multi-block AMR Programs, named multi-elliptic fields, per-stage fine-level field re-solve and AMR history rings are DEFERRED (documented / fail-loud, never a silent gap); the CUDA run is the ROMEO step. New `tests/python/integration/amr/test_amr_program_parity.py` (epic ADC-511, ADC-508).
- **Spec 6 sec.8/9: an AMR-incompatible field solver is refused at `Case.validate` / `pops.compile`** -- `Case.validate` now carries the mesh layout into each `FieldProblem.validate(context)`, and on an AMR route a field problem asks its solver's `available(context)`: a hard `no` (the spectral `FFT` on an `AMR` hierarchy) is refused with the solver's precise message (`"FFT requires Uniform(periodic=True), got AMR. Use GeometricMG()."`). The check is SCOPED to an AMR layout, so a Uniform route, a bare `BrickDescriptor` solver (bool `available`), or a solver whose `available()` returns a non-layout `no` / raises are never refused (no false positive); it completes the descriptor-level reject from ADC-512 end-to-end. New tests in `tests/python/unit/runtime/test_problem_orchestration.py` (epic ADC-511, ADC-516).
- **Spec 5 sec.5.14 (C4): typed output/checkpoint policies fire on the Uniform run loop** -- `Case.validate()` no longer defers a non-empty output policy (the `NotImplementedError` is removed); a valid `OutputPolicy`/`CheckpointPolicy` lowers and `pops.bind` flows them onto the bound `System` (a new `outputs=` arg on the `_install_compiled` seam, stored on `System._output_policies`). `System.run(output_dir=...)` then fires each at its policy cadence (`every(N)`/`always`/int) through the EXISTING `write()`/`checkpoint()` writers via a new pure-Python `pops.runtime._output_driver` hook (no C++, no codegen): the typed format maps to the writer string (`HDF5()` -> `hdf5`, default -> `npz`), `fields=` to `write(fields=)`, and the level selection (`AllLevels`/`CoarseOnly`/`SelectedLevels`) is a documented no-op on a single-level `System`. `Plotfile()` is the one precise reject (AMReX per-level format, no Uniform writer, deferred to ADC-511); AMR per-level writes stay ADC-511. New `tests/python/unit/runtime/test_output_policy_run.py` (local end-to-end on a real native System, no Kokkos compile) (epic ADC-479, ADC-509).
- **Spec 6 sec.4/8/9: solver route capabilities + precise FFT-on-AMR rejection** -- the matrix-free Krylov solvers (`CG`/`BiCGStab`/`GMRES`/`Richardson`) and the `Schur`/`CondensedSchur` solver now declare their route `capabilities()` (`supports_uniform`/`amr`/`mpi`/`gpu` all True, via a forwarded `descriptors._native(capabilities=)` kwarg), so introspection (`inspect()`) and any future route check can see they are AMR-capable instead of reading an empty set. `FFT.available(context)` is now layout-aware: with an AMR layout in the context it refuses with the PRECISE message `"FFT requires Uniform(periodic=True), got AMR. Use GeometricMG()."` (Spec 6 sec.8) instead of a vague "AMR unsupported", while a Uniform / no-layout context keeps the plain route-constraint `partial`. AMR is detected by duck-typing the layout's `capabilities()["layout"]`, so the solvers layer stays import-graph-clean (no `pops.mesh` edge). New tests in `tests/python/unit/descriptors/test_solvers_descriptors.py` (epic ADC-511, ADC-512).
- **ADC-428: named elliptic fields lower on the AMR layout** -- a second (user-named) elliptic field declared via `m.elliptic_field("psi", rhs=..., aux=[...])` now solves independently on the AMR hierarchy, mirroring the uniform System path (PR #379). The AMR native loader emits `register_elliptic_field` + `set_block_elliptic_field` on the `AmrSystem` facade (instead of the prior `NotImplementedError`); `AmrRuntime` owns a dedicated coarse `GeometricMG` per named field, sums its RHS from the per-block bricks, writes phi + centered grad into the field's own aux components and injects them to the fine levels each `solve_fields` (the default Poisson path is byte-identical). Registering a named field forces the multi-block runtime engine even for a single block. `AmrSystem._install_solver` routes a model-declared named field and rejects a typo, and `sim.field(name)` returns the solved coarse potential. New C++ test `tests/test_amr_named_field.cpp` (engine-level parity vs the default Poisson) and `tests/python/integration/amr/test_amr_named_field.py`; the `.so` end-to-end is Kokkos-gated (ROMEO).
- **Spec 5 sec.7 (C6): the GeometricMG preconditioner route for `solve_linear`** -- `P.solve_linear(method=GMRES()/BiCGStab(), preconditioner=preconditioners.GeometricMG())` now lowers to a REAL preconditioner ApplyFn (one V-cycle of the already-wired `pops::GeometricMG`, reused via a new `ProgramContext::geometric_mg_precond_apply`) instead of the transitional `NotImplementedError`; the identity path stays byte-identical (`pops::ApplyFn{}`). A non-identity preconditioner with `CG()`/`Richardson()` is rejected (those loops have no preconditioner slot) and the planned `Jacobi()`/`BlockJacobi()` are rejected as "planned, not wired"; the `.so` convergence is Kokkos-gated (ROMEO). New tests in `tests/python/unit/time/test_time_solve_linear.py` (epic ADC-479).
- **Spec 5 C5 (ADC-510): runtime parameters in a compiled time Program** -- a Program whose physics reads `dsl.Param(..., kind="runtime")` in a directly-lowered source / linear-source kernel now lowers `ctx.program_params(block).get(index)` (replacing the former "a later phase" reject), backed by a per-block `pops::RuntimeParams` owned by the System (`System::seed_program_params` / `set_program_params` / `program_params`); `install_program` seeds the declaration defaults from the new `pops_program_param_*` `.so` metadata and `pops.bind(params=...)` routes the values to `set_program_params`, so a different `params=` value yields a different result without recompiling the `.so` (the no-recompile contract mirrored from the AOT-native `set_block_params`) (epic ADC-479).
- **Spec 5 sec.11 (#3/#7): `pops.bind` is the public path; `pops.physics.Model` is a writing facade** -- the System / AmrSystem `install` method is renamed to the internal `_install_compiled` seam (off the public surface and `__all__`), so `compiled = pops.compile(...)` then `sim = pops.bind(compiled, state=, params=, aux=, solvers=)` then `sim.run(...)` is the one documented flow (`pops.bind` lowers through the seam); the early-validation messages name `pops.bind(...)`. `pops.physics.Model` now exposes `lower()` / `to_module()` returning the `pops.model.Module` that `pops.compile` consumes and drops its direct `compile()` (no compilation from the physics facade). Architecture assertions added in `tests/python/architecture/test_spec5_public_api.py` (epic ADC-479).
- **Spec 5 sec.7 (#11): FFT-on-non-periodic-BC enforced** -- `FieldProblem.validate()` rejects a periodic-only solver (FFT, `supports_wall_bc` False) paired with a Dirichlet/Neumann boundary, naming `Periodic()` / `GeometricMG()`; no false positive (FFT+Periodic and GeometricMG+Dirichlet still pass) (epic ADC-479).
- **Spec 5 sec.13.12 (#36): the `.so` manifest emits layouts/platforms** -- `compiled_manifest_json()` now emits `supports_uniform/amr/mpi/gpu` (artifact-honest: the AOT route is uniform single-rank, `gpu` is the build's device token), and `apply_native_manifest` overlays them so the artifact manifest supersedes the model-caps overlay (epic ADC-479).
- **Spec 5 sec.12.5 (#43): AMR/MPI profiling counters** -- the `AmrRuntime` times `regrid` / `fill_boundary` / `average_down` into an `AmrSystem`-owned `Profiler` (+ `mpi_reductions` / `mpi_messages` under MPI np>1, `enabled()`-guarded, MockImpl-safe); `AmrSystem` gains `enable_profiling` / `profile_report` + a typed `profile()` context, and `PerformanceSummary.by_amr_mpi()` surfaces them (declared-unavailable on a host / non-AMR build). New `tests/python/integration/mpi/test_spec5_amr_mpi_counters.py` (epic ADC-479).
- **Spec 5 sec.15 (#23/#27): the time Program requires typed operators + terms (one public path)** -- the public `P.call` now requires an operator HANDLE (a bare string operator name is refused, naming `m.rate(...)`/`m.field_operator(...)`/`m.source_term(...)`) and the public `P.rhs` requires the typed `terms=[Flux(), source]` list (the legacy `flux=`/`sources=`/`fluxes=` and a bare `P.rhs` are refused, naming `terms=`); no opt-in flag (`Program(strict_typed=)` / `POPS_STRICT_TYPED` removed). The byte-identical string/`flux=`/`sources=` builders survive only as the internal `Program._call` / `_rhs_legacy` that the `pops.lib.time` macros and the typed front doors lower through (same `_ir_hash`). New `tests/python/unit/runtime/test_spec5_typed_program.py` (epic ADC-479).
- **Spec 5 (#19): solver-gen DSL -> `pops.codegen.solvers` (internal/experimental)** -- the `@solver` DSL + `generate_solver_cpp` move out of `pops.lib.solvers` into `pops.codegen.solvers` (internal-marked); `pops.lib.solvers` keeps only the preset re-exports (epic ADC-479).
- **Spec 5 (#7): `pops.lib` is presets-only** -- the `lib.spatial` / `lib.fields` brick catalogs move to `pops.numerics.spatial` / `pops.fields.catalog`; `pops.lib` now holds only models / time / presets, enforced by a tightened architecture test (epic ADC-479).
- **Spec 5 sec.13.12: authoritative native capability matrix** (ADC-479, criteria #36/#37): capability values now come from the C++ core via `pops::module_capabilities()` + `_pops.__abi_version__` (not a Python descriptor walk); `inspect_capabilities()` cross-checks the native source and fails loud (`CapabilityMismatchError`); `Problem.explain_routes()` prints the route matrix; `POPS_DEFINE_COMPILED_BLOCK` emits a per-`.so` `pops_compiled_manifest()` JSON. `supports_partial_imex_mask` is honestly false (no C++ path backs it).
- **Spec 5 sec.12.5: elliptic-solver native profiling counters** (ADC-479, criteria #42/#43): the opaque `field_solve` profiler scope is broken into `mg_cycles` / `krylov_iters` / `mg_levels` / `elliptic_bottom`, read back from `GeometricMG` at the field-solve seam (guarded by `enabled()`, zero cost when off); `PerformanceSummary.by_elliptic()` surfaces them.
- **Spec 5 sec.7 / sec.8.6 early-validation gaps: incompatible solver, WENO5 ghost depth, Refine role existence** (epic ADC-479, criteria 11 and 31): three pre-runtime checks a sec.15 audit surfaced, each under a no-false-positive discipline (reject only a KNOWN / declared incompatibility, never a problem that legitimately works today). `pops.fields.PoissonProblem.validate` now cross-checks the chosen elliptic solver's declared capabilities against the operator kind: a `ScreenedPoissonProblem` / `AnisotropicPoissonProblem` paired with a solver that declares `supports_screened` / `supports_anisotropic` literally `False` (the spectral `FFT`) is refused with an actionable message naming the operator and pointing at `pops.solvers.elliptic.GeometricMG()`; a solver that declares `supports_variable_epsilon` (GeometricMG subsumes a screened / anisotropic-by-coefficient operator) or that exposes no `capabilities()` surface at all (a bare object, an external brick) is NOT rejected. `pops.numerics.reconstruction` now declares each scheme's required ghost depth (WENO5 / WENO5Z >= 3, MUSCL >= 2, first-order 1) and a `validate_ghost_depth(reconstruction, available)` checker, wired onto `Spatial.validate(ghost_depth=)`: a reconstruction whose declared stencil exceeds an EXPLICIT block halo raises a clear error ("WENO5 requires ghost_depth >= 3, block 'plasma' has ghost_depth=2"), while WENO5 with no explicit constraint (the native runtime grows the halo to match the scheme) and MUSCL at depth 2 pass. `pops.mesh.amr.Refine.validate(model)` now checks the refinement subject (role / state component / named aux) against the model's declared subjects (read duck-typed, no physics/model import, so the mesh layer stays import-graph-clean) and rejects a bogus subject listing the declared ones; with no model context it self-validates the predicate/threshold shape and defers the subject check to `problem.amr.refine`, where the block model is available, so the role check happens before runtime. New `tests/python/unit/runtime/test_spec5_validation_gaps.py`.
- **Spec 5 sec.15 audit follow-ups: route matrix + Optimization de-string + Problem is not a Descriptor** (epic ADC-479, criteria 37 and 20-21): `pops.Problem.explain_routes()` (sec.13.12.1) returns a printable, inert route matrix (feature x layout x backend x platform -> status / limitation / error_message) sourced from the assembled descriptors' `available()` / `capabilities()` and the honest backend capability table, reading metadata only (compiling, binding and running nothing) and returning an honest empty envelope when the problem has no block yet, in new `pops._route_matrix`. `pops.codegen.Optimization` now rejects a bare string for its typed `math=` / `fuse=` algorithm selectors at construction with a clear, actionable message (e.g. a typed StrictMath() / FastMath() / DebugMath() / GpuRegisterAware(), not the string "fast") instead of silently mis-setting and crashing later in `options()`; the typed usage is unchanged and a bare `Optimization()` still defaults to StrictMath. `pops.Problem` is no longer a `pops.descriptors.Descriptor` (sec.6 table: a Problem is an assembly that contains descriptors, not one itself); its chaining and inspectable surface (`block` / `field` / `param` / `aux` / `output` / `time`, `layout` / `amr`, `validate` / `available` / `inspect` / `options` / `requirements` / `capabilities` / `lower`) is implemented directly so it still duck-types the descriptor protocol. New `tests/python/unit/runtime/test_spec5_routes_optim.py` plus architecture assertions in `tests/python/architecture/test_spec5_typed_api_negatives.py`.
- **Spec 5 sec.12.4 `POPS_*` codegen environment completeness + JIT-backdoor guard** (epic ADC-479, criteria 47 and 48): `compile_problem` now resolves the codegen environment through a new stdlib-only `pops.codegen.env.CodegenEnv` (additive contract, an explicit Python argument always wins over the env, lenient coercion). `POPS_CODEGEN_DIR` redirects the out-of-source `.so` plus any kept source / dump into a chosen directory; `POPS_KEEP_GENERATED` keeps the generated `.cpp` next to the `.so` (the same effect `compile_problem(debug=True)` has, which always wins); `POPS_DUMP_IR` / `POPS_DUMP_CPP` dump the IR / C++ after a successful compile or cache hit via the existing `compiled.dump_ir()` / `dump_cpp()`; `POPS_CODEGEN_LOG` / `POPS_LOG` set a simple, honest quiet-by-default log level for the compile path (the codegen-specific name wins). Because no autotuning engine exists, `POPS_AUTOTUNE` is rejected on presence and is absent from artifact snapshots and reports; no inert tuning level is accepted. `POPS_JIT_BACKDOOR` is a disabled-by-default, never-implicit UNSAFE debug gate: when set it emits a loud warning and is surfaced in `compiled.inspect()` and flagged by `pops.doctor()`, so it can never be silently honored (no backdoor behavior is wired -- the guard is the deliverable). The resolved environment snapshot is recorded on the `CompiledProblem` (`compiled.codegen_env`) and surfaced in `compiled.inspect()` / the `CompiledReport`, so the env state that governed a compile is inspectable rather than hidden. `docs/sphinx/reference/environment-variables.md` documents every variable with its semantics, the explicit-argument-wins rule, and the `POPS_JIT_BACKDOOR` debug/unsafe warning. New `tests/python/unit/runtime/test_pops_env.py`.
- **Spec 5 sec.13.12 rich compiled-artifact manifest** (epic ADC-479, criterion 36): the new `pops.external.CompiledArtifactManifest` widens the thin brick-id / category `CompiledManifest` into the full self-description a runtime needs to bind a `.so` safely -- `model_name`, `abi_key`, `required_headers_sig`, `blocks`, `variables`, `roles`, `aux_required`, `params_const`, `params_runtime`, `ghost_depth`, `field_outputs`, the `supports_*` capability flags and `native_entrypoints`. A new `CompiledProblem.manifest()` accessor builds it INERTLY from the metadata the handle already carries (its `arguments()` bind table, the carried physical model, its `abi_key`); it binds, dlopens and runs nothing. Fields are populated from REAL metadata where genuinely available (blocks / variables / params split by kind / aux / ghost_depth / field_outputs / headers signature; `supports_uniform`/`amr`/`mpi`/`gpu` from the model's backend caps); capability flags the C++ codegen does NOT yet emit (`supports_stride`, `supports_partial_imex_mask`, `supports_named_fields`, `abi_version`, `native_entrypoints`) are honestly `None` / empty (UNKNOWN), never a fabricated `True`/`False`, and `needs_cpp_followup()` reports exactly which need a C++ codegen follow-up. The `check_layout_supported(manifest, layout)` validator raises the sec.13.12 error shape ("Compiled artifact cannot be used with layout=AMR(...): supports_amr=false") ONLY on a KNOWN-`False` flag; an UNKNOWN (`None`) flag is NOT a rejection (the no-false-positive discipline). The `external` layer stays import-graph-clean (the builder reads the passed handle's public surface; `pops.descriptors` is imported function-locally). New `tests/python/unit/runtime/test_manifest_richness.py`.
- **Spec 5 sec.13.11.3 scratch-plan inspection: `compiled.scratch_plan()` / `Program.scratch_plan()`** (epic ADC-479, criterion #38): a lowered `pops.time.Program` (and the `pops.compile_problem(...)` handle that carries one) can now be inspected for its step-body scratch budget WITHOUT binding, compiling or allocating. New `pops.codegen.scratch_plan.build_scratch_plan(program[, model])` runs the LIVENESS analysis the Program already exposes (`Program.scratch_liveness` / `buffer_reuse_report`, ADC-465) and returns an inert `ScratchPlan` reporting the per-category scratch-buffer counts (state scratch from `linear_combine` / local solves, rhs scratch from `rhs` / `source` / `apply` / `coupled_rate`, scalar-field scratch from `solve_linear` / `cell_compare`, the operator-declaration `linear_source` bucketed apart so it inflates no real category), the REUSED buffers (each scratch that lands on a recycled buffer, naming the earlier scratch(es) it shares it with -- PROVABLE: their SSA live ranges are disjoint), the REJECTED reuse (each scratch on a fresh buffer, with an inspectable reason -- a still-live occupant overlaps, or an `rhs` / `source` / `apply` reads the shared aux a field solve writes), and the PERSISTENT Krylov / multigrid solver buffers a `solve_linear` / `solve_fields` carries across a whole solve. The step-body reuse is EXACT (from the liveness ranges); the persistent solver counts are CONSERVATIVE rules of thumb (the exact Krylov vector count / V-cycle depth is a solver-dependent bind input) and the plan SAYS SO -- `conservative` is True iff any figure is over-counted, and the assumptions are listed in `notes`, never a fabricated reuse. `ScratchPlan` is printable and serialises via `to_dict()` / `to_json(path)`; `compiled.scratch_plan()` and `Program.scratch_plan()` are thin delegators to the codegen builder. Inert: it reads the Program IR and the model component counts only (no `_pops` / numpy at module scope, `pops.time` imported lazily, import graph acyclic). New `tests/python/unit/runtime/test_scratch_plan.py`.
- **Spec 5 sec.14.2.4 typed `P.rhs(terms=[...])` RHS composition** (epic ADC-479, criterion 27): `pops.time.Program.rhs` now ADDITIONALLY accepts a single typed `terms=` list -- a `pops.numerics.terms.Flux` plus the source terms to fold in (`P.rhs(U, fields=f, terms=[Flux(), electric])`) -- as a pure front door over the legacy `flux=`/`sources=` path. A `Flux()` in the list sets `flux=True` (its absence sets `flux=False`); every source term contributes its NAME to `sources=` in list order, accepting a `pops.numerics.terms.SourceTerm` / `LocalTerm` descriptor (its name must be set), an `pops.model.OperatorHandle` from `m.source_term`, or a plain source name string. The lowering is unchanged, so a `terms=` call builds the BYTE-IDENTICAL IR (same `_ir_hash`) as the equivalent `flux=`/`sources=` call and the legacy path is untouched. `terms=` is MUTUALLY EXCLUSIVE with `flux=`/`sources=`/`fluxes=` (passing both is a clear `ValueError` naming the conflict); a non-term object in the list (e.g. a bare bool -- `Flux()` is a term, not a bool) is a clear `TypeError`. `pops.numerics.terms` is imported function-locally so `pops.time` stays codegen-free and the import graph acyclic. New `tests/python/unit/runtime/test_rhs_terms.py`.
- **Spec 5 sec.8.12 / sec.8.4 AMR runtime inspection surface** (epic ADC-479, criterion #34): `AmrSystem.amr` returns a live, INERT `pops.runtime.amr.AmrRuntimeView` over the built hierarchy, with `patch_table()` / `hierarchy_snapshot()` / `explain_regrid()` / `explain_ghosts()` / `explain_reflux()` / `explain_checkpoint()` returning short, deterministic, array-free report value classes (`PatchReport` / `HierarchySnapshot` / `RegridReport` / `GhostReport` / `RefluxReport` / `CheckpointReport`, each printable + `to_dict()`). The view READS the live runtime box accessors (`patch_rectangles` / `patch_boxes` / `coarse_local_boxes` / `coarse_total_boxes`, guarding the pre-build state as "not built yet") and the retained config, and REUSES `pops.inspect_amr()` for the config envelope (levels / ratio / native limitations); it builds, allocates and steps NOTHING. A measure the native build cannot answer is DECLARED unavailable, never fabricated (per-level ghost depth, per-stage reflux timing, composite multi-level Poisson restart). `explain_checkpoint()` surfaces the sec.8.11 bit-identical v1 envelope (single block / single rank / frozen regrid) as an inert explanation, naming any constraint the live system violates. `compiled.inspect_amr(layout=None)` on `CompiledModel` / `CompiledProblem` delegates the STATIC report to top-level `pops.inspect_amr` (the artifact carries no embedded layout). `System.amr` raises a clear `AttributeError` pointing to `AmrSystem` / `pops.inspect_amr`. The new `pops.runtime.amr` package keeps the import graph acyclic (report classes import nothing; only the view reaches `_pops` through the bound `AmrSystem`). New `tests/python/integration/amr/test_amr_runtime_inspect.py` (host-runnable on Kokkos-Serial; a full regrid / MPI / GPU run is Kokkos/ROMEO-gated).
- **Spec 5 sec.14.2.3 typed operator handles** (ADC-464, epic ADC-479): the user-facing operator declarers `m.rate_operator` / `m.source_term` / `m.linear_source` now RETURN a lightweight, inert `pops.model.OperatorHandle` (carrying the operator `name` and `kind`) instead of `None`, and `P.call` accepts EITHER the string operator name OR that handle, coercing a handle to its `.name` through the IDENTICAL registry lookup + lowering, so `P.call(handle, ...)` builds the BYTE-IDENTICAL IR (same `_ir_hash`) as `P.call(name, ...)` and the legacy string path is unchanged; a non-str / non-handle selector is a clear `TypeError`. The handle holds no numerics, IR or Program reference, keeps `pops.time` codegen-free and the import graph acyclic, and is covered by `tests/python/unit/codegen/test_operator_handles.py`.
- **Spec 5 sec.12.1 compiled/sim inspection completeness** (epic ADC-479, criterion #15): a `pops.compile_problem(...)` handle and a configured `System` / `AmrSystem` can now be fully inspected WITHOUT binding, compiling or allocating, building on the ADC-509 introspection and the ADC-463 validation. New `compiled.inspect()` returns a printable `pops.codegen.CompiledReport` aggregating the carried metadata into the Spec 5 sec.12.1 `print(compiled)` shape (name, backend, platform, layout, committed blocks + state / components, elliptic / Krylov fields + solver-to-supply, program op count + commits, the REQUIRED runtime inputs from `arguments()`, the on-disk artifacts so_path / abi_key / cache_key, and the `compiled, waiting for pops.bind(...)` status), with `to_dict()` / `to_json(path)`. New `compiled.requirements()` returns a `RequirementsReport` of the COMPILE-TIME constraints DISTINCT from the runtime bind inputs -- the model capabilities the lowered flux relies on (`wave_speeds` / `hllc_star_state` / `roe_dissipation`, read from the carried model flags), the layout / backend / ABI constraints, and an honest `unknown` list for pieces genuinely not in today's metadata (the spatial scheme is a bind input; the reconstruction stencil width is unrecorded) rather than a fabricated value. New `compiled.inspect_capabilities()` delegates to the top-level `pops.inspect_capabilities()` machinery scoped to the descriptor categories a compiled Program selects at bind. New `compiled.dump_ir(path=None)` writes the serialized Program IR JSON (the SAME `_serialize` the IR hash digests), `compiled.dump_cpp(target)` writes the generated C++ source by REUSING the existing `Program.emit_cpp_program` codegen, and `compiled.dump_schedule(path=None)` writes the block commit order; each raises a clear error naming what is missing if the handle carries no Program (never faking a file). New `System.explain_bind(compiled)` / `AmrSystem.explain_bind(compiled)` return a printable `BindReport` computing provided-vs-still-required per group (instances / params / aux / solvers) by reusing the ADC-463 `collect_missing_arguments`. All inert host-side metadata reads (no `_pops`, acyclic import graph -- `pops.time` / runtime imported lazily) except `dump_cpp`, which reuses the existing emit. New `tests/python/unit/runtime/test_inspection_completeness.py`; the dump on a real `.so` stays Kokkos-gated.
- **Spec 5 sec.12.1 print-readability gate extended to the runtime/compiled headline objects** (ADC-505, epic ADC-479, criteria #40-41): the inert architecture gate `tests/python/architecture/test_print_readability.py` asserts the short / deterministic / array-free print contract for the inert authoring objects only -- it cannot build `pops.System` / `pops.AmrSystem` (constructing one allocates Fabs and initialises Kokkos) or `pops.codegen.CompiledProblem` (normally a Kokkos-gated `compile_problem`), and it runs source-only (before `_pops` is built) so it would only skip them. All three already define a short `__str__` (`System` / `AmrSystem` since ADC-505, `CompiledProblem` since ADC-509) but nothing asserted it. New `tests/python/integration/runtime/test_print_readability_runtime.py` runs in the `gate-python` step AFTER `_pops` is built: it constructs a real empty `System` / `AmrSystem` and a SYNTHETIC `CompiledProblem` (a real in-memory `Program` + a real `CompiledModel`, no compile -- mirroring `test_compiled_introspection`) and asserts each `str(obj)` is under 800 chars, deterministic, free of any raw `array(` / `ndarray` dump, and never the default `<...object at 0x...>` repr. The inert gate gains a docstring note pointing at this runtime counterpart. Test-only; no production code changed.
- **Spec 5 sec.10 strict early install validation + `AmrSystem.install` parity** (ADC-463, epic ADC-479): `System.install` now reads the compiled artifact's DECLARED bind inputs (`compiled.arguments()`, ADC-509) and rejects, BEFORE any native call, an install that fails to supply a REQUIRED argument the artifact declares (a committed instance, a `kind="runtime"` param, a required aux field, a required solver), with one clear actionable error naming exactly what is missing and the matching `install` keyword to supply it (e.g. `runtime param 'cs2'; pass install(params={'cs2': <value>})`); multiple misses aggregate into one message. The check is inert (it reads `arguments()` metadata and compares dicts -- no compile, bind or allocation) and conservative: it enforces only the `required` flag, so an optional input (a const param, the default-Poisson solver) is never demanded and a previously valid install passes through unchanged; a handle whose `arguments()` is unavailable or raises is skipped rather than rejected (a missing check never breaks a working install), and a NATIVE install (`compiled=None`) carries no declared arguments and is skipped. `AmrSystem` gains an `install(compiled=None, *, instances=, params=, aux=, solvers=, cadence=)` with signature parity to `System.install` that runs the SAME early validation, then wires the NATIVE AMR path (`add_equation` / `set_poisson` / `set_magnetic_field` / `set_aux_field` / `set_density`) and raises a clear `NotImplementedError` for the genuinely un-wired parts (a compiled time Program -- no AMR `install_program` seam; `params=` -- no `set_block_params`; `cadence=` -- no `set_program_cadence`). The shared pure router `collect_missing_arguments` and `validate_install_arguments` live in `pops.runtime._system_unified_install`. New `tests/python/integration/runtime/test_install_validation.py` (pure-Python; the Kokkos-gated end-to-end is covered by `test_unified_install.py` / `test_install_requirement_validation.py`).
...
- **Spec 3 IR dead-node elimination pass** (ADC-465, epic ADC-450): `pops.time.eliminate_dead_nodes(P)`
  ...
  `tests/python/unit/codegen/test_ir_passes.py` pins removal, the parity byte-identities, the side-effect and
...
- **Spec 3 remaining IR optimization passes** (ADC-465, epic ADC-450, section 28): the section-28
  ...
  is a ROMEO profile; the estimate is the host-side prediction. New `tests/python/unit/codegen/test_ir_opt_passes.py`
...
- **Spec 3 profiling wired into System** (ADC-459, epic ADC-450): `sim.enable_profiling()` /
  ...
  `System::Impl` and is not referenced by `SystemProgramDriver`, so the `MockImpl` is unaffected.
  ...
  `System::solve_fields`). Disabled by default (no hot-path cost). New `tests/python/unit/runtime/test_profiling.py`
...
- **Spec 3 per-node Program profiling** (ADC-459, epic ADC-450): the compiled time Program now times
  ...
  unchanged. New `tests/python/unit/runtime/test_pernode_profiling.py` pins the generated source.
- **Spec 3 custom solver DSL (IR-authoring slice)** (ADC-462, epic ADC-450): `@pops.lib.solver`
  ...
  rather than fake a Python solve. New `tests/python/unit/solvers/test_solver_dsl.py` (11);
...
- **Spec 3 coupled_rate operator + multi-output P.call** (ADC-457, epic ADC-450): a `coupled_rate`
  ...
  rather than faking it as independent single-block rates). New `tests/python/unit/runtime/test_coupled_rate.py`
...
- **Spec 3 unified-scheduler authoring** (ADC-458, epic ADC-450): `pops.time` schedules
  ...
  cache/checkpoint runtime is the C++ follow-up. New `tests/python/unit/runtime/test_schedule_authoring.py`
...
- **Spec 3 native Riemann capability validation** (ADC-456, epic ADC-450): board `m.riemann(...)`
  ...
  `star_state.*` hook-selector descriptors. New `tests/python/unit/physics/test_riemann_capabilities.py` and
...
- **Spec 3 inspection / debug API** (ADC-460, epic ADC-450, spec 3 section 33): show the lowering of
  ...
  `tests/python/unit/runtime/test_inspection.py`; examples `examples/spec3/board_vs_operator_ir_equivalence.py`
...
- **Blackboard-style physics DSL** (ADC-451/452/453/454, epic ADC-450, spec 3): a layer-1 user API
  ...
  the spec-1 `pops.dsl` and the `P` builder styles are unchanged. New `tests/python/unit/physics/test_physics_board.py`,
...
- **Install-time operator-requirement validation** (ADC-446, epic ADC-436, spec 2 criterion 24):
  ...
  `tests/python/integration/runtime/test_install_requirement_validation.py` (ROMEO).
- **Pure `pops.model.Module` compiles** (ADC-447, epic ADC-436, spec 2 "model-free"): a Module
  ...
  Module (no PDE method called on the model). New `tests/python/integration/mpi/test_module_compile.py`.
...
- **Operator-first Program type diagnostics** (ADC-448, epic ADC-436, spec 2 "operator-first"): with
  ...
  `tests/python/unit/codegen/test_operator_validation.py`.
...
- **GeneratedModule metadata in the compiled `.so`** (ADC-442, epic ADC-436, spec 2 "operator-first"):
  ...
  `tests/python/unit/codegen/test_module_codegen.py`.
...
- **Compiled operator introspection** (ADC-441, epic ADC-436, spec 2): `list_operators` /
  ...
  reads the carried model's metadata, no `.so` load). New `tests/python/unit/codegen/test_operator_introspection.py`.
- **ModuleSpec hash** (ADC-443, epic ADC-436, spec 2): `pops.model.Module.module_hash` folds the
  ...
  artifact. New `tests/python/unit/codegen/test_module_hash.py`.
- **Operator-first standard macros** (ADC-440, epic ADC-436, spec 2): `pops.time.std`.
  ...
  New `tests/python/unit/codegen/test_operator_macros.py`.
- **Public `pops.model.Module`** (ADC-439, epic ADC-436, spec 2 "operator-first"): the model-free
  ...
  reusable across any Module with matching signatures. New `tests/python/unit/codegen/test_operator_module.py`.
- **Typed `P.call` and `m.rate_operator`** (ADC-438, epic ADC-436, spec 2 "operator-first"):
  ...
  New `tests/python/unit/codegen/test_operator_call.py`.
- **Internal typed operator registry** (ADC-437, epic ADC-436, spec 2 "operator-first"):
  ...
  (operator-first Programs). New `tests/python/unit/codegen/test_operator_registry.py`.
- **Multi-block compiled time Programs** (ADC-426, epic ADC-399, spec "Multi-blocs"):
  ...
  `tests/python/unit/time/test_time_multiblock.py`.
- **`pops.time.std.condensed_schur` theta != 1** (ADC-427, epic ADC-399, completes ADC-421): the macro
  ...
  `tests/python/unit/time/test_time_condensed_schur.py` (theta == 1 and theta == 0.5 compiled-vs-offline parity,
...
- **Named multi-elliptic-field runtime** (ADC-428, epic ADC-399; completes ADC-419): a SECOND elliptic
  ...
  path. New `tests/python/unit/time/test_time_multielliptic.py`.
- **Implicit-flux BDF via matrix-free Newton-Krylov** (ADC-431, epic ADC-399): `pops.time.std.bdf`
  ...
  and still selected by naming a `linear_source`. New `tests/python/unit/time/test_time_bdf.py` (BDF1/BDF2
...
- **`pops.time.std` library completion + `@P.step` decorator** (ADC-423, epic ADC-399): pure-Python
  ...
  once at build time. New `tests/python/unit/time/test_time_std_decorator.py`, `test_time_std_rk.py`,
...
- **GMRES Krylov solver** (ADC-420, epic ADC-399): `pops.time.Program.solve_linear(method="gmres",
  ...
  converges to the same solution as BiCGStab. New `tests/python/unit/time/test_time_gmres.py` + C++ coverage in
...
- **Multi-component matrix-free linear solve** (ADC-416, condensed-Schur foundation):
  ...
  path is unchanged and bit-identical. New `tests/python/unit/time/test_time_multicomp_solve.py`.
- **Per-cell conditional select** (ADC-418, spec section 17): the `pops.time.Program` builder gains
  ...
  per-cell kernel pattern; no new `_pops` header). New `tests/python/unit/time/test_time_where.py`.
- **Named fluxes and named elliptic fields** (ADC-419, epic ADC-399, spec "Flux nommes optionnels" +
  ...
  `tests/python/unit/time/test_time_named_flux_elliptic.py`.
- **Program op-set completeness** (ADC-414, spec ops 10/16/21/22/23 + validation #18/#19): the
  ...
  `tests/python/unit/time/test_time_ops_polish.py`.
...
- **Divergence primitive + condensed-Schur partial** (ADC-412, acceptance 32): a centered
  ...
  `P.solve_linear` against the same offline CG reference (new `tests/python/unit/time/test_time_divergence.py` +
...
- **step_cfl routes through an installed compiled program** (ADC-413, epic ADC-399 criterion 7):
  ...
  `dt` through the SAME cadence helper as `step()` (the new `SystemProgramDriver::run_program_cadence`, factored
  ...
  does not apply. New test `tests/python/unit/time/test_time_step_cfl.py`: `step_cfl` advances the state via the
...
- **Substeps + stride in the compiled time program** (ADC-411): `pops.CompiledTime(substeps=, stride=)`
  ...
  `tests/python/unit/time/test_time_substeps_stride.py`: a compiled Forward-Euler program over an uncoupled
...
- **Compiled Strang macro reproduces native pops.Strang** (ADC-410): a new test
  `tests/python/unit/time/test_time_strang_parity.py` demonstrates that the compiled `pops.time.std.strang`
  combinator (the Program-IR Strang macro `H(dt/2); S(dt); H(dt/2)`) reproduces the native engine
  Strang macro-step (`SystemProgramDriver::step_strang`) BIT-EXACTLY on a simple case: an uncoupled isothermal
...
- **Multistep histories + Adams-Bashforth 2 in the time program** (ADC-406, Phase 7a): a compiled
  ...
  `tests/python/unit/time/test_time_history.py` (AB2 vs an offline recurrence to machine precision; an
...
- **Checkpoint/restart of compiled-Program histories + program-hash guard** (ADC-406, Phase 7b): a
  ...
  deep copy. Validated by `tests/python/integration/io/test_time_history_checkpoint.py` (continuous == restart to
...
- **Matrix-free operators + global `solve_linear` in the time program** (ADC-405, Phase 6b): a compiled
  ...
  C++-side, invisible to the IR. Validated by `tests/python/unit/time/test_time_solve_linear.py`: a compiled
...
- **Structured for-loops + if + norm_inf in the time-program codegen** (ADC-404, Phase 5b):
  ...
  `tests/python/unit/time/test_time_control_flow_b.py` (the unrolled-vs-loop codegen + a dt-free contraction run
...
- **Control flow + reductions in the time-program codegen** (ADC-404, Phase 5): `pops.time.Program`
  ...
  reference to machine precision (`tests/python/unit/time/test_time_control_flow.py`). `ctx.range` (static/dynamic
...
- **Named-source RHS in the time-program codegen + predictor-corrector** (ADC-403, Phase 4): a
  ...
  machine precision (`tests/python/unit/runtime/test_predictor_corrector.py`). Closes ADC-403. (Per-stage field
...
- **`pops.time.Program` split-source / local-linear-operator IR ops** (ADC-403, Phase 4): `P.source`
  ...
  `tests/python/unit/time/test_time_local_solve.py`.
- **Multi-stage compiled time programs (SSPRK2, SSPRK3, RK4)** (ADC-399 / ADC-407):
  ...
  reference to machine precision (`tests/python/unit/time/test_time_multistage.py` + the `examples/time_programs/`
...
- **`pops.compile_problem` + `sim.install_program` + `pops.CompiledTime`: run a compiled time Program
  ...
  `tests/python/integration/mpi/test_compile_problem.py` checks the rejections and the parity of a compiled
...
- **`pops.time.Program.emit_cpp_program`: lower the IR to a `problem.so` source** (ADC-401, Phase
  ...
  mis-lowering. `tests/python/unit/time/test_time_codegen.py` pins the generated ABI + algorithm and the refusals.
...
- **C++ `ProgramContext` runtime seam for compiled time programs** (ADC-401, Phase 2b): a new
  ...
  `Impl::program_step_` member), and `SystemProgramDriver::step` dispatches to the installed Program when
...
- **Generic moment Roe flux: `m.roe_from_jacobian()` + `build_moment_model(roe=True)`** (ADC-368):
  ...
  error for `n_vars != 4`. Covered by `tests/python/unit/codegen/test_dsl_roe_from_jacobian.py` (codegen + AOT
...
- **DSL surface for the real/complex spectrum predicate** (ADC-362): `dsl.eig_all_real(rows,
  ...
  (`tests/python/unit/runtime/test_projection_eig_predicate.py`).
- **Public `System.set_source_stage` on the Python facade** (ADC-308): the Schur-condensed source
  ...
  (`tests/python/unit/runtime/test_set_source_stage_facade.py`).
- **One-command Python build** (ADC-358): `scripts/build_python.sh` activates the `pops` env, sizes the
  production-module heavy-TU Ninja pool (`POPS_HEAVY_MODULE_TU_POOL`) from cores capped by RAM, exports the Kokkos/CMake
...
- **2D-core invariant published in `pops.capabilities()`** (ADC-294): `capabilities()` now exposes a
  ...
  `tests/python/unit/runtime/test_capabilities.py` pins the key. The ND core (`BoxND` / `GeometryND`) stays
...
- **Multi-rank test for the collective IO gather** (ADC-257): `tests/test_mpi_system_io_gather.cpp`
  ...
  mono-rank by `tests/python/integration/io/test_io_multirank.py`, which deferred the `np>1` case to a C++ file that
...
- **Spec 5 C1-System (#479): named elliptic fields lower on the uniform System** -- a `Case` with a named non-Poisson field validates (the `_POISSON_FIELD_NAMES` whitelist reject is removed) and `CompiledModel` carries `elliptic_field_names` (populated from `m._elliptic_fields`); at install, `_install_solver` routes a solver selection for a field a block's model DECLARES through the shared system elliptic solver (`set_poisson`) and rejects an undeclared field name (a typo) naming the declared set. The AMR named-field path stays deferred (ADC-428). New `tests/python/integration/runtime/test_unified_install_named_field_system.py` (epic ADC-479).
- **Spec 5 C3 (#479): multi-block Cases lower on a Uniform layout** -- `Case.validate` no longer rejects more than one block, and `pops.compile` resolves EACH block's physics and carries the `{block: model}` table on the handle (`_block_models`) so `pops.bind` installs each block with its own model; multi-block on an AMR layout stays a HONEST deferral (the whole-system AMR multi-block seam is not wired). New `tests/python/integration/runtime/test_case_multiblock_uniform.py` / `test_case_multiblock_amr.py` (epic ADC-479).

### Fixed
- **test-architecture follow-up: link the MPI AMR/System suites against their runtime object libs** -- the manifest-driven `tests/CMakeLists.txt` MPI loop selected the `pops_runtime_amr` / `pops_runtime_system` object lib with `$`-anchored regexes (`^(test_mpi_amr_|...)$`), which only matched the bare prefix and left `test_mpi_amr_twoblock_parity` / `test_mpi_amr_distributed_coarse` / `test_mpi_amr_compiled_parity` and the `test_mpi_system_*` suites without their runtime, so the MPI link failed with undefined `AmrSystem`/`System` symbols. Switched to prefix matches (`^test_mpi_amr_`, `^test_mpi_system_`, plus the exact `test_mpi_coupled_source` / `test_amr_regrid_mpi_parity`).
- **ADC-611 follow-up: emit the versioned schema from the compiled-library brick manifest** -- `pops.compile_library(..., emit=True)` lowers a C++ `pops_brick_manifest()` via `python/pops/codegen/library_codegen.py`; that emitter still wrote the pre-ADC-611 `{"library": name, "bricks": [...]}` shape, so a library `.so` loaded through `pops.lib.load_cpp_library` -> `pops.descriptors.parse_brick_manifest` was rejected by the strict parser (unknown top-level field `library`, missing `schema_version`). The emitter now writes `{"schema_version", "bricks": [...]}` (single-sourcing `BRICK_MANIFEST_SCHEMA_VERSION`); the informational library name is dropped from the strict brick manifest and stays exported via `pops_library_name()`. Fixes `test_compile_library.py::test_emit_exports_the_brick_manifest_for_load_cpp_library`.
- **ADC-620: give the compiled-AMR fine seed a coherent one-box dmap under `distribute_coarse`** -- `build_amr_compiled` (`include/pops/runtime/builders/compiled/amr_dsl_block.hpp`) built the single-box fine seed on the COARSE `DistributionMapping`; with `distribute_coarse=true` the coarse dmap has one entry per coarse box, so pairing it with a one-box BoxArray tripped the `MultiFab` layout check added by ADC-590/#416 (`box_array.size=1, dmap.size=4`) and aborted the `test_mpi_amr_distributed_coarse_np{1,2,4}` runs. The seed now carries its own one-box round-robin dmap (box 0 -> rank 0), mirroring `make_shared_amr_layout`; ownership, MPI comms and the numerical trajectory are unchanged (the previous `dm[0]` was already rank 0 in both replicated and distributed modes).

### Changed
- **ADC-528/529/530/531/532: the operator-first model+program core is handle-only end to end** -- the `pops.lib.time` scheme macros (forward_euler, ssprk2/3, rk4, rk, explicit_rk, imex_local[_linear], adams_bashforth, bdf, strang, lie, predictor_corrector_local_linear, condensed_schur) now take typed `pops.model.OperatorHandle` / `LocalLinearOperatorHandle` selectors instead of operator name strings, and `P.linear_source` / `P.apply` accept a handle (unwrapped to `.name`, byte-identical IR); the private `_call("...")` / `_rhs_legacy` free-string seams stay internal only, enforced by the new source-only fence `tests/python/architecture/test_lib_time_no_string_selectors.py`. A deprecated string operator kwarg raises a clear `TypeError` pointing at the handle form (no silent acceptance). The physics facade `Equation` now refuses `bool()` / `if equation:` (`pops._ir.expr.Equation.__bool__`), a runtime IR `Value` refuses `len(value)`, and IR nodes expose a derived logical shape plus an optional source location in the inspection view -- all EXCLUDED from `_serialize` / `_ir_hash`, so production routes stay BIT-IDENTICAL (proven by `_ir_hash` parity tests: every macro's handle-built Program hashes equal to the equivalent legacy/manual Program). `pops.model` documents the operator requirements vocabulary (`OPERATOR_REQUIREMENT_KEYS`); `Program.keep_history(checkpoint_policy=)` carries the policy as metadata. New `tests/python/unit/time/test_lib_time_ir_parity.py`, `tests/python/unit/runtime/test_facade_lowering_parity.py`, `tests/python/unit/runtime/test_equation_not_bool.py`, `tests/python/unit/runtime/test_program_ir_nodes.py`, `tests/python/unit/codegen/test_operator_signature_errors.py`, and architecture fences `test_operator_core_no_name_caps.py` / `test_lib_time_no_string_selectors.py`.
- **ADC-556: `m.solve_field(equation=, outputs=, solver=)` is the single field-solve facade with a typed handle** -- the board `Model.solve_field` now both wires the Poisson coupling (`elliptic_rhs`) AND records the inspectable `pops.fields.PoissonProblem` descriptor (unifying it with `Model.field_problem`, so one entry point runs AND inspects), and it returns a typed `pops.physics.FieldsHandle` that IS a `pops.model.OperatorHandle` of kind `"field_operator"` (resolving through the one public `P.call` path). Its `.outputs` are a structured `pops.physics.FieldOutputs` object reachable BOTH as an attribute (`fields.outputs.E`) and by item (`fields.outputs["E"]`), still iterable like the underlying mapping (backward compatible with dict readers); an unknown output raises a structured error naming the known handles instead of a silent miss. `FieldsHandle.__call__(state)` lowers to that Program's per-stage `solve_fields` (returning the ADC-588 FieldContext-tagged value), and a bare call without a Program value is refused; `__call__` is defined only on `FieldsHandle`, never on the base `OperatorHandle`. The `field_operator` operator KIND (the compile-time contract) is unchanged and there is no free-string `.output(str)` lookup or `field_operator(...)` authoring verb. New `tests/python/unit/physics/test_solve_field_facade.py` and source-only `tests/python/architecture/test_no_field_operator_surface.py`.
- **#434: manifest-driven GoogleTest test architecture** -- the whole test suite is reorganized under `tests/cpp/{unit,integration}/<domain>/` (native GoogleTest 1.17, one executable per suite via `pops_add_gtest_suite`, ~150 files ported from the wrapped legacy runner to native `TEST`/`EXPECT_*`, 366 serial ctest cases vs 175 before) and `tests/python/{architecture,unit,integration}/<domain>/` (AST-driven per-file process isolation in `tests/python/conftest.py`, `POPS_SKIP` sentinel so a gated script reports SKIPPED instead of silently passing), all registered in the descriptive `tests/test_manifest.toml` that the CI impacted-test selector reads; MPI suites register per-rank variants (np 1/2/4) and a heavy file can raise its process timeout via a module-level `POPS_PROCESS_TIMEOUT`.
- **ADC-610: group the large append-only AMR build structs into named option groups and surface the fixed capacities** -- the 31-field `AmrBuildParams` and the 22-closure `AmrCompiledHooks` (`include/pops/runtime/amr_system.hpp`) are regrouped into named sub-structs (`mesh` / `physics` / `regrid` / `poisson` / `initial` / `schur` / `named_aux` for the params; `base` / `stability` / `checkpoint` / `mpi_gather` tiers plus the lifetime holder for the hooks), so ownership is explicit at the call site (`bp.mesh.n`, `bp.schur.theta`, `h.checkpoint.level_state`) and a new setting goes INTO its semantic group instead of onto an append-only tail. The append-only-tail ABI idiom is documented as retired: the struct crosses the dlopen `.so` boundary by value, and any layout change shifts `POPS_HEADER_SIG` (a sha256 over `include/`), which re-keys `pops_native_abi_key`, so a stale `.so` is REJECTED at `add_native_block` (never silently truncated). The hidden `kMaxRuntimeParams` capacity is now surfaced: mirrored to Python as `_pops.__max_runtime_params__` (single C++ source, read hasattr-gated), the codegen overflow error names the limit / count / offending params (with the correct `include/pops/runtime/config/runtime_params.hpp` path), the `install_program` / `add_compiled_block` host paths REJECT a `.so` declaring more params than the fixed carrier can hold (previously a silent clamp), and the capacity utilization `{count, limit, status}` appears in the `ModuleManifest`, the compiled report and the program report; the coupled-source `kCsMax*` bounds are surfaced by `CompiledCoupledSource.utilization()`. `SystemBlockStore::BlockState` is deliberately NOT regrouped (a named-group declaration comment records the decision: the stepper templates + `test_strang_splitting.cpp` MockImpl mirror its contract, so regrouping would churn the stepper/loaders for zero functional gain). New `tests/python/unit/runtime/test_capacity_limits.py`.
- **ADC-611: version and harden the external-brick manifest schema** -- the external-brick manifest (the JSON `pops_brick_manifest()` exports) is now STRICT and versioned: `BrickRegistry::to_json` (`include/pops/runtime/program/external_brick.hpp`) emits a top-level `schema_version` (1) and the `.so`'s own `abi_key`, and the single host parser `pops.descriptors.parse_brick_manifest` (used by `_register_manifest` and `pops.external.read_manifest`) REFUSES a manifest missing `schema_version` (legacy -> "regenerate the brick library"), a wrong version, a missing required entry field (`id` / `category` / `requirements` / `capabilities`), or an UNKNOWN field (top-level or per entry), each error naming the offending field -- emitter and parser stay in lockstep on the shared version. The rich `CompiledArtifactManifest` gains `SCHEMA_VERSION` in `to_dict()` and a strict `from_dict()` round-trip parser (version check, unknown-field refusal); registered external bricks now appear in `pops.inspect_capabilities()` as `source="external"` rows. New `tests/python/unit/runtime/test_manifest_schema.py`; updated `tests/python/unit/runtime/test_external_bricks.py` fixtures to the versioned schema.
- **ADC-621: accelerate the PR gate -- import-closure test selection + a header-build cache guard** -- the `gate-python` selector now picks impacted tests from the ACTUAL static import graph instead of a name-token area heuristic: a new stdlib-only `scripts/ci_import_closure.py` AST-walks `python/pops/**` (every import at any depth, so the pervasive function-scope lazy imports and relative imports are captured, plus the ancestor packages whose `__init__` runs on import), reverse-closes a changed pops module to the tests that transitively import it, and closes the bare-sibling cross-test edges both ways (a shared helper drags its dependents in). `scripts/ci_select_tests.py` `plan_python` wires it in with every conservative rule intact (broad/bindings/CMake -> all; a new or unparseable pops file fails safe to all; the >75% and unknown-path nets stay) and now emits the firing rule (`python_why`) to the job summary. On the build side, `gate-python-build` compiles the module with `POPS_HEAVY_MODULE_TU_POOL=2` (the two ~4 GB `-O3` heavy TUs fit the 16 GB Serial runner, halving the header-PR critical path that pool=1 serialized to ~37 min) and gains an eviction guard: a `pops-module-*` cache keyed on every `.so`-affecting input (`include/**`, `python/bindings/**`, the CMake/preset files, the pinned Kokkos version+commit, and the gcc major) reuses the compiled `_pops.so` on a pure-Python PR (exact hit + `cpp` unchanged), re-syncing the checkout's Python sources over it and skipping the cmake build; a near-miss or any C++ change rebuilds (no restore-keys, so a stale `.so` is never served), and push/nightly stay a full build. Routing and the `gate` aggregation are unchanged. New source-only `tests/python/architecture/test_ci_impacted_selection.py`. CI-only; no change to the shipped library or test coverage (the full suite still runs on master push + nightly).
- **ADC-593: generate per-route native seam TUs from one combination manifest** -- the 19 `(transport, flux)` leaves are generated at configure time from `src/runtime/builders/seam_combinations.cmake` and its six `templates/*.cpp.in` shapes into `${build}/src/generated_seams/`. `src/CMakeLists.txt` owns the single generated product and the `pops_runtime_{system,amr}` OBJECT targets consumed by both `_pops` and native tests; Python/tests never regenerate or relist it. One manifest row remains the only growth point, while the two unique AMR riemann dispatchers stay hand-written under `src/runtime/builders/`. The permanent source gate is `tests/python/architecture/test_runtime_builder_manifest.py`; runtime coverage remains in `tests/python/unit/runtime/test_seam_combinations.py`.
- **ADC-594: isolate the compiled-Program runtime state in a shared `ProgramRuntimeState`** -- the ~10 compiled time-Program members (installed step, dt bound, cadence, program hash, name-based block map, runtime params, recorded diagnostics, multistep history rings, scheduler cache, profiler) and the `HistoryManager` are extracted out of the `System::Impl` / `AmrSystem::Impl` god-object into one inspectable `pops::runtime::program::ProgramRuntimeState` (`include/pops/runtime/program/program_runtime_state.hpp`), embedded verbatim by BOTH runtimes as a single shared subsystem (the AMR runtime uses the common subset; `dt_bound_` / `cache_` / `hist_` stay empty there, the documented Uniform/AMR divergence). The self-contained cadence / diagnostics / runtime-param / history-ring logic moves onto the struct with Program-subsystem-worded errors; `SystemProgramDriver` reads only `program_.step_/substeps_/stride_/dt_bound_`, and the `tests/cpp/unit/numerics/test_strang_splitting.cpp` `MockImpl` now embeds the real struct (no mocked members). A new Python `ProgramRuntimeReport` (`pops.runtime.program_report`, surfaced by `System.program_report()` / `AmrSystem.program_report()` and on the bound-simulation view) aggregates the bound accessors into one structured, JSON-ready report, and the `sim.inspect()` program section is now built from it (single source). New const getters `System::program_substeps()` / `program_stride()` (and AMR counterparts). Behavior-preserving refactor (runtime bit-identical, no public API change); adding the header changes `ADC_HEADER_SIG` (a full CI rebuild + AOT cache invalidation, by design). New `tests/python/unit/runtime/test_program_report.py`.
- **ADC-583: `pops.bind` now returns a bound-simulation view over internal runtime adapters (`System`/`AmrSystem` become internal backends)** -- a new internal `pops.runtime._bind_adapters` layer (`adapter_for` selecting `_UniformRuntimeAdapter` for `layout=Uniform` and `_AmrRuntimeAdapter` for `layout=AMR`, sharing the bind logic) builds the engine from validated/lowered objects and installs through the internal `_install_compiled` seam, and `pops.bind` wraps the engine in a `pops.runtime._bound_sim.BoundSimulation` VIEW instead of returning the raw engine. The view delegates only the run/data/diagnostic/io surface and hides the assembly vocabulary (`add_block`/`add_equation`/`set_poisson`/`set_refinement`/`install_program`/...) behind a clear AttributeError that no longer recommends the legacy setters; the engine stays reachable for low-level tests as `sim._engine`. The AMR layout-lowering helpers moved from `pops.codegen.orchestration` to the adapter module, and the runtime error remedies that used to recommend `System`/`AmrSystem`/`set_poisson`/`install_program`/`set_refinement` now point at the `pops.Case` + `pops.compile`/`pops.bind` flow. New `tests/python/integration/bindings/test_bind_adapters.py`.
- **ADC-590: remove the implicit Euler fallback from the generic HLLC/Roe routes; add the explicit `euler_hllc` / `euler_roe` routes** -- the native Euler brick (`include/pops/physics/fluids/euler.hpp`) now PROVIDES the HLLC/Roe capabilities (`contact_speed`, `hllc_star_state`, `roe_dissipation`) with the canonical-Euler formulas copied verbatim from the former fallback, so it satisfies `HasHLLCStructure`/`HasRoeDissipation` and takes the GENERIC `HLLCFlux`/`RoeFlux` path BIT-IDENTICALLY (proved by `tests/cpp/integration/native_loader/test_amr_riemann_native.cpp`, `tests/cpp/integration/amr/test_amr_spatial_parity.cpp`, `tests/cpp/unit/codegen/test_block_builder.cpp`). `HLLCFlux`/`RoeFlux` are now generic-only (a `static_assert` fires without the capability, no hidden n_vars==4 branch); the moved canonical 2D Euler arithmetic lives in two explicit structs `pops::EulerHLLCFlux2D` / `pops::EulerRoeFlux2D` reached by the new `riemann='euler_hllc'` / `'euler_roe'` routes (descriptors `EulerHLLC2D()` / `EulerRoe2D()`; +2 rows in `dispatch_tags.hpp` `kRiemanns`, `route_ids.hpp` `kRiemannRoutes` and `python/pops/runtime/routes.py`, so the riemann family signature goes `riemann:4` -> `riemann:6` and the artifact cache re-keys per ADC-599). BEHAVIOR CHANGE: a DSL model that only declared a primitive `'p'` (no `m.enable_hllc()`/`m.enable_roe()`) is no longer silently accepted on `riemann='hllc'`/`'roe'`; the install guard now names the missing capability and offers both remedies (call `m.enable_hllc()`/`m.enable_roe()` for a generic model, or select `EulerHLLC2D()`/`EulerRoe2D()` for a canonical 4-variable Euler transport). The new explicit routes refuse a non-Euler layout or a model that emitted the generic capability (no ambiguity). New `tests/python/unit/physics/test_euler_riemann_routes.py`; updated `tests/python/unit/codegen/test_cache_key_routes.py` (riemann count 4->6), `tests/python/unit/runtime/test_route_ids.py`, `tests/cpp/integration/runtime/test_route_ids.cpp`, `tests/cpp/unit/codegen/test_block_builder.cpp`.
- **Spec 5 sec.12 cleanup: single-source the doctor capability lists + runtime-param docs freshness + `run(cfl=)` test** (epic ADC-479): `pops.capabilities()` (the `pops.doctor` matrix) now DERIVES its riemann / limiter / reconstruction / Poisson token lists from the descriptor catalogs via a new `pops.runtime.doctor._descriptor_tokens` (the same single source `pops._capabilities.inspect_capabilities` reads) instead of hardcoding them, so adding or retiring a brick can no longer silently desync the two matrices (output shape and the `test_capabilities` ordered contract are unchanged). `docs/DSL_MODEL_DESIGN.md` is refreshed to state that `m.param(kind="runtime")` is SHIPPED -- the former "later phase" / `NotImplementedError` gap lines now describe the two wired paths (the AOT block `set_block_params` and, since ADC-510, a compiled time Program lowering `ctx.program_params(block).get(index)` set via `pops.bind(params=...)`, seeded from the `.so` `pops_program_param_*` metadata). New `tests/python/integration/runtime/test_run_cfl_default.py` host-tests the #379 C7 `System.run(cfl=None)` defaulting to the pinned program-cadence CFL (and an explicit `cfl=` overriding it), the test the CHANGELOG previously claimed.
- **Spec 6 hygiene (ADC-518): clarify the facade compile engine is internal** (epic ADC-518): `pops.physics.facade.Model.compile` (the kept internal codegen engine) now documents in its docstring that the PUBLIC compile front door is `pops.compile(case, backend=pops.codegen.Production())` and that its own `backend` argument is the internal lowering vocabulary (a typed `Production()`/`AOT()`/`JIT()` descriptor lowered via `lower_backend`, or the legacy token, kept for the codegen path / internal callers / tests). No behavior change; the audited `set_poisson` / `set_disc_domain` / `lower_backend` seams were confirmed internal and left as-is (typed object on the public surface, string token internal).
- **Spec 6 hygiene (ADC-518): separate historical docs from the active docs** -- 15 closed audits, completed roadmaps and delivered design notes (`CODEBASE_AUDIT.md`, `STYLE_CONFORMANCE_AUDIT.md`, `COMMENTS_AUDIT.md`, `CONFORMANCE_AUDIT.md`, `STL_BOOST_AUDIT.md`, `DEAD_CODE_AUDIT.md`, `DOC_REFONTE_AUDIT.md`, `TOOLCHAIN_ROBUSTESSE_AUDIT_2026-06-10.md`, `PAPER_ROADMAP.md`, `FULL_MODEL_VALIDATION_ROADMAP.md`, `BUILD_UX_ROADMAP_2026-06-10.md`, `PERF_SCALING_TODO.md`, `RESEARCH_BACKLOG.md`, `SAMRAI_BACKEND_PLAN.md`, `AMR_CONDENSED_SCHUR_DESIGN.md`) move from `docs/` to `docs/archive/` (history preserved via `git mv`); the active `AUDIT.md` index, `CODING_STANDARDS_DECISIONS.md`, `DOC_QUALITY.md`, the archive `README.md` and one Sphinx link are repointed. Design notes still cited by live code comments (`SCHUR_CONDENSATION_DESIGN.md`, `AMR_MULTIBLOCK_DESIGN.md`, `GPU_RUNTIME_PORT.md`, `PERFORMANCE.md`, the HOFFART notes) stay at the `docs/` root with their historical banner. Stale prose corrected: the deprecated `pops.Implicit` alias and the now-shipped `kind="runtime"` params (ADC-510) in `SCHUR_CONDENSATION_DESIGN.md` / `DSL_MODEL_DESIGN.md`.
- **Spec 5 docs (#3/#7): demote the native System runtime methods from the public front door** (epic ADC-479): the main user docs (README + `docs/sphinx/**` getting-started / simulation / models / how-to / amr / advanced / reference pages) now lead with the typed public flow -- `pops.Case(layout=Uniform(mesh)).block(...).field(PoissonProblem(...)).time(...)` -> `pops.compile(case, backend=Production())` -> `pops.bind(compiled, state=, params=, aux=, solvers=)` -> `sim.run(...)` -- and reframe `System` / `AmrSystem` `add_block` / `add_equation` / `set_poisson` / `step_cfl` as the low-level runtime seam `pops.bind` builds on (kept for the native/AMR runtime and the tests, not the recommended path). The elliptic surface is the typed `pops.fields.PoissonProblem(unknown=, equation=(-laplacian(phi) == ChargeDensity.from_blocks()), solver=GeometricMG(), bcs=[Dirichlet()])`. A one-line internal-note is added to each method's docstring; the method behavior is unchanged.
- **Spec 5 de-string the remaining public string surfaces** (epic ADC-479): the public surfaces that routed an algorithm by a bare string now require a TYPED descriptor and reject the string with a clear error naming the typed alternative (the lowering to the internal token is byte-identical -- the IR / manifest output is unchanged). `Program.solve_linear(method=, preconditioner=)` takes `pops.solvers.krylov.CG()`/`GMRES()`/`BiCGStab()`/`Richardson()` and `pops.solvers.preconditioners.Identity()` (a string raises `TypeError`; `None` defaults to `CG()`/`Identity()`); `pops.compile_library(backend=)` takes `pops.codegen.Production()`/`AOT()`/`JIT()` (a string raises `TypeError`, mirroring `pops.compile`); the `pops.lib.time.condensed_schur(method=)` macro takes a typed Krylov descriptor (defaults to `BiCGStab()`); and the public param-creation surfaces `pops.physics.Model.param` / board `Model.param` / `pops.Case.param` take a typed `pops.physics.ConstParam(name, value)` / `RuntimeParam(name, value)` (the new `ConstParam` sugar) or the `(name, value)` const shorthand, rejecting a bare `kind="const"/"runtime"` keyword. New `tests/python/unit/runtime/test_spec5_destring_public.py`.
- **Spec 5 final API alignment: public-surface cleanup** (epic ADC-479): the top-level compilable assembly `pops.Problem` is renamed to `pops.Case` (no deprecated alias -- `pops.Problem` now raises `AttributeError`; the assembly's `category` is `"case"` and the route matrix `to_dict()` key is `"case"`); `pops.PythonFlux` is removed from the public surface (it computes a numpy residual in Python, which the PoPS "no public Python numeric" rule excludes) and is reachable only as `pops.experimental.PythonFlux` (a new NON-PRODUCTION / TESTS-ONLY package) for residual prototyping in tests; and the custom-solver `@solver` generation DSL stays internal-only under `pops.codegen.solvers` (no public `pops.solver` / `pops.lib.solver` decorator). `FieldProblem` / `PoissonProblem` / `LinearProblem` (a different concept) are untouched. New `tests/python/architecture/test_spec5_public_api.py`.
- **Spec 5 docs: teach the typed forms in the freshness-flagged reference pages** (epic ADC-479): after the typed-API merge train (PRs #358-#369) churned `python/pops/*`, the flagged docs are re-acked against current master and the straggler string forms are migrated to lead with the typed objects -- `pops.FluidState.compressible(gamma=1.4)` / `.isothermal(cs2=0.5)` in the model / native-brick tables, `sim.set_poisson(bc=pops.Dirichlet())` in the native Poisson examples (incl. `add_elliptic_model`), and the typed Riemann names (`Rusanov()` / `HLLC()` / `Roe()`) in the DSL / moment prose. The legacy string forms keep working (additive). Every flagged page is re-acked in `docs/docmap.toml`, so `docs/check_docs.py` exits 0 again.
...
- **Spec 4 s7: drop the `std` time-scheme bundle** (ADC-467): `pops.lib.time.std` is removed; the ready schemes are called by their explicit names (`pops.lib.time.ssprk3`, `pops.lib.time.strang`, ...). Section 7 bans the `std` name outright (not just the `std/` directory), enforced by `tests/python/architecture/test_no_forbidden_paths.py::test_no_std_name_anywhere`. The internal Spec-3 macro catalog (`pops.lib.time.macros`) now forwards to those names directly.
...
- **Spec 4 consumer migration off the flat shims** (ADC-473, epic ADC-467): every
  ...
  symbolic-IR builders move to `pops._ir` (`pops._ir.expr.Var`/`Const`,
  `pops._ir.ops.sqrt`/`eig_all_real`, `pops._ir.lowering.diff`), the PDE facade to
...
- **Spec 4 clean break** (ADC-473, epic ADC-467): removed the flat `pops.dsl` and
  `pops.moments` modules; consumers now import from `pops.physics` / `pops._ir` /
...
- **Generalize runtime/system header comments** (ADC-370): follow-up to ADC-333 on the headers that
  ...
  `docs/validation/HEADER_PROVENANCE.md`. `system_program_driver.hpp` is already generic (its
...
- **Factor the stepper global-dt-bound and grid-step duplication** (ADC-213): `step_cfl` and
  ...
  polar/Cartesian minimum-grid-step expression. Both are now private `SystemProgramDriver` helpers
...
- **Factor the stepper due-block advance loop** (ADC-213): `step` (Lie) and `step_cfl` shared,
  ...
  opt-in Schur source stage). It is now a private `SystemProgramDriver::advance_due_blocks(dt)` helper. The
...
- **Factor the multi-box global-gather idiom** (ADC-264): the five copy-pasted collective gather sites
  ...
  `tests/python/unit/physics/test_polar_theta_boxes.py` (theta_boxes=2/4).
...
- **Pin the conda build toolchain and surface the heavy-TU pool** (ADC-338): `environment.yml` pins
  ...
  -- and now prints how to widen `POPS_HEAVY_MODULE_TU_POOL` on a high-RAM host so `-j` parallelizes the
...
- **Parallelize the `_pops` build by splitting the heavy runtime TUs** (ADC-335): `python/system.cpp`
  ...
  (`amr_{block,compiled}_{exb,isothermal,compressible}`). The `POPS_HEAVY_MODULE_TU_POOL` CMake cache
  variable (default 1, anti-OOM) lets the production Ninja heavy-TU pool widen so `-j` actually
  compiles the now-smaller sub-TUs in parallel; `POPS_HEAVY_TEST_TU_POOL` controls the independent
  -O0 runtime/test pool. Byte-identical: the inner make_block / dispatch_amr ladders move unchanged, so the
...
- **Spec 6 (ADC-518): the deprecated `pops.Implicit` time-policy shim is removed** -- one clean Spec-5/6 surface, no retrocompat: `pops.Implicit` now raises `AttributeError` and is gone from `pops.__all__` and `pops.runtime.bricks`. The real treatments stay (`pops.Explicit` / `pops.IMEX` / `pops.SourceImplicit` / `pops.SourceImplicitBE` / `pops.IMEXRK`); use `pops.SourceImplicit(...)` or `pops.IMEX(...)` for the implicit backward-Euler source. The top-level `pops` docstring now opens on the typed `pops.Case` -> `pops.compile(backend=Production())` -> `pops.bind` -> `sim.run` flow (not the old `System.add_block` / `set_poisson` / `step_cfl` example), and the stale `Case` output / multi-block "deferred" docstrings are corrected (ADC-509 wired). `tests/python/unit/time/test_time_policy.py` now asserts the shim is gone.
- **Spec 5 (#1/#5): the `pops.lib.solvers` public shim is removed; one public solver home, no public `target=`** -- the solver / preconditioner descriptors now live ONLY in `pops.solvers` (`pops.lib.solvers.*` raises `ModuleNotFoundError`; `pops.lib` is presets-only), the `pops.codegen.solvers` solver-generation DSL is documented INTERNAL / EXPERIMENTAL (no public `@solver` decorator on `pops` / `pops.lib` / `pops.solvers`; users configure the provided descriptors), and the public lowering surface (`pops.compile` / `pops.bind` / `pops.Case`) takes a `layout`, never a `target=` kwarg (the remaining `target=` token is internal codegen). New guards in `tests/python/architecture/test_spec5_public_api.py` (epic ADC-479).
...
- **Uniform `pops.bind` ignored the Case mesh and built the engine on the default grid** (ADC-583) -- `pops.compile` set `compiled._layout = None` on the `Uniform` route, so the Uniform runtime adapter (`pops.runtime._bind_adapters._UniformRuntimeAdapter.build_engine`) constructed a bare `System()` on the `SystemConfig` defaults (`n=64`, `L=1.0`, periodic) and silently dropped the layout's mesh. A `pops.Case(layout=Uniform(CartesianMesh(n=16, ...)))` therefore compiled and bound onto a 64x64 engine, and a 16x16 initial state failed at install (`System::set_state : taille != ncomp*n*n`). `compile` now carries `compiled._layout = problem.layout` on the Uniform route too, and `build_engine(layout)` derives the `SystemConfig` (`n` / `L` / `periodic`) from `layout.mesh` (new `_system_config_from_layout`, mirroring `_amr_config_from_layout` for AMR), so the bound engine matches the Case's grid; a handle with no layout (not produced by `pops.compile`) still binds on the bare `System()` defaults. New `test_bind_system_config_from_uniform_layout` in `tests/python/unit/runtime/test_problem_orchestration.py`; the gated `test_full_bind_flow_uniform_gated` now uses `n=16` to lock the fix.
- **File-size architecture gate restored by splitting the oversized modules** (ADC-619) -- `tests/python/architecture/test_file_sizes.py` regressed after #415/#416 pushed six modules over the Spec 4 500-line cap (`__init__.py` over the 120-line facade cap). Behavior- and surface-neutral decompositions along the established sibling/mixin pattern bring each under budget: `_capabilities` splits into `_capabilities_common` / `_capabilities_report` / `_capabilities_inspect`, `descriptors` sheds the typed-protocol family into `_descriptor_protocol`, `codegen/loader` and `codegen/inspect_report` move `CompiledModel` / `CompiledReport` into private siblings, and `AmrSystem` extracts the `pops.bind` install seam into `_amr_system_install`. Every original import path keeps working (re-exported from its home module).
- **Spec 5 C7 (#479): a numeric compiled-program CFL no longer silently ignored** -- `pops.CompiledTime(cfl=X)` for a NUMERIC `X` is now accepted (only a self-computed `cfl="program"` sub-program is still deferred), and the install seam drops the stale numeric-cfl `NotImplementedError`. The previous wiring stored the cadence but never applied the cfl (a silent no-op the reviewer flagged): the cadence now PINS its numeric cfl on the bound `System`, and `System.run(cfl=)` defaults to it when the caller passes none, so a bare `sim.run(t_end)` after `bind(..., cadence=CompiledTime(cfl=0.5))` advances at `cfl=0.5` (an explicit `run(cfl=)` still overrides). New default-resolution test in `tests/python/integration/runtime/test_unified_install.py` (epic ADC-479).
- **Typed compile backend rejected by the model facades** (epic ADC-479, Spec 5 sec.8.15): the typed backend descriptors `pops.codegen.Production()` / `AOT()` / `JIT()` already lowered inside the compile drivers (`compile_model` / `compile_problem`), but the higher-level authoring facades validated or stored `backend` themselves before reaching a driver, so `m.compile(backend=pops.codegen.Production())` raised `unknown backend Production(...)` (`HybridModel.compile` likewise, and `CoupledSource.compile` stored the descriptor object instead of its string). `pops.physics.facade.Model.compile`, `pops.physics.hybrid.HybridModel.compile` and `pops.physics.multispecies.CoupledSource.compile` now run the same additive `lower_backend` coercion first, so a typed descriptor is accepted wherever a string is (a plain string / `None` still passes through unchanged; the thin `HyperbolicModel.compile` delegator already coerced via `compile_model`). The Spec-5 model docs (`docs/sphinx/models/index.md`, `docs/DSL_API.md`, `docs/sphinx/tutorials/moment-model-hyqmom15.md`) now teach the typed form uniformly. New facade-coercion tests in `tests/python/unit/runtime/test_backend_platform.py`.
- **Docmap depends_on paths stale after the package restructure** (epic ADC-479, Spec 5): the Spec-4/5 split turned several flat `python/pops/*.py` modules into packages, so `docs/docmap.toml` entries still pointing at `python/pops/dsl.py` / `time.py` / `physics.py` / `lib.py` / `moments.py` / `library.py` / `library_codegen.py` made `docs/check_docs.py` fail with 26 "depends_on introuvable" violations (the doc looked perpetually suspect). Each `depends_on` now points at the current package directory (`python/pops/_ir` for the symbolic DSL, `python/pops/time` / `physics` / `lib` / `moments`) or the moved file (`python/pops/codegen/library.py` / `library_codegen.py`), preserving the freshness contract; the strict `getting-started/tutorial.md` entry is re-acked. The stale `../python/pops/dsl.py` link in `docs/TOOLCHAIN_ROBUSTESSE_AUDIT_2026-06-10.md` now points at `codegen/toolchain.py`, where `_default_cxx()` lives. No doc prose changed.
...
- **Symbolic IR imports without numpy** (epic ADC-467, Spec 4): `pops._ir.expr` / `pops._ir.values` imported numpy at module scope, but the build-time codegen tool `scripts/gen_solver_kernel.py` runs under the C++ job's bare interpreter (no numpy) and imports `pops.lib` -> `pops._ir` to emit the custom-solver kernel header. The `ir` restructure thus reddened the `gate-cpp` / MPI / OpenMP build at the gen-solver step (`ModuleNotFoundError: No module named 'numpy'`); the python PRs route to `gate-python` and skip the C++ build, so it only surfaced on the master push. numpy is now imported lazily inside the host `.eval()` interpreter (the only place it is used; IR construction and C++ emission never touch it), so the symbolic IR imports in a bare interpreter. A new `tests/python/architecture/test_codegen_without_numpy.py` re-runs the real codegen tool with numpy forced absent and runs in `gate-python`, so the regression is caught before the master push.
...
- **Source-only RHS no longer leaks the flux** (ADC-430, epic ADC-399, sibling of ADC-425):
  ...
  fluxes is rejected. New `tests/python/unit/time/test_time_rhs_flux_false.py`.
- **Flux-only RHS no longer leaks the default source** (ADC-425, epic ADC-399, spec criterion 17):
  ...
  double-count). New `tests/python/unit/time/test_time_rhs_sources.py`.
...
- **Quasi-vacuum velocity bound for the isothermal model** (ADC-77, third stability barrier of the
  ...
  `tests/python/integration/runtime/test_isothermal_vacuum_floor_system.py`. The energy (Euler) model is out of scope:
...
- **Stale negative control in the compiled positivity-floor test** (ADC-341): ADC-324 made
  `set_refinement(1e30)` a mono-level hierarchy, so the `tests/python/integration/mpi/test_amr_compiled_positivity_floor.py`
