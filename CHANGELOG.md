...
### Added
- **ADC-510: runtime params in compiled time Programs** -- a `dsl.Param(..., kind="runtime")` read in a Program source / flux / linear-source kernel now lowers to a hoisted uniform `ctx.param(<index>)` read (instead of the previous NotImplementedError reject), wired through a new `ProgramContext::param` -> `System::program_param` live store; the `.so` exports a `pops_program_param_name` ABI table that `install_program` reads to validate names and seed the declaration defaults. `pops.bind(compiled, params={...})` then `sim.set_param(name, value)` change a compiled Program's numeric result WITHOUT recompiling the `.so` and without re-installing; a const param stays inlined byte-identically. New `python/tests/test_time_runtime_params.py` (epic ADC-399).
