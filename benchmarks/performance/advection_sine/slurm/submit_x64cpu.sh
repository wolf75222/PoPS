...
CAMPAIGN="${1:?usage: submit_x64cpu.sh CAMPAIGN.json}"
shift
if (( $# != 0 )); then
  echo "submit_x64cpu.sh refuses caller-provided sbatch options" >&2
  exit 2
fi
...
: "${POPS_PERF_JOB_PYTHON:?export an absolute x86_64 Python 3.10+ used inside x64cpu jobs}"
...
SCHEDULER_DIRECTORY_INPUT="${SCHEDULER_ROOT}/x64cpu"
...
SBATCH_EXPORT_VARIABLES=(
  ...
  POPS_CMAKE POPS_NINJA POPS_CXX_COMPILER
