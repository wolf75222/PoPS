...
prepare_embedded_boundary_geometry_collectively(
  ...
  for (std::size_t local = 0; local < fields.phi.local_size(); ++local)
    for_each_cell(fields.phi.box(local),
                  SampleValidKernel<Dim>{level_set, geometry, fields.phi.fab(local).view()});

  HaloSchedule<Dim> schedule = collective_stage(
      "prepared EB halo schedule",
      [&]() {
        return prepare_halo_schedule(fields.phi, geometry.domain(), topology,
                                     exact_halo_budget(fields.phi, geometry, topology));
      },
      communicator);
