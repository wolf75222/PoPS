...
function(pops_configure_parallel_hdf5_mpi_contract output)
  ...
  set(_pops_common_mpi_core FALSE)
  foreach(_pops_file IN LISTS _pops_mpi_c_files)
    ...
    if(_pops_name_lower MATCHES "\\.(a|lib)$")
    ...
    if("${_pops_real}::${_pops_hash}" IN_LIST _pops_mpi_cxx_identities)
      if(_pops_name_lower MATCHES "(mpi|mpich|msmpi)")
        set(_pops_common_mpi_core TRUE)
      endif()
      # FindMPI may expose required shared companions whose loader name does not contain `mpi`
      # (OpenMPI's libopen-pal is the canonical example).  They are part of the exact process
      # identity when both MPI_C and MPI_CXX name the same path and bytes, so authenticate them
      # alongside libmpi instead of rejecting the HDF5 dependency closure as foreign.
      list(APPEND _pops_common_mpi_runtimes "${_pops_real}")
    endif()
  endforeach()
  list(REMOVE_DUPLICATES _pops_common_mpi_runtimes)
  if(NOT _pops_common_mpi_core OR NOT _pops_common_mpi_runtimes)
    message(FATAL_ERROR
