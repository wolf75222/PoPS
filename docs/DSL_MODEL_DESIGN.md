...
- **Phase A** (#89/#90): `pops.physics.facade.Model` facade (`dsl.py:Model`), named `Param` (`dsl.py:Param`,
  ...
  dispatch `ModelSpec`->`add_block` vs `CompiledModel`->backend adder), `pops.FiniteVolume(limiter=,
...
- **`AmrSystem` in native production** (#92): `pops.AmrSystem.add_native_block` and
...
- **WENO5 everywhere**: SHIPPED on the `.so`/`CompiledModel` paths (`aot` AND `production`, #102;
  ...
  2 ghosts" rejection for the `.so`. (The native `add_block` path via `pops.Model` already carried
...
- `python/pops/dsl.py`: `HyperbolicModel` (all methods), the codegen
...
- `python/pops/__init__.py`: runtime sugar `Model`/`Spatial`/`Explicit`/`IMEX`/
...
- `python/bindings/system/base/system.cpp` + `python/bindings/core/bindings.cpp`: adders `add_block`,
  ...
  `step_adaptive`.
- `include/pops/core/state/variables.hpp`: `VariableRole`/`VariableSet`/`role_from_name`/
  `POPS_EXPORT_BLOCK_METADATA`/`POPS_EXPORT_BLOCK_GAMMA`.
- `include/pops/core/model/physical_model.hpp`: contract `PhysicalModel`/
  `HyperbolicPhysicalModel`/`aux_comps`.
- `include/pops/runtime/builders/compiled/compiled_block_abi.hpp`: AOT ABI `POPS_DEFINE_COMPILED_BLOCK`.
- `include/pops/runtime/dynamic/dynamic_model.hpp`: `IModel`/`ModelAdapter` (JIT).
- `include/pops/runtime/builders/compiled/dsl_block.hpp`: `add_compiled_model` (native, template).
- `include/pops/runtime/builders/compiled/amr_dsl_block.hpp`: `add_compiled_model` on the `AmrSystem` side.
...
3. The NUMERICAL flux is `pops.FiniteVolume(limiter=, riemann=, variables=)` -- `riemann`, NOT `flux` (which is the physical flux). (section 6)
...
1. JIT: `.so` exposes `pops_make_model`/`pops_model_nvars`/`pops_destroy_model`
...
2. AOT: `.so` exposes the `POPS_DEFINE_COMPILED_BLOCK` ABI (`compiled_block_abi.hpp:156`),
...
> `extern "C"` symbols: `pops_native_abi_key` + `pops_install_native`) that inlines
...
| `m.param(name, value)` | `Param` + `Model.param` (#89) | SHIPPED in `const` mode (NAMED constant inlined at codegen, stored in `m.params`) AND in `runtime` mode (section 2b): settable on the `aot` brick (P7-b, `set_block_params`) and in a compiled time Program (ADC-510, `bind(params=)` / `sim.set_param`); the `prototype`/`production` backends freeze it to its declaration value. Case `name=="gamma"` also calls `set_gamma` |
...
- `m.param(name, value)`: SHIPPED in `const` mode (#89, `Param` class + `Model.param`,
  `dsl.py`) AND in `runtime` mode (section 2b): settable on the `aot` brick (P7-b) and in a compiled
  time Program (ADC-510); the `prototype`/`production` backends freeze it to its declaration value.
...
> STATUS. Mode (a) SHIPPED (#89, `Param` class, `dsl.py`). Mode (b) SHIPPED too: on the `aot` brick
> (P7-b, `set_block_params`) and in a compiled time Program (ADC-510, `bind(params=)` /
> `sim.set_param`). The reasoning below records the original engine-change analysis (the routes it
> describes as "Phase 2" are the ones now realized).
...
`:316` -> `POPS_EXPORT_BLOCK_GAMMA` (`variables.hpp:153`), read by `read_block_meta`
...
A `kind="runtime"` parameter can have its value CHANGED at run time WITHOUT recompiling the
`.so`. Two surfaces ship it; both emit `params.get(<index>)` (a member / snapshot of
`pops::RuntimeParams`, include/pops/runtime/config/runtime_params.hpp) wherever the formula
reads the param, instead of an inline `Const`:

1. AOT BRICK path (P7-b, route 2 below realized): a generated brick carries a
   `pops::RuntimeParams params` member; the AOT ABI (`compiled_block_abi.hpp`) transports a flat
   block of doubles (the `_p` symbols), and `System.set_block_params(name, values)` overwrites the
   SHARED block so the brick behaves differently at the next step. cf. the "Runtime params" bullet
   in section 1.
2. COMPILED TIME PROGRAM path (ADC-510): a `dsl.Param(kind="runtime")` read in a Program source /
   flux / linear-source kernel is hoisted ONCE per kernel as a uniform `ctx.param(<index>)` read
   into a `pops::RuntimeParams params` snapshot before the `for_each_cell` loop (captured by value
   into the device lambda); the per-cell expression keeps `params.get(<index>)`. The `.so` exports a
   `pops_program_param_name` table; `install_program` reads it to validate names and seed a LIVE
   `System` param store (`System::program_param` / `set_program_param`), which `ProgramContext::param`
   reads EVERY step (no frozen copy). `pops.bind(compiled, params={...})` and `sim.set_param(name,
   value)` change the numeric result at the next step WITHOUT recompiling or re-installing.

The original two routes for a REAL runtime parameter, kept for context:
1. Pass the parameters via the `Aux` channel (a constant aux field per cell): feasible without a
   codegen change but limited to the canonical components and wasteful (an n*n field for a scalar).
   NOT the route taken.
2. Extend the ABI with a parameter block of doubles and a `Model` carrying members: the route TAKEN
   for the AOT brick (P7-b), mirrored for the Program path by the live `System` param store (ADC-510).

VERDICT. `m.param(name, value)` ships BOTH modes: mode (a) (frozen `const`, inlined hard) and mode
(b) (`runtime`, modifiable without recompilation) -- on the AOT brick (P7-b, `set_block_params`) and
in a compiled time Program (ADC-510, `bind(params=)` / `sim.set_param`). The `prototype` (JIT) and
`production` (native brick) backends still FREEZE a runtime param to its declaration value (a brick
`set_param` there has no effect, reported via `CompiledModel.runtime_param_names`); use the `aot`
backend for a settable brick, or a compiled time Program for a settable whole-system step.
...
`System` facade that routes according to the type of `model`: a `ModelSpec` (`pops.Model(...)`) ->
...
| set_param unknown name | `sim.set_param(name)` / `set_program_param` for a name the installed Program does not declare | "'<name>' is not a runtime parameter of the installed Program (declared: ...); ... INSTANCE/block params are set via bind(params=) / set_block_params" | SHIPPED (ADC-510, `System::set_program_param`). Runtime mode itself is supported (section 2 mode b): settable on `aot` (P7-b) and in a compiled time Program (ADC-510); `prototype`/`production` bricks freeze it (a no-op `set_param` is reported, not raised) |
...
- `production` SHIPPED (#85 System, #92 AmrSystem): `add_compiled_model` (`dsl_block.hpp`) builds
  ...
  has its Python binding (`pops.AmrSystem.add_native_block`, #92); WENO5 + Rusanov +
...
|---|---|---|
| `pops.FiniteVolume(limiter=, riemann=, variables=)` | `FiniteVolume` (`__init__.py:FiniteVolume`), remapped onto `pops.Spatial(limiter=, flux=, recon=)` | SHIPPED (#89). DECISION (point 3): the NUMERICAL flux (Rusanov/HLL/HLLC/Roe) is called `riemann=`, NOT `flux=`, so as not to collide with the PHYSICAL flux `m.flux` of the model. Spec 5 sec.7: each slot takes a TYPED `pops.numerics` descriptor, not a bare string. `limiter=` -> `Spatial.limiter` (`FirstOrder()`/`Minmod()`/`VanLeer()`/`WENO5()`), `riemann=` -> `Spatial.flux` (`Rusanov()`/`HLL()`/`HLLC()`/`Roe()`), `variables=` (`Conservative()`/`Primitive()`) -> `Spatial.recon`. |
| `pops.IMEX(substeps=)` | `pops.IMEX(substeps=)` (`__init__.py:IMEX`) | EXISTS identically. `Explicit(substeps=, method=)` (`ssprk2`/`ssprk3`, #88) and `Implicit(dt_ratio=, substeps=)` (alias of IMEX) too. |
| `pops.DivEpsGrad` | `pops.DivEpsGrad(epsilon=)` (`__init__.py:DivEpsGrad`) | EXISTS. Elliptic operator `div(eps grad)`. eps(x) variable via `set_epsilon_field`. |
| `pops.DirichletWall.circle(radius=)` | `set_poisson(wall="circle", wall_radius=)` (`system.cpp`) | sugar GAP: no `DirichletWall` object; today it is a string argument of `set_poisson`/`add_elliptic_model`. `DirichletWall.circle(r)` would be a constructor returning `(wall="circle", wall_radius=r)`. |
...
| `pops.System(n=, periodic=)` | EXISTS (`__init__.py:System`) | identical. |
...
the rest is renaming/remapping of arguments. Remaining sugar GAP: `pops.DirichletWall`.
...
> (runtime params) is SHIPPED on the `aot` backend (P7-b; `kind="runtime"` + `set_block_params`) AND in
> a compiled time Program (ADC-510; `ctx.param` + `bind(params=)` / `sim.set_param`). The sequencing
> below is kept for traceability; the per-phase tags indicate the real state.
...
3. `CompiledModel` (section 3) produced by `m.compile`, packaging `so_path` +
   metadata already known. WRITE-SET: `python/pops/dsl.py`.
4. Non-dispatch runtime sugar (section 6): `pops.FiniteVolume` (remap of `Spatial`),
   `pops.DirichletWall.circle`, `sim.run`. WRITE-SET: `python/pops/__init__.py`.
5. `sim.add_equation` (section 3): dispatch `ModelSpec` -> `add_block`;
   ...
   -> `add_compiled_block`. WRITE-SET: `python/pops/__init__.py` (method of the `System`
...
6. Error taxonomy (section 4) on the Python side: extend `check()` for the unset
   params, flux/`p` guard, backend/device guard. WRITE-SET: `python/pops/dsl.py`
...
10. SHIPPED. `add_equation`/`add_native_block` on the `pops.AmrSystem` side (`__init__.py:AmrSystem`) route
...
### Phase E: `m.param` runtime -- SHIPPED (AOT brick + compiled time Program)

11. Mode (b) (section 2b): the "ABI with parameters + codegen with members" route is REALIZED.
    AOT brick (P7-b): `include/pops/runtime/config/runtime_params.hpp` (`pops::RuntimeParams`),
    `include/pops/runtime/builders/compiled/compiled_block_abi.hpp` (`_p` ABI),
    `python/bindings/system/base/system.cpp` (`set_block_params`). Compiled time Program (ADC-510):
    the codegen hoists `ctx.param(<index>)` per kernel (`python/pops/codegen/program_emit_*.py`), the
    `.so` exports the `pops_program_param_name` table, and the live `System` param store
    (`System::program_param` / `set_program_param`, read by `ProgramContext::param`) is set via
    `pops.bind(params=)` / `sim.set_param`.
...
12. `pops.Model(...)` composes 100% NATIVE bricks (ModelSpec, C++ tags); `pops.physics.facade.Model(...)` generates
    ...
    **API**: `pops.physics.bricks.HyperbolicBrick` / `pops.physics.bricks.SourceBrick` / `pops.physics.bricks.EllipticBrick` -> `.compile()` ->
    ...
    (>= 1 DSL slot required; otherwise `pops.Model`). `.compile(backend=, target=)` -> `CompiledModel` attachable
    via `add_equation`.

    **Design (option B)**: the mixing is generated at the compilation of the FINAL `pops::CompositeModel<H,S,E>`
    ...
    (`struct NatSrc : pops::PotentialForce { NatSrc(){ qom = pops::Real(-1.0); } };`): native numerics
    ...
    **Covered (prototype)**: backends `aot` (add_compiled_block), `production` (add_native_block,
    ...
    (production only, loader `pops_install_native_amr`); extensible aux (B_z `n_aux=4`, T_e `n_aux=5`)
    ...
    MPI job). WRITE-SET: `python/pops/dsl.py` (partial bricks +
    `HybridModel`), `python/pops/__init__.py` (`pops.CompositeModel`). NO C++ change (the
    `CompositeModel` already composes the mixing; the roles come from `POPS_EXPORT_BLOCK_METADATA`).
