...
| `m.param(name, value)` | `Param` + `Model.param` (#89) | SHIPPED. `const` mode = NAMED constant inlined at codegen (stored in `m.params`); `kind="runtime"` SHIPPED on the `aot` backend (P7-b, ADC-510): emits `params.get(<index>)` and `System.set_block_params` changes the value at runtime without recompiling (the `prototype`/`production` backends freeze it to its declaration value). Case `name=="gamma"` also calls `set_gamma` |
...
- `m.param(name, value)`: SHIPPED in `const` mode (#89, `Param` class + `Model.param`,
  `dsl.py`). `runtime` mode SHIPPED on the `aot` backend (P7-b, ADC-510; `set_block_params`);
  the `prototype`/`production` backends freeze a runtime param to its declaration value.
...
- The AOT ABI (`compiled_block_abi.hpp:156`) carries no parameter: the
  signatures `pops_compiled_residual`/`advance`/`max_speed` (`compiled_block_abi.hpp:159-176`)
...
2. Extend the ABI: add a parameter/block of doubles to the signatures
   `pops_compiled_*`, generate a `Model` constructed with these parameters (members), and
...
| param runtime mode | `m.param(name, value, kind="runtime")` | SHIPPED on `aot` (P7-b, ADC-510): emits `params.get(<index>)`, mutated by `System.set_block_params`. On `prototype`/`production` the runtime param is FROZEN to its declaration value (no error). | `aot` codegen + `set_block_params` (section 2 mode b) |
...
| `pops.IMEX(substeps=)` | `pops.IMEX(substeps=)` (`__init__.py:IMEX`) | EXISTS identically. `Explicit(substeps=, method=)` (`ssprk2`/`ssprk3`, #88) and `Implicit(dt_ratio=, substeps=)` (DEPRECATED alias of IMEX, emits a `DeprecationWarning`; use `SourceImplicit`/`IMEX`) too. |
