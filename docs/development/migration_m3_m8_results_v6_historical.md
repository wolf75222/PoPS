...
The candidate-v6 receipts cited below are external retained evidence from `/Users/romaindespoulain/dev/tmp/PoPS-migration-20260908-evidence/`, which is not tracked in this checkout. The migration manifest is retained separately at `/Users/romaindespoulain/Documents/Codex/2026-09-07/infer-the-intended-scope-from-the/notes/migration-manifest.json`; these paths identify historical provenance and are not repository links.
...
| --- | --- |
| Source and freeze | `9164272546e0074d825603a1ea299990b5ee9ace`; clean MAIN/FROZEN equality is recorded in the external source-freeze receipt (`candidate-final-v6-source-freeze.json`). |
| Native CMake cache | Release, `POPS_NATIVE_DIM=2`, MPI and HDF5 enabled, tests enabled, Python build disabled; exact configure/build commands are in the external configure receipt (`candidate-final-v6-native-current-configure-command.json`) and build receipt (`candidate-final-v6-native-current-build-command.json`). |
...
| Native execution | Exact CTest command, summary, and JUnit are in the external receipts (`candidate-final-v6-native-current-ctest-command.json`, `candidate-final-v6-native-current-summary.json`, and `candidate-final-v6-native-current-ctest.xml`). |
| Python source contract batch | Exact command, result, and JUnit are in the external receipts (`candidate-final-v6-source-repair-batch-command.json`, `candidate-final-v6-source-repair-batch-result.json`, and `candidate-final-v6-source-repair-batch.xml`). |
...
| Skip disposition | The external skip-disposition receipt (`candidate-final-v6-ctest-skip-disposition.json`) identifies `17` duplicate serial discoveries covered by exact named MPI2 registrations and `2` required cells that did not execute: the hyperbolic-boundary MPI2 registration and the Dim1 `PreparedBz` control. They remain open rows. |
| Static and private wheels | The external static-gates receipt (`candidate-final-v6-static-gates.json`) passes M2/M3/M4 check-only, catalog, packaging, Ruff, and Pyright (`662` files, `0` errors, `2` warnings). The external Dim1 (`candidate-final-v6-dim1-private-proof.json`) and Dim2 (`candidate-final-v6-dim2-private-proof.json`) private-proof receipts pass. These establish artifacts and contracts, not numerical science. |
| Source contract batch | The exact rebuilt-package batch contains `169` passing tests with no failure/error/skip in its JUnit. The external source-contract result receipt (`candidate-final-v6-source-repair-batch-result.json`) proves source/native proof integrity; it is not an M3-M8 scientific matrix. |
| Original tutorials | The external tutorial-conformance summary (`candidate-final-v6-launcher-fixed-tutorial-conformance/summary.json`) passes both unchanged profiles: OpenMP at one rank and MPI at two ranks. The private external MPI-launcher repair receipt (`candidate-final-v6-private-prefix-mpiexec-repair.json`) only links the selected private prefix to the installed MPICH launcher; it changes no production or wheel bytes. Tutorial completion does not establish convergence, performance, portability, or accuracy beyond tutorial assertions. |
| PMPI inventory | The external v6 review (`candidate-final-v6-pmpi-inventory-review.md`) and receipt (`candidate-final-v6-pmpi-inventory-receipt.json`) find `14` wrapped communication APIs, `20` explicit non-payload APIs, and `0` unknown/unclassified APIs. This is static inventory only: no new PMPI measurement or MPI replay was performed. |
...
The issue links below are the complete remaining M3-M8/R1 set from the
externally retained, pinned migration manifest (`migration-manifest.json`).

| Scope | Linear issues | Current disposition and remaining boundary |
| --- | --- | --- |
| M3: unify field problems and solve results | [ADC-903](https://linear.app/romain7522/issue/ADC-903); M3.1 [ADC-925](https://linear.app/romain7522/issue/ADC-925), M3.2 [ADC-926](https://linear.app/romain7522/issue/ADC-926), M3.3 [ADC-927](https://linear.app/romain7522/issue/ADC-927), M3.4 [ADC-928](https://linear.app/romain7522/issue/ADC-928) | Static/source contracts and the 169-test batch are recorded. Field accuracy, invalidation, and complete variable/joint field numerical rows remain open. The 21-file/27-site bind-context inventory is preflight only; see the external bind-context preflight (`candidate-final-v6-bind-context-preflight.json`). |
...
| Row | Required update |
| --- | --- |
| Native conditional coverage | Execute the two required cells identified by the external skip disposition (`candidate-final-v6-ctest-skip-disposition.md`), or preserve an evidence-backed refusal/unavailable disposition. Reconcile the full CTest count after any registration change. |
| M3 field accuracy/reuse | Finish the external field plan (`fields-qualification-plan.json`), including accuracy and invalidation cases, with one exact source/artifact/configuration pin. |
