...
| --- | --- |
| Source and freeze | `9164272546e0074d825603a1ea299990b5ee9ace`; clean MAIN/FROZEN equality is recorded in External retained evidence (not tracked in this checkout): `candidate-final-v6-source-freeze.json`. |
| Native CMake cache | Release, `POPS_NATIVE_DIM=2`, MPI and HDF5 enabled, tests enabled, Python build disabled; exact configure/build commands are External retained evidence (not tracked in this checkout): `candidate-final-v6-native-current-configure-command.json` and External retained evidence (not tracked in this checkout): `candidate-final-v6-native-current-build-command.json`. |
...
| Native execution | Exact External retained evidence (not tracked in this checkout): `candidate-final-v6-native-current-ctest-command.json`, External retained evidence (not tracked in this checkout): `candidate-final-v6-native-current-summary.json`, and External retained evidence (not tracked in this checkout): `candidate-final-v6-native-current-ctest.xml`. |
| Python source contract batch | Exact External retained evidence (not tracked in this checkout): `candidate-final-v6-source-repair-batch-command.json`, External retained evidence (not tracked in this checkout): `candidate-final-v6-source-repair-batch-result.json`, and External retained evidence (not tracked in this checkout): `candidate-final-v6-source-repair-batch.xml`. |
...
| Skip disposition | The External retained evidence (not tracked in this checkout): `candidate-final-v6-ctest-skip-disposition.json` identifies `17` duplicate serial discoveries covered by exact named MPI2 registrations and `2` required cells that did not execute: the hyperbolic-boundary MPI2 registration and the Dim1 `PreparedBz` control. They remain open rows. |
| Static and private wheels | External retained evidence (not tracked in this checkout): `candidate-final-v6-static-gates.json` pass M2/M3/M4 check-only, catalog, packaging, Ruff, and Pyright (`662` files, `0` errors, `2` warnings). The External retained evidence (not tracked in this checkout): `candidate-final-v6-dim1-private-proof.json` and External retained evidence (not tracked in this checkout): `candidate-final-v6-dim2-private-proof.json` private proofs pass. These establish artifacts and contracts, not numerical science. |
| Source contract batch | The exact rebuilt-package batch contains `169` passing tests with no failure/error/skip in its JUnit. The External retained evidence (not tracked in this checkout): `candidate-final-v6-source-repair-batch-result.json` proves source/native proof integrity; it is not an M3-M8 scientific matrix. |
| Original tutorials | External retained evidence (not tracked in this checkout): `candidate-final-v6-launcher-fixed-tutorial-conformance/summary.json` passes both unchanged profiles: OpenMP at one rank and MPI at two ranks. The private External retained evidence (not tracked in this checkout): `candidate-final-v6-private-prefix-mpiexec-repair.json` only links the selected private prefix to the installed MPICH launcher; it changes no production or wheel bytes. Tutorial completion does not establish convergence, performance, portability, or accuracy beyond tutorial assertions. |
| PMPI inventory | The External retained evidence (not tracked in this checkout): `candidate-final-v6-pmpi-inventory-review.md` and External retained evidence (not tracked in this checkout): `candidate-final-v6-pmpi-inventory-receipt.json` find `14` wrapped communication APIs, `20` explicit non-payload APIs, and `0` unknown/unclassified APIs. This is static inventory only: no new PMPI measurement or MPI replay was performed. |
...
External retained evidence (not tracked in this checkout): `migration-manifest.json`.

| Scope | Linear issues | Current disposition and remaining boundary |
| --- | --- | --- |
| M3: unify field problems and solve results | [ADC-903](https://linear.app/romain7522/issue/ADC-903); M3.1 [ADC-925](https://linear.app/romain7522/issue/ADC-925), M3.2 [ADC-926](https://linear.app/romain7522/issue/ADC-926), M3.3 [ADC-927](https://linear.app/romain7522/issue/ADC-927), M3.4 [ADC-928](https://linear.app/romain7522/issue/ADC-928) | Static/source contracts and the 169-test batch are recorded. Field accuracy, invalidation, and complete variable/joint field numerical rows remain open. The 21-file/27-site bind-context inventory is preflight only; see External retained evidence (not tracked in this checkout): `candidate-final-v6-bind-context-preflight.json`. |
...
| Row | Required update |
| --- | --- |
| Native conditional coverage | Execute the two required cells identified by the External retained evidence (not tracked in this checkout): `candidate-final-v6-ctest-skip-disposition.md`, or preserve an evidence-backed refusal/unavailable disposition. Reconcile the full CTest count after any registration change. |
| M3 field accuracy/reuse | Finish the declared External retained evidence (not tracked in this checkout): `fields-qualification-plan.json`, including accuracy and invalidation cases, with one exact source/artifact/configuration pin. |
