...
## Follow-up: architecture interruption race on a1563f5

[CI run `34777658868`](https://github.com/wolf75222/PoPS/actions/runs/34777658868) records 70 successful checks, two skipped checks and two failures. All 37 Python shards, C++ checks, MPI and Docs PR lint pass. The architecture job has one failing test, 764 passing tests and one expected skip; the aggregate correctly fails solely because architecture is required.

The interruption test terminated its subprocess on any active call phase, including the first test before its failure report existed. A targeted local reproduction confirms the saved active node is `test_failure`, with zero completed tests and zero failure receipts. The corrected synchronization waits for `test_active` specifically, after the earlier failure and teardown have been recorded. The assertions still require the failure traceback, incomplete session/file and absence of a session-finish event; they now also require exactly one of two collected tests to have finished and the second call to remain active. The production timing plugin and required CI gate are unchanged.

Both affected parameterizations pass in 0.34 seconds (`timing-receipts.log`, `after.xml`), compared with one failure and one pass before the correction (`before.xml`). The subprocess snapshots and event logs preserve the before/after evidence under `/Users/romaindespoulain/dev/tmp/PoPS-ci-fixes-20260913-evidence/round5`. No native rebuild, broad architecture suite or previously successful native validation was repeated. PR677 remains closed and preserved; PR678 remains the integration branch. CI for this test correction awaits the next push and is not yet verified.
