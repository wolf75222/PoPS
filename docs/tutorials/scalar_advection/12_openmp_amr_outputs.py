...
if PARAVIEW_ROOT:
    paraview_root = Path(PARAVIEW_ROOT).expanduser()
    pvpython_candidates = (
        paraview_root / "Contents" / "bin" / "pvpython",
        paraview_root / "bin" / "pvpython",
    )
    pvpython = next((path for path in pvpython_candidates if path.is_file()), None)
    if pvpython is None:
        raise RuntimeError(
            "POPS_PARAVIEW_ROOT names no ParaView installation containing bin/pvpython: %s"
            % paraview_root
        )
