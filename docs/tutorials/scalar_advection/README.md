...
Les scripts OpenMP configurent un thread par defaut avant l'initialisation native.
`POPS_THREADS=4` en demande quatre sans modifier le script. Les scripts MPI
...
`docs/tutorials/scalar_advection/results/01_openmp_preset_ssprk2.npz`.
...
`docs/tutorials/scalar_advection/results/12_openmp_amr_outputs/`. Le script rouvre ensuite le catalogue
...
```bash
OUTPUT_DIR="$PWD/docs/tutorials/scalar_advection/results/14_mpi_amr_outputs/solution/tracer"
...
POPS_CATALYST=1 scripts/paraview_python.sh \
  docs/tutorials/scalar_advection/12_openmp_amr_outputs.py
```
...
`docs/tutorials/scalar_advection/results/13_openmp_amr_restart/`.
...
```bash
python docs/tutorials/scalar_advection/15_openmp_convergence.py
```
