#!/usr/bin/env python3
"""Advection scalaire AMR OpenMP avec horloges de niveaux synchrones.

Le ratio spatial reste 2:1, mais les deux niveaux avancent avec le meme pas temporel. Cette
variante isole la politique d'horloge sans modifier la physique ni le tagging.
"""
...
THREADS = int(os.environ.get("POPS_THREADS", "1"))
pops.set_threads(THREADS)
...
model = pops.Model("scalar_advection_amr_synchronous", frame=frame)
U = model.state(
    "U",
    components=("u",),
...
case = pops.Case("tutorial_scalar_advection_amr_synchronous")
...
# 5. Programme et condition initiale identiques a la variante subcyclee.
...
# 6. Le tagging reste sur u ; seule l'execution temporelle devient synchrone.
