...
from pops.time import FixedDt, StagePoint, TimePoint, evaluation_partition
...
# Chaque evaluation choisit son horloge de partition ; les points des etats restent inchanges.
program = pops.Program("Strang explicit")
q = program.state(tracer_U)
relaxation_map = relaxation_operator(program=program)

with evaluation_partition(program, "source"):
...
with evaluation_partition(program, "explicit"):
