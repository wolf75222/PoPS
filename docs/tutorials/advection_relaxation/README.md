...
```bash
python docs/tutorials/advection_relaxation/01_openmp_imex_local.py
```
...
```bash
python docs/tutorials/advection_relaxation/02_openmp_imex_cg.py
```
...
```bash
python docs/tutorials/advection_relaxation/03_openmp_lie_splitting.py
```
...
```bash
python docs/tutorials/advection_relaxation/04_openmp_strang_splitting.py
```

Dans le script de Strang, les `StagePoint` portent deux temps : celui de la
source et celui du transport explicite. Les blocs
`with evaluation_partition(program, "source")` et `"explicit"` choisissent
le temps de chaque evaluation sans modifier les points des etats. Cette
selection explicite est requise lorsque les deux partitions n'ont pas encore
avance jusqu'au meme temps.
