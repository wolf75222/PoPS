...
The native `POPSAND8` accepted-state format records the actual last committed AMR attempt even when its face ledger is empty. Failed attempts are excluded from the accepted image. A fresh process starts allocating after the recorded committed attempt; a live process retains its already consumed attempt identities across rollback, regrid or in-place restoration. Exact continuation comparisons therefore require the same subsequent failure schedule. They do not reproduce uncommitted failures that occurred only in an abandoned process. Legacy formats 4–7 remain inspectable but lack this authority and are refused for continuation.
...
The tutorial samples at intervals of `min(POPS_OUTPUT_INTERVAL,0.01)` through `t=1.5` to populate the fitting windows, then at the ordinary cadence and required paper times. The recorded calendar forms the union using exact decimal rational targets and converts each once to binary64. Its cap-safe subdivisions remain anchored to the full global interval across checkpoint restarts. These are integration endpoints, not interpolated images. `chunks.jsonl` retains each actual returned public-run boundary and latest accepted dt; it does not claim to enumerate all smaller AdaptiveCFL steps inside a call.
...
When fitted curves exist, `fourier-growth-rates.csv` and its PNG/PDF table export the measured rates, fixed intervals, sample counts, fit statistics and signed percentage difference from nominal theory, corresponding to the comparison in Figure 5.4(d). Only completed fits appear as rows. The slope standard error describes the regression; it does not estimate spatial or temporal discretization error. No rate table is produced before any fit is available.
