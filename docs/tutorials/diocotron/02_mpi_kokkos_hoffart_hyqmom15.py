#!/usr/bin/env python3
"""Hoffart's conducting-disk HYQMOM15--Poisson benchmark, in mapped finite volumes.

The physical model is the full finite-Omega fifteen-moment Vlasov--Poisson system.
PoPS stores q=r*M for every raw Cartesian velocity moment through degree four on (r,theta), retaining Cartesian momentum components.
...
model = pops.Model("Hoffart HYQMOM15", frame=frame)
...
physical_flux = model.flux("mapped_transport", frame=frame, state=U,
    components={
        radial_axis: tuple(radial_cosine * fx + radial_sine * fy
                           for fx, fy in zip(closed_flux.x, closed_flux.y, strict=True)),
        angular_axis: tuple((-angular_sine * fx + angular_cosine * fy) / radius
                            for fx, fy in zip(closed_flux.x, closed_flux.y, strict=True)),
    })
# The complete directional Jacobian includes the mapped linear combination.
# No Cartesian chain decomposition is assumed for an oblique direction.
model.wave_speeds_from_jacobian(im_tol=1e-8, eig_max_iter=1000)
...
case = pops.Case("Hoffart HYQMOM15 mode %d" % MODE)
...
# One affine velocity map acts on all fifteen moments. Applying CN independently
# to the different moment degrees would not preserve realizability.
source_endpoint = program.affine_moment_update(q.n, mean_endpoint,
    linear_operator=rotation, theta_dt=s, order=4, name="common affine velocity map")
...
# replacement Maxwellian is introduced in initialization or the closure.
initial_x_moments = []
initial_y_moments = []
...
# Convex parent injection and volume averaging preserve the full moment cone.
# This variant is spatially first order; its resolution error is measured separately.
...
if RANK == 0:
    print("Validating the authored HYQMOM15 case", flush=True)
