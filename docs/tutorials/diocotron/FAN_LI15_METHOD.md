...
**Development status:** the module contains the complete linear physical-model, method, source, initialization, AMR, checkpoint, and output pipeline. Its native common-face transport/CFL, certified candidate admissibility, and AMR nonconservative side contributions are still being integrated. The full case is not currently executable or scientifically qualified in this unbuilt worktree. Syntax or public authoring acceptance must not be reported as native runtime acceptance, a successful simulation, or reproduction of a paper figure.
...
The solver explicitly selects **`interface_coupling="fine_flux"`**. The conservative full-tensor implementation is integrated from revision `8d351df`. The actual linear tutorial passes public authoring, validation, resolution and complete native source emission, including the explicit pole-face contract, one Schur source prefix and both SSPRK2 hierarchy barriers. Native compilation and actual Fan–Li execution remain separate qualification gates. The selected interface method retains the full finite-`Omega` tensor and its original residual tolerance.

The native transport route carries the common-face covector and full-system bound, conservative flux, and two separate nonconservative side contributions. Its hierarchy barrier replaces coarse-side contributions with canonical fine subfaces before dependent stages and records only the conservative flux in the reflux basis. Refluxing `P_g` as an antisymmetric flux would change the method. Exact parent injection is required for both prolongation and coarse/fine traces; reconstructed coarse traces would need the missing within-cell path treatment. Face, candidate, and synchronized-state covariance certification and collective transactional failure still require real MPI qualification.

Only after those repairs and a coherent native rebuild should low-resolution runs establish first-step validity, sustained admissibility, source/field timestamps, conservation of the appropriate transport rows, actual coarse/fine behavior, and checkpoint/restart parity. Then real time/space studies, stiff-source limitations, mass and energy budgets, and measured time/storage costs determine campaign readiness. This authoring work has performed no native simulation, deployment, or high-resolution allocation.
