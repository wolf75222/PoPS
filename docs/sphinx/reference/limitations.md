...
ce sont les bords du perimetre valide aujourd'hui.
...
- `production` est le seul branchable sur AMR (`AmrSystem.add_equation` exige
  ...
  chemin `.so` (l'ABI du loader ne les transporte pas). Detail :
  [DSL_MODEL_DESIGN.md](https://github.com/wolf75222/adc_cpp/blob/master/docs/DSL_MODEL_DESIGN.md).

## Poisson FFT : refuse sous MPI

Le solveur Poisson spectral (`PoissonFFTSolver`) est mono-rang PAR DESIGN. Sous MPI (`n_ranks >
...
Etat detaille et statuts par aspect :
