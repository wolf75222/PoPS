...
A `pops.physics.Model` selects the solver/reconstruction with TYPED `pops.numerics`
descriptors (`pops.numerics.spatial.FiniteVolume(riemann=HLLC(), reconstruction=WENO5Z())`;
Spec 5 sec.7 rejects a bare string) and generates the model hooks from physical roles via `m.enable_hllc()` /
