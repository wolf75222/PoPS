...
- Exposed: `add_block(..., recon=Conservative()|Primitive(), ...)`; Python
  `pops.Spatial(recon=Primitive())` or `Spatial(recon=pops.numerics.variables.Primitive())`. AMR rejects the primitive
...
sim.add_block("ne", model=models.diocotron(B0=1.0, alpha=1.0),
              spatial=pops.Spatial(limiter=pops.numerics.reconstruction.FirstOrder(), flux=Rusanov()))
