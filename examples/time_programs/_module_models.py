"""Small pure-Module models shared by the time-program examples.

These helpers keep the examples on the corrective route:

    pops.model.Module -> pops.compile_problem(...) -> sim.install(...)

They do not use the legacy physics facade or top-level runtime brick aliases.
"""
...
def isothermal_transport_module(name, *, cs2=0.5):
    """Three-component isothermal transport with a default Poisson field operator."""
...
def linear_source_module(name, *, coefficient):
    """One-component zero-flux model with a default linear source S(rho)=c*rho."""
    mod = model.Module(name)
    ...
    mod.operator(
        ...
        kind="local_source",
        capabilities={"default": True},
        expr=[float(coefficient) * rho],
    )
    return mod


def passive_scalar_module(name):
    """One-component zero-flux model used by matrix-free Program examples."""
    return linear_source_module(name, coefficient=0.0)
