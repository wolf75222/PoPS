...
``@adc.lib.solver`` registers a GENERATED-brick solver whose body BUILDS a solver
...
mirrors ``adc.lib.solvers.GMRES()``), and the generated-C++ lowering + run is the
...
def main():
    ...
    # same category and a scheme string as adc.lib.solvers.GMRES().
