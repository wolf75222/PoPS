...
native (``adc::HLLCFlux``) or generated brick is selected -- it is the fourth Spec 3 brick kind
(native / generated / macro / external-C++). The C++ side registers a manifest entry at static-init
time and exports an ``adc_brick_manifest()`` reader; the Python side dlopens it and surfaces an
``external_cpp`` :class:`adc.lib.BrickDescriptor`. This example builds an Euler-like board, selects
...
The C++ an advanced user writes (built into ``my_riemann.so`` with the adc headers)::
    ...
    struct MyRiemann {
      ...
      // as the native adc::HLLCFlux. (Kernel body omitted; it is ordinary finite-volume code.)
    };

    // Register the brick id, its catalog slot, and the model capabilities it requires (CSV).
    // The 3-argument ADC_REGISTER_BRICK macro leaves the PROVIDES field empty.
    ADC_REGISTER_BRICK("my_riemann", "riemann", "pressure,wave_speeds");

    // The host the user links exports the manifest reader load_cpp_library() calls:
    extern "C" const char* adc_brick_manifest();  // JSON over BrickRegistry::instance()
...
# The manifest the user's ``adc_brick_manifest()`` would return for the brick above. The example
# falls back to registering it through the real ``adc.lib._register_manifest`` seam (the same code
...
def register_external_brick():
    ...
    If ``ADC_BRICK_SO`` points at a loadable brick library, :func:`adc.lib.load_cpp_library`
    ...
    """
    so_path = os.environ.get("ADC_BRICK_SO")
...
def main():
    ...
    # 2) Register the brick (real .so dlopen when ADC_BRICK_SO is set, else the manifest seam).
