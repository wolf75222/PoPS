...
[[nodiscard]] SolveOutcome backward_euler_source(
  ...
  const int safeguard_steps = static_cast<int>(detail::collective_max_component(statistics, 7, lane));
  ...
  if (failed_cells > 0) {
    ...
    const int reason_high =
        static_cast<int>(detail::collective_reason(statistics, status_code, failure.index, 10, lane));
