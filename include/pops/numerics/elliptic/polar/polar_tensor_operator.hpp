...
/// WITHOUT the azimuthal term (computed separately). Free device-clean functor (POPS_HD). Face
...
/// POPS_HD accessors recomputed in the kernel).
...
struct PolarJacobiApplyKernel {
  ...
  POPS_HD void operator()(int i, int j) const { out(i, j) = z(i, j) * idiag(i, j); }
};
