...
template <class Model>
struct ModelRecoveryCandidate<Model, std::void_t<typename Model::Prim>> {
...
struct ModelVariableInversionSource {
  ...
  operator()(const typename Model::State& state, const ProviderValues<0>&,
    ...
    if constexpr (requires(const Model& value, const typename Model::State& input) {
                    { value.recover(input).succeeded() } -> std::same_as<bool>;
                    { value.recover(input).value[0] } -> std::convertible_to<Real>;
                  }) {
      const auto recovered = model.recover(state);
      if (!recovered.succeeded())
        return InversionResult<ModelRecoveryCandidateType<Model>,
                               VariableRecoveryInversionFailure>::
            fail(VariableRecoveryInversionFailure::kSourceRejected);
      return InversionResult<ModelRecoveryCandidateType<Model>,
                             VariableRecoveryInversionFailure>::success(recovered.value);
    } else if constexpr (HasPrimitiveVars<Model>) {
      return InversionResult<ModelRecoveryCandidateType<Model>,
...
/// Explicit no-op projection authority.  The recovery schedule below does not request it, but it
/// remains prepared so a future model-declared schedule can make the projection act observable
/// without adding a second candidate path.
template <int Dim, class Candidate>
struct IdentityVariableRecoveryProjection {
  static constexpr int dimension = Dim;

  static constexpr PreparedProviderIdentity provider_identity() noexcept {
    return {"pops.runtime.identity-variable-recovery-projection", 1};
  }
  ...
  POPS_HD ProjectionResult<Candidate> operator()(const Candidate& candidate,
                                                 const ProviderValues<0>&) const {
...
struct ModelRecoveryAdmissibilityProvider {
  ...
  static constexpr PreparedProviderIdentity provider_identity() noexcept {
    if constexpr (requires { Model::provider_identity(); })
...
class PreparedModelVariableInversionRecovery final {
 public:
  ...
  using Projection =
      ProjectionProvider<Model::dimension, Candidate, Inputs,
                         IdentityVariableRecoveryProjection<Model::dimension, Candidate>>;
  ...
  explicit PreparedModelVariableInversionRecovery(const Model& model)
      : inversion_(Problem{"conservative-state",
        ...
        admissibility_(prepare_model_recovery_admissibility(model)),
        projection_("primitive-candidate", "recovery-inputs",
                    IdentityVariableRecoveryProjection<Model::dimension, Candidate>{}),
        schedule_(acceptance_schedule_()) {}
  ...
  [[nodiscard]] PreparedVariableRecoveryAttempt<N> recover(const Real (&conserved)[N]) {
    ...
    Candidate candidate = std::move(inversion_outcome).consume().candidate();
    const EnforcementRule rule = schedule_.at(EnforcementPhase::kAcceptance);
    if (!rule.check) {
    ...
    AdmissibilityResult decision = admissibility_.evaluate(candidate);
    if (!decision.accepted && rule.project_if_invalid) {
      auto projected = projection_.project(candidate, Inputs{});
      recovered.projection_attempted = true;
      recovered.projection_changed = projected.changed();
      candidate = std::move(projected).consume();
      decision = admissibility_.evaluate(candidate);
    }
    if (!decision.accepted) {
      recovered.outcome.status = RecoveryStatus::kRejected;
      recovered.outcome.cause = RecoveryCause::kInadmissibleCandidate;
      recovered.outcome.reason_code = decision.diagnostic_code;
      return recovered;
    }

    for (int component = 0; component < N; ++component)
      recovered.outcome.value[component] = candidate[component];
