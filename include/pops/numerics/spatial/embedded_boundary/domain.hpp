...
///   - @c POPS_HD Real level_set(Real x, Real y) const : signed level set, < 0 INSIDE the active
///     domain, 0 on the boundary, > 0 outside (the conducting-wall convention of GeometricMG);
///   - @c POPS_HD Real operator()(Real x, Real y) const : the CALLABLE form consumed by cut_fraction /
...
///   - @c POPS_HD bool cell_active(Real x, Real y) const : a cell is ACTIVE when its CENTER is inside
