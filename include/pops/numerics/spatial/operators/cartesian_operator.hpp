...
struct AssembleRhsKernel {
  ...
  POPS_HD void operator()(int i, int j, std::uint64_t& failure) const {
    ...
    const auto Ryp = reconstruct_pp_recovered<Model>(model, u, i, j + 1, 1, -1, lim, recon_prim,
                                                     pos_floor, pos_comp);
    if (!record_reconstruction_recoveries(failures, failure, Lxm, Rxm, Lxp, Rxp, Lym, Rym, Lyp,
                                          Ryp)) {
      for (int c = 0; c < Model::n_vars; ++c)
        r(i, j, c) = Real(0);
      return;
    }
    const auto evaluation_xm = evaluate_numerical_flux_at(nflux, model, Lxm.value, ax, i - 1, j,
                                                          Rxm.value, ax, i, j, xface);
    const auto evaluation_xp = evaluate_numerical_flux_at(nflux, model, Lxp.value, ax, i, j,
    ...
    const auto evaluation_ym = evaluate_numerical_flux_at(nflux, model, Lym.value, ax, i, j - 1,
                                                          Rym.value, ax, i, j, yface);
    const auto evaluation_yp = evaluate_numerical_flux_at(nflux, model, Lyp.value, ax, i, j,
...
struct HllFaceSpeedXKernel {
  ...
  POPS_HD void operator()(int i, int j, std::uint64_t& failure) const {
    const auto left = reconstruct_pp_recovered<Model>(model, u, i - 1, j, 0, +1, lim, recon_prim,
                                                      pos_floor, pos_comp);
    const auto right = reconstruct_pp_recovered<Model>(model, u, i, j, 0, -1, lim, recon_prim,
                                                       pos_floor, pos_comp);
    if (!record_reconstruction_recoveries(failures, failure, left, right)) {
      ws(i, j, 0) = Real(0);
      ws(i, j, 1) = Real(0);
      return;
    }
    ...
    Real lower, upper;
    hll_speeds(physical, make_face_trace_at<Model>(left.value, ax, i - 1, j),
               make_face_trace_at<Model>(right.value, ax, i, j), face, lower, upper);
    ws(i, j, 0) = lower;
    ws(i, j, 1) = upper;
  }
};
...
struct HllFaceSpeedYKernel {
  ...
  POPS_HD void operator()(int i, int j, std::uint64_t& failure) const {
    const auto left = reconstruct_pp_recovered<Model>(model, u, i, j - 1, 1, +1, lim, recon_prim,
                                                      pos_floor, pos_comp);
    const auto right = reconstruct_pp_recovered<Model>(model, u, i, j, 1, -1, lim, recon_prim,
                                                       pos_floor, pos_comp);
    if (!record_reconstruction_recoveries(failures, failure, left, right)) {
      ws(i, j, 2) = Real(0);
      ws(i, j, 3) = Real(0);
    ...
    const PhysicalFluxView<Model> physical{model};
    Real lower, upper;
    hll_speeds(physical, make_face_trace_at<Model>(left.value, ax, i, j - 1),
               make_face_trace_at<Model>(right.value, ax, i, j), face, lower, upper);
    ws(i, j, 2) = lower;
    ws(i, j, 3) = upper;
  }
...
inline void fill_hll_face_speed_cache(const Model& model, const MultiFab& U, const MultiFab& aux,
                                      ...
                                      FluxEvaluationTracker& failures) {
  for (int local = 0; local < U.local_size(); ++local) {
    ...
    failures.merge(reduce_max_uint64_cell(
        xface_box(U.box(local)),
        HllFaceSpeedXKernel<Limiter, Model>{model, state, providers, speeds, limiter, recon_prim,
                                            pos_floor, pos_comp, failures.recorder()}));
    failures.merge(reduce_max_uint64_cell(
        yface_box(U.box(local)),
        HllFaceSpeedYKernel<Limiter, Model>{model, state, providers, speeds, limiter, recon_prim,
                                            pos_floor, pos_comp, failures.recorder()}));
  }
}
...
struct AssembleRhsHllCachedKernel {
  ...
  POPS_HD void operator()(int i, int j, std::uint64_t& failure) const {
    ...
    const auto evaluation_xm =
        hll_flux_with_speeds(physical, make_face_trace_at<Model>(Lxm.value, ax, i - 1, j),
                             make_face_trace_at<Model>(Rxm.value, ax, i, j), xface, sLxm, sRxm);
    const auto evaluation_xp =
        hll_flux_with_speeds(physical, make_face_trace_at<Model>(Lxp.value, ax, i, j),
                             make_face_trace_at<Model>(Rxp.value, ax, i + 1, j), xface, sLxp, sRxp);
    failures.record(evaluation_xm, failure);
    failures.record(evaluation_xp, failure);
    const auto Fxm = apply_face_measure(evaluation_xm.checked_density(), xface).value;
    const auto Fxp = apply_face_measure(evaluation_xp.checked_density(), xface).value;
    ...
    const auto evaluation_ym =
        hll_flux_with_speeds(physical, make_face_trace_at<Model>(Lym.value, ax, i, j - 1),
                             make_face_trace_at<Model>(Rym.value, ax, i, j), yface, sLym, sRym);
    const auto evaluation_yp =
        hll_flux_with_speeds(physical, make_face_trace_at<Model>(Lyp.value, ax, i, j),
                             make_face_trace_at<Model>(Ryp.value, ax, i, j + 1), yface, sLyp, sRyp);
    failures.record(evaluation_ym, failure);
    failures.record(evaluation_yp, failure);
    const auto Fym = apply_face_measure(evaluation_ym.checked_density(), yface).value;
    const auto Fyp = apply_face_measure(evaluation_yp.checked_density(), yface).value;
...
struct FaceFluxHllCachedXKernel {
  ...
  POPS_HD void operator()(int i, int j, std::uint64_t& failure) const {
    ...
    const auto evaluation = hll_flux_with_speeds(
        physical, make_face_trace_at<Model>(left.value, ax, i - 1, j),
        make_face_trace_at<Model>(right.value, ax, i, j), face, speed_left, speed_right);
...
struct FaceFluxHllCachedYKernel {
  ...
  POPS_HD void operator()(int i, int j, std::uint64_t& failure) const {
    ...
    const PhysicalFluxView<Model> physical{model};
    const auto evaluation = hll_flux_with_speeds(
        physical, make_face_trace_at<Model>(left.value, ax, i, j - 1),
        make_face_trace_at<Model>(right.value, ax, i, j), face, speed_left, speed_right);
    failures.record(evaluation, failure);
    const auto value = apply_face_measure(evaluation.checked_density(), face).value;
    for (int component = 0; component < Model::n_vars; ++component)
