...
/// exposes apply(System&, const System::field_type& aux, Real dt). The auxiliary carrier therefore
/// has the same exact compile-time rank and memory space as every block in the system.
template <class C, class System>
concept CoupledSourceFor = CoupledSystemLike<System> &&
                           requires(const C c, System& s, const typename System::field_type& aux,
                                    Real dt) { c.apply(s, aux, dt); };
...
struct NoCoupledSource {
  ...
  void apply(System&, const typename System::field_type&, Real) const {}
};
