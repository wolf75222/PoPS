...
class AdmissibleSet final {
 public:
  ...
  static_assert(std::tuple_element_t<0, std::tuple<Constraints...>>::kind ==
                    AdmissibilityConstraintKind::kFinite,
                "an admissible set must run a finite constraint before model predicates");
