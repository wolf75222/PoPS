...
#include <pops/core/foundation/kokkos_env.hpp>  // detail::ensure_kokkos_initialized: Kokkos init BEFORE kokkos_malloc
...
class ManagedArena {
 public:
  ...
  // issue #271, gdb on CI glibc): instance() is inline and the pybind11 module _pops is
  ...
  void* allocate(std::size_t bytes) {
    ...
    void* p =
        Kokkos::kokkos_malloc<Kokkos::SharedSpace>("pops_fab", bytes);  // portable unified memory
    if (!p)
      throw std::bad_alloc();
    ++misses_;
    reserved_ += bytes;
