...
class AmrProgramContext {
 ...
 private:
  ...
  void restart_regrid_() const {
    ...
    for (const std::string& name : facade_->history_names()) {
      HistoryImage image;
      image.name = name;
      image.depth = facade_->history_depth(name);
      image.components = facade_->history_ncomp(name);
      image.initialized = facade_->history_initialized(name);
      image.fill_count = facade_->history_fill_count(name);
      for (int slot = 0; slot < image.depth; ++slot) {
        image.slot_dt.push_back(facade_->history_slot_dt(name, slot));
        image.slots.push_back(facade_->history_global(name, slot));
      }
      histories.push_back(std::move(image));
    }
    const std::vector<std::uint8_t> descriptor_state = facade_->program_accepted_state();
    runtime_state().hist_ = {};
    history_levels_.clear();
    ...
    if (!histories.empty()) {
      ...
      for (const HistoryImage& image : histories) {
        names.push_back(image.name);
        depths.push_back(image.depth);
        components.push_back(image.components);
      }
      AmrProgramAcceptedState<Dim> rematerialized =
          deserialize_amr_program_accepted_state<Dim>(descriptor_state);
      rematerialized.spatial_contract = runtime_->spatial_contract();
      rematerialized.topology_epoch = runtime_->topology_epoch();
      rematerialized.materialization_generation = runtime_->materialization_generation();
      rematerialized.level_clocks.clear();
      for (std::size_t level = 0; level < runtime_->hierarchy().num_levels(); ++level)
        rematerialized.level_clocks.push_back(
            {static_cast<int>(level), facade_->macro_step(), {0, 1}, facade_->time()});
      rematerialized.accepted_face_flux = {};
      rematerialized.accepted_interface_flux.clear();
      rematerialized.synchronization_events.clear();
      rematerialized.flux_budget_contract.clear();
      rematerialized.coupling_contract.clear();
      if (rematerialized.temporal_partition.kind == TemporalPartitionKind::CellLocal)
        rematerialized.temporal_partition.topology_epoch = rematerialized.topology_epoch;
      facade_->materialize_program_restart_histories(
          serialize_amr_program_accepted_state(rematerialized), names, depths, components);
      for (const HistoryImage& image : histories) {
        for (int slot = 0; slot < image.depth; ++slot) {
          facade_->restore_history(image.name, slot, image.slots[static_cast<std::size_t>(slot)]);
          facade_->restore_history_slot_dt(image.name, slot,
                                           image.slot_dt[static_cast<std::size_t>(slot)]);
        }
        facade_->restore_history_metadata(image.name, image.initialized, image.fill_count);
      }
    }
