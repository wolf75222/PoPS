      ...
      for_each_cell(rhs.box(local), [=] POPS_HD(const Index<Dim>& cell) {
        for (int component = 0; component < components; ++component) {
          ...
          for (int axis = 0; axis < Dim; ++axis) {
            ...
            ++upper_face[axis];
            divergence += (named_flux_face_value_(views[static_cast<std::size_t>(axis)], upper_face,
                                                  axis, component) -
                           named_flux_face_value_(views[static_cast<std::size_t>(axis)], lower_face,
                                                  axis, component)) /
                          geom.spacing(axis);
          }
...
RuntimeParams program_params(int program_block) const {
  ...
  return facade_->program_params_(program_block);
}
...
void commit_many(std::initializer_list<std::pair<field_type*, const field_type*>> commits) const {
  ...
  if (!active_attempt_states_.empty()) {
    ...
    try {
      ...
      for (std::size_t candidate = 0; candidate < commits.size(); ++candidate) {
        ...
        expression_candidate[workspace.commit_targets[candidate]] =
            snapshot_flux_expressions[candidate];
      }
    } catch (...) {
  ...
  for (std::size_t candidate = 0; candidate < commits.size(); ++candidate)
    facade_->validate_prepared_amr_state_publication_candidate(
        *workspace.commit_runtime_blocks[candidate], active_level_,
        *workspace.commit_snapshots[candidate]);

  std::fill(workspace.accepted_snapshot_by_runtime.begin(),
            workspace.accepted_snapshot_by_runtime.begin() + static_cast<std::size_t>(n_blocks()),
            commits.size());
  std::size_t accepted_targets = 0;
  for (std::size_t candidate = 0; candidate < commits.size(); ++candidate) {
    const int runtime_block = *workspace.commit_runtime_blocks[candidate];
    if (workspace.commit_targets[candidate] !=
        &facade_->program_prepared_amr_block_state_(runtime_block, active_level_))
      continue;
    ++accepted_targets;
    workspace.accepted_snapshot_by_runtime[static_cast<std::size_t>(runtime_block)] = candidate;
  }
  if (accepted_targets != 0) {
    ...
    try {
      for (int runtime_block = 0; runtime_block < n_blocks(); ++runtime_block) {
        const std::size_t snapshot =
            workspace.accepted_snapshot_by_runtime[static_cast<std::size_t>(runtime_block)];
        field_type& publication = workspace.publication_candidates.at(
            static_cast<std::size_t>(active_level_) * workspace.block_capacity +
            static_cast<std::size_t>(runtime_block));
        if (snapshot == commits.size())
          copy_full_(facade_->program_prepared_amr_block_state_(runtime_block, active_level_),
                     publication);
        else
          copy_full_(*workspace.commit_snapshots[snapshot], publication);
      }
    } catch (...) {
    ...
    facade_->publish_prepared_amr_program_candidates(
        active_level_, std::span<field_type* const>(workspace.publication_program_candidates));
    return;
  }
}
