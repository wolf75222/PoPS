...
#include <pops/numerics/elliptic/linear/generic_krylov.hpp>  // ApplyFn / cg / bicgstab / gmres / richardson (solve_linear_schur seam)
...
/// Krylov method id for the ``ctx.solve_linear_schur(...)`` seam (ADC-633). The codegen emits the id as
/// a plain int; both ProgramContext (uniform) and AmrProgramContext (hierarchy) dispatch on it.
enum SchurSolveMethod {
  kSchurSolveCg = 0,          ///< pops::cg_solve (SPD, no preconditioner parameter)
  kSchurSolveBicgstab = 1,    ///< pops::bicgstab_solve (the condensed-Schur default)
  kSchurSolveGmres = 2,       ///< pops::gmres_solve (restarted; @p restart = basis size)
  kSchurSolveRichardson = 3,  ///< pops::richardson_solve (omega = 1)
};
...
class ProgramContext {
 public:
  ...
  /// The MultiFab a condensed-Schur reconstruction should READ its per-level potential from (ADC-633).
  /// Identity on the uniform System (the field passed is the level-0 solution the emitted solve wrote);
  /// the AMR ProgramContext redirects the READ to the current level's published composite potential on a
  /// refined hierarchy. Trivial + inline so the uniform .so is byte-for-byte unchanged.
  MultiFab& schur_source(MultiFab& field, int /*role*/) const { return field; }

  /// Solve the matrix-free linear system A(phi) = rhs of a condensed-Schur Program (ADC-633). On the
  /// uniform System this dispatches by @p method to the SAME matrix-free Krylov call the codegen used to
  /// emit INLINE (cg / bicgstab / gmres / richardson) with byte-identical arguments, so a uniform
  /// Program's trajectory is unchanged; only the emission indirects through this seam. The AMR
  /// ProgramContext OVERRIDES this method to route a refined hierarchy through the composite FAC. @p
  /// method: 0 = cg, 1 = bicgstab, 2 = gmres, 3 = richardson (SchurSolveMethod). @p precond is the
