/// @file
/// @brief Internal authority for exact-ranked generated AMR Program execution.
...
class AmrProgramContext {
 public:
  ...
  /// Legacy/manual route for the hierarchy's default coarse-provider solve. Generated Programs
  /// use one of the exact-provider, evaluation-point-qualified field solve routes below.
  [[nodiscard]] SolveOutcome solve_default_field_on_coarse_level() const {
    if (active_level_ != 0)
      throw std::logic_error(
          "AmrProgramContext::solve_default_field_on_coarse_level is level-0-only; "
          "coarse-to-fine auxiliary injection is not a fine-level solve");
    refresh_resources_();
