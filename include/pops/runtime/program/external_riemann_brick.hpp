...
// extern "C" pops_brick_residual entry point.
template <class Model, class Flux>
...
class ExternalBrickHandle {
 public:
  ...
  ExternalBrickHandle(const std::string& so_path, const std::string& id) : id_(id) {
    ...
    residual_ = reinterpret_cast<ResidualFn>(dynlib::sym(handle_, "pops_brick_residual"));
    if (residual_ == nullptr)
      throw std::runtime_error("external riemann brick '" + id_ +
                               "' does not export pops_brick_residual(); rebuild the .so with "
...
// the flux is a STATIC template argument, never a per-cell string lookup. pops_brick_nvars /
