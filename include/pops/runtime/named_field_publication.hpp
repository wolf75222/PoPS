/// @file
/// @brief Dimension-generic publication of solved named fields into a compact field-owned carrier.
...
struct NamedFieldPublicationKernel {
  ...
  FieldView<Real, Dim> outputs;
  NamedFieldOutput<Dim> output;
  Real inverse_two_spacing[Dim]{};

  POPS_HD void operator()(const Index<Dim>& index) const {
    outputs(index, output.potential_component()) = potential(index, 0);
    ...
    for (int axis = 0; axis < Dim; ++axis) {
      ...
      --lower[axis];
      ++upper[axis];
      outputs(index, output.gradient_component(axis)) =
...
template <int Dim>
void publish_named_field(const MultiFab<Dim>& potential, MultiFab<Dim>& outputs,
  ...
  if (potential.layout() != outputs.layout() ||
      potential.distribution() != outputs.distribution() ||
      potential.local_rank() != outputs.local_rank())
    throw std::invalid_argument(
        "named elliptic potential and output carriers must share one exact layout");
  ...
  output.validate_width(outputs.ncomp(), "named elliptic field");
  if (output.has_gradients())
    for (int axis = 0; axis < Dim; ++axis)
  ...
  for (std::size_t local = 0; local < potential.local_size(); ++local) {
    detail::NamedFieldPublicationKernel<Dim> kernel{
        potential.fab(local).view(), outputs.fab(local).view(), output, {}};
    for (int axis = 0; axis < Dim; ++axis)
