...
inline PopsComponentStatusV1 unwritten_component_status() {
  return {sizeof(PopsComponentStatusV1), kUnwrittenComponentStatusCode, POPS_COMPONENT_ABORT_RUN_V1,
...
inline bool valid_precision(PopsPrecisionV1 value) {
  return value == POPS_PRECISION_FLOAT16_V1 || value == POPS_PRECISION_BFLOAT16_V1 ||
         value == POPS_PRECISION_FLOAT32_V1 || value == POPS_PRECISION_FLOAT64_V1;
}

inline void validate_execution_context(const PopsExecutionContextV1& context) {
  if (context.struct_size < sizeof(PopsExecutionContextV1) || context.context_version != 1 ||
      ...
      !component_text(context.backend_identity) || !component_text(context.device_identity) ||
      !component_text(context.stream_identity) || !component_text(context.communicator_identity) ||
...
inline void validate_logical_time(const PopsLogicalTimeV1& time) {
  if (time.struct_size < sizeof(PopsLogicalTimeV1) || !component_text(time.clock_identity) ||
      time.tick < 0 || time.level < 0 || time.substep < 0 || time.stage < 0 ||
      time.fraction_numerator < 0 || time.fraction_denominator <= 0 ||
...
inline bool empty_field_view(const PopsConstFieldViewV1& view) {
  if (view.struct_size != 0 || view.data != nullptr || view.dimension != 0 ||
      ...
      view.layout_identity != nullptr || view.patch_identity != nullptr || view.ownership != 0)
    return false;
  for (std::int32_t axis = 0; axis < 3; ++axis)
    if (view.extents[axis] != 0 || view.axis_strides[axis] != 0 || view.ghost_lower[axis] != 0 ||
...
inline void validate_boundary_region(const PopsBoundaryRegionV1& region) {
  if (region.struct_size < sizeof(PopsBoundaryRegionV1) || region.dimension < 1 ||
      region.dimension > 3 || region.codimension < 1 || region.codimension > region.dimension ||
      region.axis_count != static_cast<std::size_t>(region.codimension) || region.axes == nullptr ||
      region.sides == nullptr || !component_text(region.region_identity))
...
inline void validate_const_fields(const PopsQualifiedConstFieldV1* rows, std::size_t count,
                                  const char* where) {
...
inline void validate_optional_const_field(const PopsQualifiedConstFieldV1& row, const char* where) {
  if (row.struct_size < sizeof(PopsQualifiedConstFieldV1) || row.present > 1)
...
inline void validate_scalars(const PopsQualifiedScalarV1* rows, std::size_t count,
                             const char* where) {
  if (count != 0 && rows == nullptr)
    throw std::invalid_argument(std::string(where) + " table is null");
  std::unordered_set<std::string> identities;
...
inline int evaluate_faces(const PopsNumericalFluxApiV1& api, void* state,
  ...
  validate_execution_field(request.execution, result.normal_flux, "numerical flux output");
...
inline int evaluate_field_boundary(const PopsFieldBoundaryClosureApiV1& api, void* state,
  ...
  validate_execution_field(request.execution, request.coordinates, "field boundary coordinates");
  if (request.coordinates.component_count != static_cast<std::size_t>(request.region.dimension) ||
  ...
  validate_const_fields(request.directions, request.direction_count, "field boundary directions");
  ...
  validate_scalars(request.parameters, request.parameter_count, "field boundary parameters");
  validate_optional_const_field(request.rate, "field boundary rate");
  validate_optional_const_field(request.nonlinear_iterate, "field boundary nonlinear iterate");
  if (request.rate.present)
    validate_execution_field(request.execution, request.rate.values, "field boundary rate");
...
inline int tag_batch(const PopsTaggerApiV1& api, void* state, const PopsTaggerRequestV1& request,
  ...
  for (std::size_t index = 0; index < request.program.stencil_count; ++index) {
    ...
    if (stencil.struct_size < sizeof(PopsTaggingStencilV1) ||
        ...
        std::string_view(stencil.route) != POPS_TAGGING_STENCIL_ROUTE_LINEAR_AXIS_STENCIL_L2_V1 ||
        !component_text(stencil.norm) || std::string_view(stencil.norm) != "l2" ||
        !component_text(stencil.scale) || std::string_view(stencil.scale) != "inverse_cell_size" ||
    ...
    for (std::size_t axis_index = 0; axis_index < stencil.axis_count; ++axis_index) {
      ...
      if (axis.struct_size < sizeof(PopsTaggingAxisStencilV1) ||
          axis.axis != static_cast<std::int32_t>(axis_index) || axis.derivative_order != 1 ||
          axis.formal_order < 1 || axis.term_count == 0 ||
          static_cast<std::size_t>(axis.formal_order) > axis.term_count ||
          axis.term_count > POPS_TAGGING_MAXIMUM_STENCIL_TERMS_V1 || axis.offsets == nullptr ||
          axis.coefficients == nullptr)
      ...
      for (std::size_t term = 0; term < axis.term_count; ++term) {
        ...
        if (!std::isfinite(axis.coefficients[term]))
          throw std::invalid_argument("tagger graph axis stencil coefficient is not finite");
        ghost_lower = std::max(
            ghost_lower, static_cast<std::size_t>(std::max<std::int64_t>(0, -widened_offset)));
        ghost_upper = std::max(ghost_upper,
                               static_cast<std::size_t>(std::max<std::int64_t>(0, widened_offset)));
      }
...
inline int cluster_tags(const PopsClusteringApiV1& api, void* state,
                        const PopsClusteringRequestV1& request, PopsComponentStatusV1& status) {
  require_operation(api.cluster != nullptr, "cluster");
  validate_execution_context(request.execution);
  if (request.struct_size < sizeof(PopsClusteringRequestV1) ||
      request.tags.struct_size < sizeof(PopsConstByteViewV1) || request.tags.data == nullptr ||
      request.extents == nullptr || request.dimension < 1 || request.dimension > 3 ||
      request.boxes == nullptr || request.box_capacity == 0 || request.box_count == nullptr)
    throw std::invalid_argument("clustering request is incomplete");
  std::size_t points = 1;
  for (std::int32_t axis = 0; axis < request.dimension; ++axis) {
    if (request.extents[axis] <= 0 || static_cast<std::uint64_t>(request.extents[axis]) >
  ...
  *request.box_count = 0;
  ...
  if (*request.box_count > request.box_capacity)
    throw std::runtime_error("clustering component exceeded the output capacity supplied by PoPS");
  return code;
}
...
inline int apply_transfer(const PopsTransferApiV1& api, void* state,
  ...
  for (std::int32_t axis = 0; axis < request.dimension; ++axis) {
    ...
    const auto source_interior = request.source.extents[axis] - request.source.ghost_lower[axis] -
...
inline std::size_t writer_box_volume(const std::int64_t* lower, const std::int64_t* upper,
...
inline bool writer_boxes_overlap(const std::int64_t* left_lower, const std::int64_t* left_upper,
                                 const std::int64_t* right_lower, const std::int64_t* right_upper,
                                 std::int32_t dimension) {
  for (std::int32_t axis = 0; axis < dimension; ++axis)
...
inline bool writer_box_contains(const std::int64_t* outer_lower, const std::int64_t* outer_upper,
                                const std::int64_t* inner_lower, const std::int64_t* inner_upper,
                                std::int32_t dimension) {
  for (std::int32_t axis = 0; axis < dimension; ++axis)
...
inline void validate_writer_request(const PopsWriterRequestV1& request) {
  ...
  if (request.geometry_count == 0 || request.geometries == nullptr ||
      ...
      !component_text(request.metadata_json) || !component_text(request.selection_identity) ||
  ...
  for (std::size_t index = 0; index < request.geometry_count; ++index) {
    ...
    if (geometry.struct_size < sizeof(PopsWriterGeometryV1) ||
        ...
        geometry.cell_shape == nullptr || geometry.box_count == 0 || geometry.boxes == nullptr)
      throw std::invalid_argument("Writer geometry is incomplete");
    if (!geometries
             .emplace(writer_geometry_key(geometry.layout_identity, geometry.level), &geometry)
             .second)
      throw std::invalid_argument("Writer geometry identity/level is duplicated");
    validate_execution_field(request.execution, geometry.cell_volumes, "Writer cell volumes");
    ...
    for (std::int32_t axis = 0; axis < geometry.dimension; ++axis) {
      if (!std::isfinite(geometry.origin[axis]) || !std::isfinite(geometry.spacing[axis]) ||
    ...
    for (std::size_t box = 0; box < geometry.box_count; ++box) {
      ...
      if (bounds.struct_size < sizeof(PopsWriterBoxV1) || bounds.dimension != geometry.dimension ||
          bounds.lower == nullptr || bounds.upper == nullptr)
      ...
      for (std::size_t prior = 0; prior < box; ++prior)
        if (writer_boxes_overlap(bounds.lower, bounds.upper, geometry.boxes[prior].lower,
  ...
  for (std::size_t index = 0; index < request.field_count; ++index) {
    ...
    if (field.struct_size < sizeof(PopsWriterFieldV1) || !component_text(field.field_identity) ||
        !component_text(field.reference_id) || !component_text(field.component_manifest_identity) ||
        !component_text(field.layout_identity) || !component_text(field.state_id) ||
        !component_text(field.centering) || !component_text(field.units) || field.dimension < 1 ||
        field.dimension > 3 || field.global_shape == nullptr || field.piece_count == 0 ||
        field.pieces == nullptr)
      throw std::invalid_argument("Writer field is incomplete");
    const auto geometry_it =
        geometries.find(writer_geometry_key(field.layout_identity, field.level));
    ...
    for (std::size_t piece = 0; piece < field.piece_count; ++piece) {
      validate_execution_field(request.execution, field.pieces[piece].values, "Writer field piece");
      ...
      const auto expected_centering = values.centering == POPS_FIELD_CENTERING_CELL_V1   ? "cell"
                                      : values.centering == POPS_FIELD_CENTERING_FACE_V1 ? "face"
                                      : values.centering == POPS_FIELD_CENTERING_NODE_V1 ? "node"
                                                                                         : "edge";
      ...
      for (std::int32_t axis = 0; axis < field.dimension; ++axis)
        if (field.pieces[piece].lower[axis] < 0 ||
            ...
            field.pieces[piece].upper[axis] > static_cast<std::int64_t>(field.global_shape[axis]) ||
            values.extents[axis] != static_cast<std::size_t>(field.pieces[piece].upper[axis] -
                                                             field.pieces[piece].lower[axis]))
          throw std::invalid_argument("Writer field piece bounds disagree with its descriptor");
      bool contained = false;
      for (std::size_t box = 0; box < geometry.box_count; ++box)
        contained =
            contained || writer_box_contains(geometry.boxes[box].lower, geometry.boxes[box].upper,
                                             field.pieces[piece].lower, field.pieces[piece].upper,
                                             field.dimension);
      if (!contained)
      ...
      const auto volume =
          writer_box_volume(field.pieces[piece].lower, field.pieces[piece].upper, field.dimension);
    ...
    for (std::size_t box = 0; box < geometry.box_count; ++box) {
      const auto volume = writer_box_volume(geometry.boxes[box].lower, geometry.boxes[box].upper,
                                            geometry.dimension);
  ...
  for (std::size_t index = 0; index < request.diagnostic_count; ++index) {
    ...
    if (diagnostic.struct_size < sizeof(PopsWriterDiagnosticV1) ||
        ...
        !component_text(diagnostic.layout_identity) || !component_text(diagnostic.state_id) ||
        !component_text(diagnostic.reduction) || !component_text(diagnostic.units) ||
        !component_text(diagnostic.terms_json) || !std::isfinite(diagnostic.value))
      throw std::invalid_argument("Writer diagnostic is incomplete");
    if (geometries.find(writer_geometry_key(diagnostic.layout_identity, diagnostic.level)) ==
            geometries.end() ||
        qualified_states.find(writer_geometry_key(diagnostic.layout_identity, diagnostic.level) +
...
inline int publish_output(const PopsWriterApiV1& api, void* state,
                          const PopsWriterRequestV1& request, PopsWriterReceiptV1& receipt) {
  ...
  if (verified != 0)
    return verified;
  if (verification.status.code != 0 || verification.status.action != POPS_COMPONENT_CONTINUE_V1)
    throw std::runtime_error(verification.status.reason == nullptr
                                 ? "Writer verify rejected publication without a reason"
                                 : verification.status.reason);
  receipt = {sizeof(PopsWriterReceiptV1),
             0,
             nullptr,
             {sizeof(PopsComponentStatusV1), 0, POPS_COMPONENT_CONTINUE_V1, nullptr}};
...
struct FieldTopologyPatchInputV2 {
  ...
  PopsFieldMaterialRepresentationV1 material_representation = POPS_FIELD_MATERIAL_FULL_V1;
 ...
 public:
  struct OwnedPatchMetadata {
    ...
    [[nodiscard]] PopsFieldPatchMetadataV1 view() const {
      PopsFieldPatchMetadataV1 result{sizeof(PopsFieldPatchMetadataV1),
                                      global_patch_index,
                                      owner_rank,
                                      level,
                                      dimension,
  ...
  [[nodiscard]] const std::string& topology_digest() const { return state_->topology_digest; }
  [[nodiscard]] const std::shared_ptr<const ImmutableState>& shared_state() const { return state_; }
...
inline std::size_t field_patch_axis_extent(const PopsFieldPatchMetadataV1& patch,
  ...
  const std::uint64_t span = static_cast<std::uint64_t>(upper) - static_cast<std::uint64_t>(lower);
...
inline void validate_field_patch_metadata(const PopsFieldPatchMetadataV1& patch,
                                          std::size_t expected_index,
                                          const char* expected_layout = nullptr) {
  if (patch.struct_size < sizeof(PopsFieldPatchMetadataV1) ||
      patch.global_patch_index != expected_index || patch.owner_rank < 0 || patch.level < 0 ||
      patch.dimension < 1 || patch.dimension > 3 || !component_text(patch.layout_identity) ||
      !component_text(patch.patch_identity) ||
      (expected_layout != nullptr && std::string_view(patch.layout_identity) != expected_layout))
  ...
  for (std::int32_t axis = 0; axis < 3; ++axis) {
    if (axis < patch.dimension) {
      if (patch.upper[axis] < patch.lower[axis] || !std::isfinite(patch.physical_lower[axis]) ||
...
inline void validate_field_global_topology(const PopsFieldGlobalTopologyV1& topology) {
  if (topology.struct_size < sizeof(PopsFieldGlobalTopologyV1) ||
      ...
      !component_text(topology.materialized_layout_identity) || topology.dimension < 1 ||
      topology.dimension > 3 || topology.patch_count == 0 || topology.patches == nullptr)
...
inline void validate_field_view_matches_patch(const PopsConstFieldViewV1& view,
                                              const PopsFieldPatchMetadataV1& patch,
                                              const char* what) {
  if (view.dimension != patch.dimension || view.centering != patch.centering ||
      view.centering_axes != patch.centering_axes || !component_text(view.layout_identity) ||
...
inline void validate_field_view_matches_patch(const PopsFieldViewV1& view,
                                              const PopsFieldPatchMetadataV1& patch,
                                              const char* what) {
  PopsConstFieldViewV1 read_only{view.struct_size,
                                 view.data,
                                 view.dimension,
                                 {},
                                 {},
...
inline void validate_topology_material_input(const FieldTopologyPatchInputV2& input,
  ...
  if (has_ids && (input.material_ids.struct_size < sizeof(PopsConstInt32ViewV1) ||
  ...
  switch (input.material_representation) {
    ...
    case POPS_FIELD_MATERIAL_BINARY_COVERAGE_V1:
      if (!has_coverage || has_fraction || has_ids)
        throw std::invalid_argument("binary topology requires only exact material coverage");
      break;
    case POPS_FIELD_MATERIAL_CUT_CELL_FRACTION_V1:
      if (!has_fraction || has_coverage || has_ids)
        throw std::invalid_argument("cut-cell topology requires only volume fractions");
      break;
    case POPS_FIELD_MATERIAL_IDS_V1:
      if (!has_ids || has_coverage || has_fraction)
        throw std::invalid_argument("multi-material topology requires only material ids");
      break;
  ...
  switch (input.material_representation) {
    ...
    case POPS_FIELD_MATERIAL_BINARY_COVERAGE_V1:
      for (std::size_t point = 0; point < points; ++point)
        expected[point] =
            require_binary(input.material_coverage.data[point], "binary material coverage");
      break;
    case POPS_FIELD_MATERIAL_CUT_CELL_FRACTION_V1: {
    ...
    case POPS_FIELD_MATERIAL_IDS_WITH_CUT_CELL_FRACTION_V1: {
      ...
      for (std::size_t point = 0; point < points; ++point) {
        ...
        if (id < 0 || (id == 0 && fraction != 0.0))
          throw std::invalid_argument("material ids and cut-cell fractions are inconsistent");
...
inline PreparedFieldTopologyV2 prepare_field_topology(
  ...
  storage->global_topology = {sizeof(PopsFieldGlobalTopologyV1),
                              storage->topology_recipe_identity.c_str(),
                              storage->source_layout_identity.c_str(),
                              storage->materialized_layout_identity.c_str(),
                              storage->dimension,
                              {},
                              {},
                              storage->periodic_axes,
                              storage->global_patches.size(),
  ...
  for (const auto& input : local_patches) {
    ...
    request_patches.push_back(
        {sizeof(PopsFieldTopologyPatchV2),
         input.metadata_index,
         input.material_representation,
         input.material_coverage,
         input.cut_cell_volume_fraction,
         input.material_ids,
         {sizeof(PopsByteViewV1), retained.material_mask.data(), retained.material_mask.size()},
  ...
  const PopsFieldTopologyRequestV2 request{
      sizeof(PopsFieldTopologyRequestV2), storage->global_topology, request_patches.size(),
      request_patches.empty() ? nullptr : request_patches.data(), execution};
  ...
  if (result.struct_size < sizeof(PopsFieldTopologyResultV2) ||
      !component_status_is_well_formed(result.status) || code != 0 || result.status.code != 0 ||
      result.status.action != POPS_COMPONENT_CONTINUE_V1)
    throw std::runtime_error(result.status.reason == nullptr ? "field topology preparation failed"
                                                             : result.status.reason);
  if (result.topology_digest == nullptr || *result.topology_digest == '\0' ||
      result.provenance == nullptr || *result.provenance == '\0' || result.labels == nullptr ||
  ...
  for (std::size_t index = 0; index < result.label_count; ++index) {
    ...
    if (label.struct_size < sizeof(PopsTopologyLabelV2) || label.id <= 0 ||
        label.label == nullptr || *label.label == '\0' || label.provenance == nullptr ||
        *label.provenance == '\0' || !vocabulary.insert(label.id).second)
      throw std::runtime_error(
          "field topology label vocabulary must be positive, unique and fully attributed");
    storage->labels.push_back({label.id, std::string(label.label), std::string(label.provenance)});
  }
  for (const auto& local : storage->local_patches) {
    for (std::size_t index = 0; index < local.material_mask.size(); ++index) {
      ...
      if (active == std::uint8_t{0xff} || label == std::numeric_limits<std::int32_t>::min())
        throw std::runtime_error(
...
class TopologyBoundFieldSolverRequestV2 final {
 ...
 private:
  friend TopologyBoundFieldSolverRequestV2 bind_field_solver_request(
      const PreparedFieldTopologyV2&, const std::vector<FieldSolverPatchBindingV2>&,
...
inline TopologyBoundFieldSolverRequestV2 bind_field_solver_request(
    ...
    const PopsExecutionContextV1& execution, const char* boundary_contract_json = nullptr,
    double relative_tolerance = 0.0, double absolute_tolerance = 0.0,
  ...
  if (!std::isfinite(relative_tolerance) || relative_tolerance < 0.0 ||
      !std::isfinite(absolute_tolerance) || absolute_tolerance < 0.0 || max_iterations <= 0 ||
      !component_text(boundary_contract_json) ||
      std::string_view(boundary_contract_json).find("\"identity\"") == std::string_view::npos)
    throw std::invalid_argument(
  ...
  bound.owned_topology_labels_ = std::make_shared<std::vector<OwnedFieldSolverTopologyLabelV2>>();
  bound.owned_topology_labels_->reserve(topology.labels().size());
  for (const auto& label : topology.labels())
    bound.owned_topology_labels_->push_back({label.id, label.label, label.provenance});
  bound.topology_labels_ = std::make_shared<std::vector<PopsFieldSolverTopologyLabelV2>>();
  ...
  bound.material_masks_ = std::make_shared<std::vector<std::vector<std::uint8_t>>>();
  bound.component_labels_ = std::make_shared<std::vector<std::vector<std::int32_t>>>();
  ...
  for (std::size_t index = 0; index < patch_bindings.size(); ++index) {
    ...
    if (binding.metadata_index != prepared.metadata_index ||
      ...
      throw std::invalid_argument("field solver local patches differ from prepared topology order");
    ...
    validate_field_view_matches_patch(binding.solution, metadata, "field solver solution");
    if (!same_field_domain(binding.rhs, binding.solution) || binding.rhs.component_count != 1 ||
        binding.solution.component_count != 1 ||
    ...
    if (!empty_field_view(binding.coefficients)) {
      validate_execution_field(execution, binding.coefficients, "field solver coefficients");
      validate_field_view_matches_patch(binding.coefficients, metadata,
                                        "field solver coefficients");
      if (field_interior_point_count(binding.coefficients) != prepared.material_mask.size())
        throw std::invalid_argument(
            "field solver coefficients do not match their prepared patch interior");
    }
    patches.push_back({sizeof(PopsFieldSolverPatchV2),
                       binding.metadata_index,
                       binding.rhs,
                       binding.solution,
  ...
  bound.patches_ = std::make_shared<std::vector<PopsFieldSolverPatchV2>>(std::move(patches));
  bound.request_ = {sizeof(PopsFieldSolverRequestV2),
                    ...
                    bound.topology_provenance_->c_str(),
                    bound.topology_digest_->c_str(),
...
inline int solve_field(const PopsFieldSolverApiV2& api, void* state,
                       const TopologyBoundFieldSolverRequestV2& bound, PopsSolveReportV2& report) {
  ...
  const auto validate_request_authority = [&]() {
    ...
    if (bound.boundary_contract_ == nullptr || bound.topology_provenance_ == nullptr ||
        ...
        request.topology.topology_recipe_identity != topology.topology_recipe_identity.c_str() ||
    ...
    if (request.local_patch_count != bound.patches_->size() ||
        ...
        request.local_patches != (bound.patches_->empty() ? nullptr : bound.patches_->data()) ||
    ...
    for (std::size_t index = 0; index < request.topology_label_count; ++index) {
      ...
      if (label.struct_size < sizeof(PopsFieldSolverTopologyLabelV2) || label.id != owned.id ||
          label.label != owned.label.c_str() || label.provenance != owned.provenance.c_str() ||
          owned.id != prepared.id || owned.label != prepared.label ||
          owned.provenance != prepared.provenance)
        throw std::invalid_argument(
    ...
    for (std::size_t index = 0; index < request.local_patch_count; ++index) {
      ...
      if (patch.struct_size < sizeof(PopsFieldSolverPatchV2) ||
          ...
          material_mask != prepared.material_mask || component_labels != prepared.component_labels)
        throw std::invalid_argument(
  ...
  if (code != 0)
    throw std::runtime_error("field solver transport failed with code " + std::to_string(code));
  try {
    validate_request_authority();
  } catch (const std::invalid_argument&) {
    throw std::runtime_error("field solver mutated its authenticated topological request");
  }
  ...
  const double ratio_scale =
      std::max({1.0, std::abs(report.relative_residual), std::abs(expected_relative_residual)});
  const bool ratio_is_coherent = std::isfinite(expected_relative_residual) &&
                                 std::abs(report.relative_residual - expected_relative_residual) <=
                                     64.0 * std::numeric_limits<double>::epsilon() * ratio_scale;
  const double relative_threshold = request.relative_tolerance * report.reference_residual_norm;
  const double convergence_threshold = std::max(request.absolute_tolerance, relative_threshold);
  const bool solved = report.status == POPS_SOLVE_SOLVED_V2;
  if (report.struct_size < sizeof(PopsSolveReportV2) || !solve_status_is_known(report.status) ||
      !solve_action_is_known(report.action) || !component_text(report.reason) ||
      report.iterations < 0 || report.iterations > request.max_iterations ||
      !std::isfinite(report.reference_residual_norm) || report.reference_residual_norm < 0.0 ||
      ...
      !ratio_is_coherent || !std::isfinite(relative_threshold) ||
      !std::isfinite(convergence_threshold) ||
