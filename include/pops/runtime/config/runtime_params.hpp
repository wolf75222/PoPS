...
#include <pops/core/foundation/types.hpp>  // Real, POPS_HD (device-callable)

#include <cstddef>

/// @file
/// @brief RuntimeParams: carrier for the RUNTIME PARAMETERS of a DSL model (P7-b). A runtime parameter
///        is declared on the Python side via pops.dsl.Param(..., kind="runtime"); its value can be CHANGED
...
///
/// MECHANICS. The codegen (python/pops/dsl.py) assigns each runtime parameter a STABLE INDEX
...
struct RuntimeParams {
  ...
  POPS_HD Real get(int k) const { return values[k]; }
};
