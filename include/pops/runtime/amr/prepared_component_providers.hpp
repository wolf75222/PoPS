...
void validate_tagging_stencil_program(
    const PreparedTaggingProgram& program,
    const std::vector<std::string>& supported_routes,
    std::size_t maximum_stencil_terms, std::int32_t runtime_dimension,
  ...
  for (std::size_t stencil_index = 0; stencil_index < program.stencils.size();
       ++stencil_index) {
    ...
    if (!referenced[stencil_index] || stencil.identity.empty() ||
        ...
        stencil.dimension != runtime_dimension ||
    ...
    for (std::size_t axis_index = 0; axis_index < stencil.axes.size(); ++axis_index) {
      ...
      if (axis.axis != static_cast<std::int32_t>(axis_index) ||
          axis.derivative_order != 1 || axis.formal_order < 1 || axis.offsets.empty() ||
      ...
      for (std::size_t term = 0; term < axis.offsets.size(); ++term) {
        ...
        if (!std::isfinite(coefficient))
          throw std::invalid_argument(
              "AMR Tagger axis stencil has a non-finite coefficient");
        lower = std::max(
            lower, static_cast<std::size_t>(std::max<std::int64_t>(0, -widened_offset)));
        upper = std::max(
            upper, static_cast<std::size_t>(std::max<std::int64_t>(0, widened_offset)));
      }
      if (lower != axis.ghost_lower || upper != axis.ghost_upper)
        throw std::invalid_argument(
            "AMR Tagger axis stencil has inconsistent halos");
      for (std::int32_t power = 0; power <= axis.formal_order; ++power) {
        ...
        if (std::abs(moment - expected) > tolerance)
          throw std::invalid_argument(
              "AMR Tagger axis stencil falsely declares its formal order");
      }
    }
  }
  for (const auto& leaf : program.leaves)
    if (leaf.stencil_index != POPS_TAGGING_NO_STENCIL_V1) {
      ...
      for (const auto& axis : program.stencils[leaf.stencil_index].axes)
        if (axis.ghost_lower > available || axis.ghost_upper > available)
          throw std::invalid_argument(
              "AMR Tagger field halo is thinner than its resolved stencil");
    }
}
...
class PreparedTaggerComponent final {
 public:
  PreparedTaggerComponent(PreparedTaggerSpec spec,
    ...
    state_ = component_->prepared_state(
        POPS_NATIVE_INTERFACE_TAGGER_V1, spec_.interface_version,
  ...
  PreparedTaggerCandidates tag(
      const std::vector<PreparedTaggingField>& fields,
      const PreparedTaggingProgram& program, const Box2D& domain, int level,
      std::int64_t tick, double physical_time, double dx, double dy,
      bool periodic_x, bool periodic_y, bool parent_replicated) const {
    static_assert(sizeof(Real) == sizeof(double),
                  "Tagger ABI v1 requires the binary64 PoPS backend");
    PreparedTaggerCandidates result{
        TagBox(domain), TagBox(domain), TagBox(domain), TagBox(domain)};
    ...
    try {
      if (domain.empty() || fields.empty() || !program.prepared ||
          ...
          dx <= 0.0 || dy <= 0.0)
        throw std::invalid_argument(
            "external AMR Tagger received an incomplete graph evaluation");
    ...
    std::vector<std::vector<PopsTaggingAxisStencilV1>> abi_axes(
        program.stencils.size());
    ...
    for (std::size_t index = 0; index < program.stencils.size(); ++index) {
      ...
      for (const auto& axis : stencil.axes)
        axes.push_back(PopsTaggingAxisStencilV1{
            sizeof(PopsTaggingAxisStencilV1), axis.axis, axis.derivative_order,
            axis.formal_order, axis.ghost_lower, axis.ghost_upper,
            axis.offsets.size(), axis.offsets.data(), axis.coefficients.data()});
      stencils.push_back(PopsTaggingStencilV1{
          sizeof(PopsTaggingStencilV1), stencil.identity.c_str(),
          stencil.route.c_str(), stencil.norm.c_str(), stencil.scale.c_str(),
          stencil.boundary_mode.c_str(), stencil.dimension, axes.size(), axes.data()});
    }
    std::vector<PopsTaggingLeafV1> leaves;
    leaves.reserve(program.leaves.size());
    for (const auto& leaf : program.leaves)
      leaves.push_back(PopsTaggingLeafV1{
          sizeof(PopsTaggingLeafV1), leaf.state_index, leaf.component,
          leaf.opcode, leaf.threshold, leaf.stencil_index});
    const PopsTaggingProgramV1 abi_program{
        sizeof(PopsTaggingProgramV1), program.provider_identity.c_str(),
        stencils.size(), stencils.data(),
        leaves.size(), leaves.data(), program.refine_ops.size(),
        program.refine_ops.data(), program.refine_args.data(),
        program.coarsen_ops.size(), program.coarsen_ops.data(),
        program.coarsen_args.data(), program.min_cycles,
        program.equality_policy, program.conflict_policy,
        program.non_finite_policy};
    const auto& api = component_->table<PopsTaggerApiV1>(
        POPS_NATIVE_INTERFACE_TAGGER_V1, spec_.interface_version);
    ...
    for (int local = 0; local < reference.local_size(); ++local) {
      ...
      for (const auto& field : fields) {
        ...
        patch_identities.push_back(
            field.qualified_identity + "@level=" + std::to_string(level) +
            "/patch=" + std::to_string(global));
        const std::size_t ghosts = static_cast<std::size_t>(field.values->n_grow());
        const PopsConstFieldViewV1 view{
            sizeof(PopsConstFieldViewV1), values.p, 2,
            ...
            POPS_FIELD_CENTERING_CELL_V1, 0, {ghosts, ghosts, 0},
            {ghosts, ghosts, 0}, POPS_SCALAR_FLOAT64_V1,
            POPS_MEMORY_SPACE_HOST_V1, spec_.layout_identity.c_str(),
            patch_identities.back().c_str(), POPS_FIELD_OWNERSHIP_RUNTIME_BORROWED_V1};
        states.push_back(PopsQualifiedConstFieldV1{
            sizeof(PopsQualifiedConstFieldV1), 1, field.qualified_identity.c_str(), view});
      }
      ...
      const PopsLogicalTimeV1 logical_time{
          sizeof(PopsLogicalTimeV1), spec_.clock_identity.c_str(), tick, level,
          0, 0, 0, 1, 0.0, physical_time};
      const PopsTaggerRequestV1 request{
          sizeof(PopsTaggerRequestV1), states.size(), states.data(), abi_program,
          {valid.lo[0], valid.lo[1], 0}, {domain.lo[0], domain.lo[1], 0},
          {domain.hi[0], domain.hi[1], 0}, {dx, dy, 0.0},
          ...
          logical_time, spec_.execution->view()};
      ...
      if (!component::component_status_is_well_formed(status) || code != 0 ||
          status.code != 0 || status.action != POPS_COMPONENT_CONTINUE_V1)
        throw std::runtime_error(
            status.reason == nullptr ? "native AMR Tagger failed" : status.reason);
      std::array<TagBox*, 4> outputs{
          &result.refine, &result.coarsen,
          &result.refine_equalities, &result.coarsen_equalities};
      for (std::size_t output = 0; output < masks.size(); ++output) {
        for (std::size_t point = 0; point < local_points; ++point) {
          if (masks[output][point] > 1u)
            throw std::runtime_error(
                "native AMR Tagger did not write every candidate or returned a non-Boolean value");
    ...
    const std::array<TagBox*, 4> outputs{
        &result.refine, &result.coarsen,
        &result.refine_equalities, &result.coarsen_equalities};
    if (outputs.front()->t.size() >
        std::numeric_limits<std::size_t>::max() / outputs.size())
 ...
 private:
  static void validate_finite_samples_(
      const std::vector<PreparedTaggingField>& fields,
      const PreparedTaggingProgram& program, int local, const Box2D& valid) {
    for (const auto& leaf : program.leaves) {
        ...
        for (int i = valid.lo[0]; i <= valid.hi[0]; ++i) {
          if (!gradient) {
            if (!std::isfinite(static_cast<double>(
                    values(i, j, static_cast<int>(leaf.component)))))
              throw std::runtime_error(
          ...
          for (const auto& axis : stencil.axes)
            for (const int offset : axis.offsets) {
              ...
              if (!std::isfinite(static_cast<double>(
                      values(x, y, static_cast<int>(leaf.component)))))
                throw std::runtime_error(
  ...
  void validate_program_(const PreparedTaggingProgram& program,
    ...
    const auto supported = [](std::int32_t opcode,
    ...
    if (program.non_finite_policy != spec_.non_finite_policy ||
        ...
        program.clock_identity != spec_.clock_identity ||
        program.leaves.empty() || program.refine_ops.empty() ||
        ...
        program.refine_ops.size() + program.coarsen_ops.size() >
            spec_.maximum_instruction_count)
    ...
    for (const auto& opcodes : {&program.refine_ops, &program.coarsen_ops})
      for (const std::int32_t opcode : *opcodes)
        if (!((pops_tagging_opcode_is_leaf_v1(opcode) &&
               supported(opcode, spec_.leaf_opcodes)) ||
  ...
  void validate_() const {
    ...
    if (std::adjacent_find(stencil_routes.begin(), stencil_routes.end()) !=
            stencil_routes.end() ||
        std::any_of(stencil_routes.begin(), stencil_routes.end(),
                    [](const std::string& route) {
                      return route !=
                          POPS_TAGGING_STENCIL_ROUTE_LINEAR_AXIS_STENCIL_L2_V1;
                    }))
      throw std::invalid_argument(
          "prepared AMR Tagger declares an unsupported stencil route");
    ...
    component::require_operation(
        component_->table<PopsTaggerApiV1>(POPS_NATIVE_INTERFACE_TAGGER_V1,
...
class PreparedClusteringComponent final : public pops::amr::ClusteringProvider {
 public:
  PreparedClusteringComponent(PreparedClusteringSpec spec,
    ...
    validate_();
    state_ = component_->prepared_state(
        POPS_NATIVE_INTERFACE_CLUSTERING_V1, spec_.interface_version,
        spec_.execution->view());
  }
  ...
  std::vector<Box2D> cluster(const TagBox& tags) const override {
    ...
    try {
      ...
      raw.assign(tagged * 2u * dimension,
                 std::numeric_limits<std::int64_t>::min());
      const PopsClusteringRequestV1 request{
          sizeof(PopsClusteringRequestV1),
          {sizeof(PopsConstByteViewV1), mask.data(), mask.size()}, extents,
          dimension, raw.data(), tagged, &count, spec_.execution->view()};
      PopsComponentStatusV1 status = component::unwritten_component_status();
      const auto& api = component_->table<PopsClusteringApiV1>(
          POPS_NATIVE_INTERFACE_CLUSTERING_V1, spec_.interface_version);
      const int code = component::cluster_tags(api, state_, request, status);
      if (!component::component_status_is_well_formed(status) || code != 0 ||
          status.code != 0 ||
          status.action != POPS_COMPONENT_CONTINUE_V1)
        local_failure = status.reason == nullptr
                            ? "native AMR Clustering component failed"
                            : status.reason;
    } catch (const std::exception& error) {
      local_failure = error.what();
    } catch (...) {
    ...
    if (all_reduce_sum(local_failure.empty() ? 0L : 1L) != 0)
      throw std::runtime_error(
          n_ranks() == 1 ? local_failure
                         : "native AMR Clustering failed on at least one rank");
    ...
    try {
      ...
      for (std::size_t index = 0; index < count; ++index) {
        ...
        if (row[0] < 0 || row[1] < 0 || row[2] < row[0] || row[3] < row[1] ||
            ...
            row[0] > std::numeric_limits<int>::max() ||
            row[1] > std::numeric_limits<int>::max() ||
            row[2] > std::numeric_limits<int>::max() ||
            row[3] > std::numeric_limits<int>::max())
          throw std::runtime_error(
              "native AMR Clustering returned an out-of-region or invalid box");
        result.push_back(
            Box2D{{static_cast<int>(row[0]) + tags.box.lo[0],
                   static_cast<int>(row[1]) + tags.box.lo[1]},
      ...
      std::sort(result.begin(), result.end(),
                [](const Box2D& left, const Box2D& right) {
        ...
        for (int j = box.lo[1]; j <= box.hi[1]; ++j)
          for (int i = box.lo[0]; i <= box.hi[0]; ++i) {
            const std::size_t point =
                static_cast<std::size_t>(j - tags.box.lo[1]) * nx +
    ...
    if (all_reduce_min(local_count) != all_reduce_max(local_count))
      throw std::runtime_error(
          "native AMR Clustering returned a different box count across ranks");
    ...
    if (minimum != maximum)
      throw std::runtime_error(
          "native AMR Clustering returned different boxes across ranks");
    return result;
  }
 ...
 private:
  void validate_() const {
    ...
    component::require_operation(
        component_->table<PopsClusteringApiV1>(POPS_NATIVE_INTERFACE_CLUSTERING_V1,
                                               spec_.interface_version)
                .cluster != nullptr,
        "cluster");
  }
