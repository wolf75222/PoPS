...
std::string make_exact_amr_field_solver_contract(
    ...
    const ExactAmrFieldSolverBuildRequest<Dim>& request) {
...
class ExactAmrFieldSolverProvider {
 public:
  ...
  virtual std::uint64_t interface_version() const noexcept { return 2; }
...
class ExactAmrFieldSolverRegistry {
 public:
  ...
  void add(std::shared_ptr<const provider_type> provider) {
    if (!provider || provider->identity().empty() || provider->collective_contract().empty())
