...
class InterfaceFluxScheduler {
 ...
 public:
  ...
  void install(AxisAlignedInterface route, MultiFab& left_state, const Geometry& left_geometry,
    ...
    // MultiFab/DistributionMapping owners still use the process-world rank space. Keep that
    // authority only for admission; numerical collectives use the communicator authenticated by
    // the ExecutionContext and retained by the prepared route.
  ...
  void apply(const BoundaryEvaluationPoint& point, const std::vector<MultiFab*>& states,
    ...
    if (collective) {
      const long minimum_publication =
          all_reduce_min(publication != nullptr ? 1L : 0L, execution_communicator);
      const long maximum_publication =
          all_reduce_max(publication != nullptr ? 1L : 0L, execution_communicator);
    ...
    if (collective && publication != nullptr) {
  ...
  InterfaceFluxScheduler rematerialized(
    ...
    const CommunicatorView execution_communicator =
    ...
    finish_collective_preflight_(execution_communicator, allocation_failure,
    ...
    for (const PreparedInterface& prepared : interfaces_) {
      ...
      finish_collective_preflight_(execution_communicator, structural_failure,
      ...
      finish_collective_preflight_(execution_communicator, storage_failure,
    ...
    finish_collective_preflight_(execution_communicator, structural_registry_failure,
                                 "replacement registry structural completeness");
    if (collective && !candidate.registry_agrees_across_ranks_(execution_communicator))
      throw std::runtime_error(
 ...
 private:
  ...
  static PreparedInterface rematerialize_prepared_(const PreparedInterface& prepared,
    ...
    if (prepared.distributed && (!prepared.communicator.active() ||
      ...
      throw std::runtime_error(
          "multi-block interface execution communicator changed before hierarchy "
          "rematerialization");
    ...
    replacement.left_cells = boundary_cells_(left_state, route.left_axis, route.left_side,
                                             left_faces, prepared.communicator_rank);
    replacement.right_cells = boundary_cells_(right_state, route.right_axis, route.right_side,
                                              right_faces, prepared.communicator_rank);
    ...
    replacement.collective_identity =
        collective_plan_identity_(route, left_state, left_geometry, right_state, right_geometry,
                                  left_normal, right_normal, left_faces, prepared.component_count,
                                  prepared.communicator_identity, prepared.communicator_size);
    const std::size_t packed_size =
