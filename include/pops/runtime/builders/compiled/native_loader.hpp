...
/// MUSCL limited slope of a cell from backward difference @p am and forward @p ap. POPS_HD
...
POPS_HD inline double limited_slope(double am, double ap, int recon) {
...
/// Host residual R = -div F* + S(U, aux) (Rusanov, GLOBAL a_max like pops.PythonFlux, periodic) computed
...
/// shared_ptr owns the model: it calls pops_destroy_model then closes the .so on destruction.
...
void push_dynamic(ImplT* P, const std::string& name, pops::dynlib::handle h, int substeps,
                  std::vector<std::string> names, int recon) {
  auto mk = reinterpret_cast<void* (*)()>(pops::dynlib::sym(h, "pops_make_model"));
  auto del = reinterpret_cast<void (*)(void*)>(pops::dynlib::sym(h, "pops_destroy_model"));
  if (!mk || !del) {
    pops::dynlib::close(h);
    throw std::runtime_error(
        "add_dynamic_block: pops_make_model / pops_destroy_model missing from the .so");
  }
  ...
  std::function<Real(const MultiFab&)> max_speed = [P, im, n](const MultiFab& U) -> Real {
    ...
    for (std::size_t c0 = 0; c0 < nn; ++c0) {
      ...
      if (aux.size() >= 3 * nn) {
        ...
        // Extra fields: same SINGLE SOURCE POPS_AUX_FIELDS as load_aux and the other marshaling
...
void add_dynamic_block(System* self, ImplT* P, const std::string& name, const std::string& so_path,
  ...
  if (!nv_fn) {
    pops::dynlib::close(h);
    throw std::runtime_error("add_dynamic_block: pops_model_nvars missing from the .so");
  }
  const int nv = nv_fn();
  switch (nv) {
    case 1:
...
void add_compiled_block(System* self, ImplT* P, const std::string& name, const std::string& so_path,
  ...
  auto nv_fn = reinterpret_cast<nv_fn_t>(pops::dynlib::sym(h, "pops_model_nvars"));
  auto res_fn = reinterpret_cast<res_fn_t>(pops::dynlib::sym(h, "pops_compiled_residual"));
  auto adv_fn = reinterpret_cast<adv_fn_t>(pops::dynlib::sym(h, "pops_compiled_advance"));
  auto max_fn = reinterpret_cast<max_fn_t>(pops::dynlib::sym(h, "pops_compiled_max_speed"));
  auto poi_fn = reinterpret_cast<poi_fn_t>(pops::dynlib::sym(h, "pops_compiled_poisson_rhs"));
  ...
  // to the declaration defaults (pops_compiled_param_defaults) so a later set_param overwrites
  ...
  auto res_p_fn = reinterpret_cast<res_p_fn_t>(pops::dynlib::sym(h, "pops_compiled_residual_p"));
  ...
  std::shared_ptr<void> lib(h, [](void* p) {  // closes the .so when the closures die
    pops::dynlib::close(static_cast<pops::dynlib::handle>(p));
  });
  const int n = P->cfg.n;
  ...
  // (pops_compiled_has_projection / pops_compiled_project_p). Un .so genere avant ce chantier ne les
  ...
  auto has_proj_fn = reinterpret_cast<nv_fn_t>(pops::dynlib::sym(h, "pops_compiled_has_projection"));
  if (has_proj_fn && has_proj_fn() != 0) {
    using proj_fn_t = void (*)(double*, const double*, int, const double*, int);
    auto proj_fn = reinterpret_cast<proj_fn_t>(pops::dynlib::sym(h, "pops_compiled_project_p"));
    // Pas de close(h) ici : le shared_ptr `lib` possede deja le handle (fermeture a sa destruction).
    if (!proj_fn)
      throw std::runtime_error(
          "add_compiled_block : pops_compiled_has_projection() == 1 mais pops_compiled_project_p "
...
void add_native_block(System* self, ImplT* P, const std::string& name, const std::string& so_path,
  ...
  if (time != "explicit" && time != "ssprk3" && time != "euler" && time != "imex")
    throw std::runtime_error(
        ...
        "a composite model pops.Model(...) -> native add_block)");
  ...
  // thus calls out-of-line methods of pops::System DEFINED in THIS module; they must be
  // resolved through the dlopen against the already-loaded _pops module.
  // ELF PORTABILITY (Linux): CPython loads _pops in RTLD_LOCAL, so its symbols are NOT in
  ...
  // (pops::abi_key). On macOS it is harmless (the loader already resolves via dynamic_lookup).
#if defined(_WIN32)
  // Windows (ADC-100): NO RTLD_GLOBAL. The generated .dll is linked at COMPILATION against the
  // import libraries: _pops.lib (System symbols POPS_EXPORT: install_block/grid_context/...) and
  // kokkoscore.lib (Kokkos runtime SHARED as a DLL). Its undefineds are thus resolved by the OS loader
  // against _pops.pyd and kokkos*.dll already loaded -- no need to promote a global scope. We
  // simply load the .dll and resolve pops_install_native.
  pops::dynlib::handle h = pops::dynlib::open(so_path);
  if (!h)
    throw std::runtime_error(
        "add_native_block: LoadLibrary('" + so_path + "'): " + pops::dynlib::last_error() +
        " (the .dll must be linked against _pops.lib + kokkoscore.lib; cf. ADC-100)");
  {
    auto key_fn = reinterpret_cast<const char* (*)()>(pops::dynlib::sym(h, "pops_native_abi_key"));
    if (!key_fn) {
      pops::dynlib::close(h);
      throw std::runtime_error("add_native_block: pops_native_abi_key missing from the .dll");
    }
    ...
    if (!install) {
      pops::dynlib::close(h);
      throw std::runtime_error("add_native_block: pops_install_native missing from the .dll");
    }
  ...
  // its undefineds (the exported System methods POPS_EXPORT) against the already-loaded images. RTLD_NOW:
  // immediate resolution -> a missing System symbol (module without POPS_EXPORT) fails HERE, not in flight.
  void* h = dlopen(so_path.c_str(), RTLD_NOW | RTLD_GLOBAL);
  if (!h) {
    const char* e = dlerror();
    throw std::runtime_error("add_native_block: dlopen('" + so_path +
                             "'): " + std::string(e ? e : "?") +
                             " (the pops::System symbols (install_block/grid_context/"
                             "ensure_aux_width) must be exported AND the _pops module loaded "
  ...
  if (!key_fn) {
    pops::dynlib::close(h);
    throw std::runtime_error(
        "add_native_block: pops_native_abi_key missing from the .so (regenerate via "
  ...
  if (!install) {
    pops::dynlib::close(h);
    throw std::runtime_error(
        "add_native_block: pops_install_native missing from the .so (regenerate via "
        "dsl.compile_native / compile(backend='production'))");
  }
  // NB signature: pops_install_native now carries pos_floor (positivity limiter,
  // ADC-76) as the final flat argument. A loader generated BEFORE this addition has a different C
  // signature -- it is REJECTED upstream by the ABI key (POPS_HEADER_SIG changes with the headers), never
