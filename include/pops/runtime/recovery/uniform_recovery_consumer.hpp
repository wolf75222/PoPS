...
/// Keep the old explicit recovery-plan test seam while routing production model recovery through
/// the typed inversion authority.  Both return a detached outcome; only the latter carries the
/// observable result of an explicitly scheduled projection.
template <int N, class Plan>
PreparedVariableRecoveryAttempt<N> execute_uniform_recovery(Plan& plan, const Real (&conserved)[N],
                                                            const Real (&initial_guess)[N]) {
  if constexpr (requires { plan.recover(conserved); }) {
    return plan.recover(conserved);
  } else if constexpr (requires { plan->recover(conserved); }) {
    return plan->recover(conserved);
  } else {
...
/// Existing explicit-plan entry point retained for the separately owned AMR and compact native
/// plan seams.  It is intentionally not selected by generated Uniform materialization.
