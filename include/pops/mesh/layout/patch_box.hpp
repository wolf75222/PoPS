...
/// uses the exact per-axis cells, lengths and lower bounds on the Python side.
///
/// Invariants:
/// - level 0 = coarse, level >= 1 = fine (ratio 2 per level on each Cartesian axis);
...
/// box in the index space of the level (each base-axis count shifted by ``level``).
...
/// coordinates happens on the Python side from exact axis bounds: ``dx = Lx/(nx << level)`` and
/// ``dy = Ly/(ny << level)``, including non-zero physical origins.
struct PatchBox {
  int level;
  int ilo;
  int jlo;
