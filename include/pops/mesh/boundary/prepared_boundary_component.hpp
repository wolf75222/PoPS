 ...
 public:
    ...
    requires(Operation == PreparedBoundaryOperation::GhostRegion)
  {
    ...
    for (std::size_t local = 0; local < state.local_size(); ++local) {
      ...
      const std::string patch_identity = spec_.region.identity + "::patch:" + std::to_string(local);
    ...
    for (auto& publication : publications) {
      for (std::size_t value = 0; value < publication.values.size(); ++value)
        publication.host(publication.offsets[value]) = static_cast<Real>(publication.values[value]);
