 ...
 public:
  ...
  MultiFab& output(const std::string& identity) const {
    ...
    if (found == outputs_.end())
      throw std::invalid_argument(
          "boundary component has no exact mutable output binding for '" + identity + "'");
    return *found->second;
  }
 ...
 private:
  ...
  static void bind_const_(ConstTable& table, std::string identity, const MultiFab& field,
    ...
    if (identity.empty() || table.count(identity) != 0)
      throw std::invalid_argument(
          std::string("boundary ") + role + " identity is empty or multiply bound");
  ...
  static const MultiFab& require_const_(const ConstTable& table, const std::string& identity,
    ...
    if (found == table.end())
      throw std::invalid_argument(
          std::string("boundary component has no exact ") + role + " binding for '" +
          identity + "'");
    return *found->second;
  }
};
...
inline bool region_coordinate_matches(int coordinate, int axis, int side, const Box2D& domain,
  ...
  if (!ghosts) return coordinate == (side < 0 ? lower : upper);
  if (side < 0) return coordinate < lower && coordinate >= lower - depth;
  return coordinate > upper && coordinate <= upper + depth;
}

inline std::vector<BoundaryPointLocation> boundary_locations(
    const MultiFab& field, const Box2D& domain, const PreparedBoundaryRegion& region,
    ...
    for (int j = jlo; j <= jhi; ++j) {
      for (int i = ilo; i <= ihi; ++i) {
        ...
        for (int axis = 0; axis < 2; ++axis) {
          ...
          if (side != 0) {
            selected = selected && region_coordinate_matches(
                                       coordinates[axis], axis, side, domain, ghosts, depth);
          } else {
            selected = selected && coordinates[axis] >= domain.lo[axis] &&
                       coordinates[axis] <= domain.hi[axis];
          }
        }
        if (selected) locations.push_back({li, i, j});
      }
    }
  }
  return locations;
...
inline std::vector<double> pack_field(const MultiFab& field,
  ...
  if (field.box_array().boxes() != layout_reference.box_array().boxes() ||
    ...
    throw std::runtime_error(
        "boundary component qualified field differs from the prepared BoxArray/DistributionMapping");
  const_cast<MultiFab&>(field).sync_host();
  std::vector<double> packed(
      locations.size() * static_cast<std::size_t>(field.ncomp()));
  for (std::size_t point = 0; point < locations.size(); ++point) {
    ...
    const int i = clamp_to_domain
                      ? std::clamp(location.i, domain.lo[0], domain.hi[0])
                      : location.i;
    const int j = clamp_to_domain
                      ? std::clamp(location.j, domain.lo[1], domain.hi[1])
                      : location.j;
...
inline void scatter_field(MultiFab& field,
                          const std::vector<BoundaryPointLocation>& locations,
  ...
  for (std::size_t point = 0; point < locations.size(); ++point) {
    ...
    for (int component = 0; component < field.ncomp(); ++component)
      values(location.i, location.j, component) = static_cast<Real>(
          packed[point * static_cast<std::size_t>(field.ncomp()) +
...
inline PopsConstFieldViewV1 const_view(
    const std::vector<double>& values, std::size_t points, std::size_t components,
    const std::string& layout_identity, const std::string& patch_identity) {
  return {
      sizeof(PopsConstFieldViewV1), values.data(), 2, {points, 1, 1},
      ...
      POPS_SCALAR_FLOAT64_V1, POPS_MEMORY_SPACE_HOST_V1, layout_identity.c_str(),
      patch_identity.c_str(), POPS_FIELD_OWNERSHIP_RUNTIME_BORROWED_V1};
}

inline PopsFieldViewV1 field_view(
    std::vector<double>& values, std::size_t points, std::size_t components,
    const std::string& layout_identity, const std::string& patch_identity) {
  return {
      sizeof(PopsFieldViewV1), values.data(), 2, {points, 1, 1},
      {static_cast<std::ptrdiff_t>(components), static_cast<std::ptrdiff_t>(components), 0},
      components, 1, POPS_FIELD_CENTERING_CELL_V1, 0, {0, 0, 0}, {0, 0, 0},
      POPS_SCALAR_FLOAT64_V1, POPS_MEMORY_SPACE_HOST_V1, layout_identity.c_str(),
      patch_identity.c_str(), POPS_FIELD_OWNERSHIP_RUNTIME_BORROWED_V1};
...
inline PopsLogicalTimeV1 logical_time(
  ...
  return {sizeof(PopsLogicalTimeV1), point.clock.c_str(), point.tick, point.level,
          point.substep, point.stage, point.stage_fraction.numerator,
          point.stage_fraction.denominator, point.dt, point.physical_time};
}

inline std::vector<double> coordinates(
    const std::vector<BoundaryPointLocation>& locations, const Geometry& geometry) {
...
inline std::vector<PopsQualifiedScalarV1> scalar_table(
...
inline std::vector<PackedConstBoundaryField> pack_qualified_const_fields(
  ...
  for (const std::string& identity : identities) {
    ...
    packed.push_back({identity, pack_field(
        field, locations, domain, clamp_to_domain, layout_reference)});
  }
  return packed;
}
...
inline std::vector<PopsQualifiedConstFieldV1> qualified_const_views(
  ...
  for (const auto& row : packed)
    result.push_back({
        sizeof(PopsQualifiedConstFieldV1), 1u, row.identity.c_str(),
        const_view(row.values, count,
                   row.values.size() / count, layout, patch)});
  return result;
}
...
inline void apply_ghost_component(
    const PreparedGhostBoundaryComponent& component, MultiFab& state,
  ...
  const PopsConstFieldViewV1 interior_view =
      const_view(interior, count, static_cast<std::size_t>(state.ncomp()),
  ...
  const PopsFieldViewV1 ghost_view =
      field_view(ghosts, count, static_cast<std::size_t>(state.ncomp()),
                 spec.layout_identity, spec.region.identity);

  auto packed_dependencies = pack_qualified_const_fields(
      spec.states, [&registry](const std::string& identity) -> const MultiFab& {
        return registry.state(identity);
      }, locations, geometry.domain, state, true);
  auto packed_fields = pack_qualified_const_fields(
      spec.fields, [&registry](const std::string& identity) -> const MultiFab& {
        return registry.field(identity);
      }, locations, geometry.domain, state, true);
  packed_dependencies.insert(
      packed_dependencies.end(),
      std::make_move_iterator(packed_fields.begin()),
      std::make_move_iterator(packed_fields.end()));
  const auto dependencies = qualified_const_views(
      packed_dependencies, count, spec.layout_identity, spec.region.identity);
  const auto parameters = scalar_table(spec);
  PopsGhostBoundaryRequestV1 request{
      sizeof(PopsGhostBoundaryRequestV1), spec.producer_identity.c_str(),
      spec.state_identity.c_str(), spec.ghost_identity.c_str(), interior_view, ghost_view,
      coordinate_view, spec.region.view(), dependencies.size(), dependencies.data(),
      parameters.size(), parameters.data(), logical_time(point),
      spec.execution->view()};
  ...
  const int code = component::apply_ghost_boundary(
      component.ghost_api(), component.state(), request, status);
  PreparedGhostBoundaryComponent::require_success(
      code, status, "apply_region_batch");
...
inline void apply_ghost_component(
  ...
  if (!spec.fields.empty()) {
    if (auxiliary == nullptr)
      throw std::runtime_error(
          "GhostBoundary field dependencies have no prepared auxiliary field");
...
inline void apply_field_component(
    const PreparedBoundaryComponent<Operation>& component,
  ...
  const auto locations = boundary_locations(
      layout_reference, geometry.domain, spec.region, 1, false);
  if (locations.empty()) return;
  const std::size_t count = locations.size();
  std::vector<double> xy = coordinates(locations, geometry);
  const PopsConstFieldViewV1 coordinate_view =
  ...
  auto packed_directions = pack_qualified_const_fields(
      spec.directions, [&registry](const std::string& identity) -> const MultiFab& {
  ...
  const auto states = qualified_const_views(
      packed_states, count, spec.layout_identity, spec.region.identity);
  const auto directions = qualified_const_views(
      packed_directions, count, spec.layout_identity, spec.region.identity);
  const auto fields = qualified_const_views(
      packed_fields, count, spec.layout_identity, spec.region.identity);
  ...
  for (auto& row : packed_outputs)
    outputs.push_back({
        sizeof(PopsQualifiedFieldV1), row.identity.c_str(),
    ...
    for (const auto* table : {&packed_states, &packed_directions, &packed_fields})
      for (const auto& row : *table)
        if (row.identity == identity)
          return const_view(row.values, count, row.values.size() / count,
                            spec.layout_identity, spec.region.identity);
  ...
  const auto parameters = scalar_table(spec);
  const PopsQualifiedConstFieldV1 rate = spec.rate.empty()
      ? PopsQualifiedConstFieldV1{sizeof(PopsQualifiedConstFieldV1), 0u, nullptr, {}}
      : PopsQualifiedConstFieldV1{
            sizeof(PopsQualifiedConstFieldV1), 1u, spec.rate.c_str(),
            find_const_view(spec.rate)};
  const PopsQualifiedConstFieldV1 nonlinear = spec.nonlinear_iterate.empty()
      ? PopsQualifiedConstFieldV1{sizeof(PopsQualifiedConstFieldV1), 0u, nullptr, {}}
      : PopsQualifiedConstFieldV1{
            sizeof(PopsQualifiedConstFieldV1), 1u, spec.nonlinear_iterate.c_str(),
            find_const_view(spec.nonlinear_iterate)};
  PopsFieldBoundaryRequestV1 request{
      sizeof(PopsFieldBoundaryRequestV1), spec.target_identity.c_str(), spec.region.view(),
      coordinate_view, states.size(), states.data(), directions.size(), directions.data(),
      fields.size(), fields.data(), parameters.size(), parameters.data(), outputs.size(),
      outputs.data(), rate, nonlinear, point.level, logical_time(point),
      spec.execution->view()};
  PopsComponentStatusV1 status{
      sizeof(PopsComponentStatusV1), 0, POPS_COMPONENT_CONTINUE_V1, nullptr};
  const int code = component::evaluate_field_boundary(
      component.field_api(), component.state(), request, status, jvp);
...
template <PreparedBoundaryOperation Operation>
inline void apply_field_component(
    const PreparedBoundaryComponent<Operation>& component, const MultiFab& state,
    const MultiFab* direction, const MultiFab* auxiliary, const Geometry& geometry,
    MultiFab& output, const runtime::multiblock::BoundaryEvaluationPoint& point) {
  const auto& spec = component.spec();
  BoundaryFieldRegistry registry;
  registry.bind_state(spec.state_identity, state);
