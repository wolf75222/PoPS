...
#include <adc/numerics/elliptic/poisson_fft.hpp>  // fft1d (1D DFT/FFT reused verbatim)
...
class PolarPoissonSolver {
 public:
  ...
  void solve() {
    if (phi_.local_size() == 0) return;  // rank with no local box (MPI): nothing to do (cf. guard)
  ...
  Real residual() {
    ...
    for (int m = 0; m < nth; ++m) {
      const int kw = (m <= nth / 2) ? m : m - nth;
      const Real lam_th = -static_cast<Real>(kw) * static_cast<Real>(kw);
      const bool pin0 = neumann_both && m == 0;  // gauge mode: row 0 pinned (excluded from the residual)
      for (int i = 0; i < nr; ++i) {
        if (pin0 && i == 0) continue;  // pinned identity row: not a physical equation
