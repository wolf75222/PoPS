...
static AmrRuntime make_two_block(int N, double L, double B0, double q0, double q1,
  ...
  bp.n = N;
  bp.L = L;
  bp.regrid_every = 0;      // le runtime porte sa propre cadence via set_regrid (la facade ne pilote pas ici)
...
int main(int argc, char** argv) {
  ...
  // ============================================================================================
  {
    auto run_frozen = [&]() {
      ...
      for (int s = 0; s < 8; ++s) rt.step(Real(0.01));
  ...
  // ============================================================================================
  {
    ...
    for (int s = 0; s < 6; ++s) rt.step(Real(0.01));
  ...
  // ============================================================================================
  {
    ...
    AmrRuntime rtp = make_two_block(N, L, B0, +1.0, -1.0, blob(N, 0.5, 0.5, 1.5, 1.0, 0.06),
                                    flat(N, 1.0));
    ...
    rtp.set_phi_tag_predicate(TagGradPhiAbove{Real(1e-6)});  // |grad phi| > epsilon -> tague le bord
  ...
  // ============================================================================================
  {
    ...
    for (int s = 0; s < 3; ++s) rt.step(Real(0.01));
