...
int main() {
  ...
  {
    BCRec mixed;
    mixed.xlo = BCType::Dirichlet; mixed.xlo_val = 3.0;
    mixed.yhi = BCType::Foextrap;
    EllipticProblem prob{Real(1), mixed, false};
    BCRec h0 = homogeneous(prob.bc);
    BCRec h1 = homogeneous_bc(prob);
    chk(h0.xlo == h1.xlo && h0.xhi == h1.xhi && h0.ylo == h1.ylo &&
            h0.yhi == h1.yhi,
        "homogeneous_bc_meme_types");
    chk(h0.xlo_val == h1.xlo_val && h0.xhi_val == h1.xhi_val &&
            h0.ylo_val == h1.ylo_val && h0.yhi_val == h1.yhi_val,
  ...
  GeometricMG mg_named =
      make_elliptic_solver<GeometricMG>(geom, ba, prob);
  solve_into(mg_ref);
  solve_into(mg_named);
  ...
  for (int j = 0; j < N; ++j)
    for (int i = 0; i < N; ++i)
      if (pr(i, j) != pn(i, j)) phi_bit_eq = false;
  ...
  field_postprocess(phi, plus, cx, cy,
  ...
  bool plus_bit_eq = true;
  {
    ...
    for (int j = v.lo[1]; j <= v.hi[1]; ++j)
      for (int i = v.lo[0]; i <= v.hi[0]; ++i)
        for (int c = 0; c < 3; ++c)
          if (ar(i, j, c) != ap(i, j, c)) plus_bit_eq = false;
  }
  ...
  field_postprocess(phi, minus, cx, cy,
  ...
  bool minus_bit_eq = true;
  {
    ...
    for (int j = v.lo[1]; j <= v.hi[1]; ++j)
      for (int i = v.lo[0]; i <= v.hi[0]; ++i) {
        if (am(i, j, 0) != -ar(i, j, 1)) minus_bit_eq = false;
        if (am(i, j, 1) != -ar(i, j, 2)) minus_bit_eq = false;
      }
  }
  chk(minus_bit_eq, "FieldPostProcess_Minus_egale_moins_grad");

  if (fails == 0) std::printf("OK test_elliptic_problem\n");
  return fails == 0 ? 0 : 1;
}
