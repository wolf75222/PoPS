...
// Solve the same manufactured problem with a half-domain refined patch translated
...
inline TensorSeamMeasurement tensor_periodic_seam_witness(
  ...
  if (n < 8 || n % 4 != 0 || fine_offset < 0 || fine_offset >= 2 * n ||
  ...
  const Geometry<2> coarse_geometry = Geometry<2>::from_bounds(
      domain, RealVector<2>{0, 0}, RealVector<2>{1, 1});
  ...
  for (int level = 0; level < 2; ++level) {
    ...
    for (BoundarySide side : {BoundarySide::lower, BoundarySide::upper})
      faces[static_cast<std::size_t>(Face<2>{0, side}.ordinal())] =
          {PhysicalBoundaryKind::dirichlet, Real(0)};
  ...
  for (int part = 0; part < parent_parts; ++part) {
    parent_boxes.push_back({Index<2>{0, part * n / parent_parts},
                            Index<2>{n - 1, (part + 1) * n / parent_parts - 1}});
  ...
  for (int part = 0; part < lane.size(); ++part) {
    ...
    const int upper_y = fine_offset + n - 1;
    fine_boxes.push_back({Index<2>{lower_x, fine_offset},
                          Index<2>{upper_x, std::min(upper_y, 2 * n - 1)}});
    fine_owners.push_back(Index<2>{part, 0});
    if (upper_y >= 2 * n) {
      fine_boxes.push_back({Index<2>{lower_x, 0}, Index<2>{upper_x, upper_y - 2 * n}});
      fine_owners.push_back(Index<2>{part, 0});
    }
  }
  ...
  const Real phase = Real(fine_offset) / Real(2 * n);
    ...
    for (std::size_t local = 0; local < rhs[level]->local_size(); ++local) {
      ...
      for (int j = valid.lo[1]; j <= valid.hi[1]; ++j)
        for (int i = valid.lo[0]; i <= valid.hi[0]; ++i) {
          ...
          const Real wave = std::sin(pi * x) * std::cos(Real(2) * pi * y);
          const Real axx = Real(1) + Real(0.1) * std::cos(Real(2) * pi * y);
          const std::array<Real, 4> tensor{axx, Real(0.15), Real(-0.1), Real(1.5)};
          for (int slot = 0; slot < 4; ++slot)
            tensor_host[slot](offset(coefficients[level][slot]->fab(local).grown_box(), i, j)) = tensor[slot];
          right_host(offset(right.grown_box(), i, j)) =
              pi * pi * (axx + Real(6)) * wave + Real(0.1) * pi * pi *
              std::cos(pi * x) * std::sin(Real(2) * pi * y);
        }
  ...
  FullTensorCompositeFac<2> solver(bindings, ratios, lane);
  Controls controls;
  controls.relative_tolerance = Real(2e-9);
  controls.absolute_tolerance = Real(1e-12);
  controls.maximum_iterations = 120;
  controls.fine_sweeps = 48;
  controls.coarse_cycles = 128;
  controls.coarse_relative_tolerance = Real(1e-11);
  TensorSeamMeasurement result;
  result.remote_parent_gather = solver.has_remote_parent_gather();
  result.report = solver.solve(controls, lane);
  if (!result.report.solved())
    throw std::runtime_error("tensor periodic seam MMS did not converge: " + result.report.reason);
  result.coarse.assign(static_cast<std::size_t>(n * n), Real(0));
  result.fine.assign(static_cast<std::size_t>(n * n), Real(0));
  for (int level = 0; level < 2; ++level) {
      ...
      for (int j = valid.lo[1]; j <= valid.hi[1]; ++j)
        for (int i = valid.lo[0]; i <= valid.hi[0]; ++i) {
          ...
          if (!std::isfinite(value))
            throw std::runtime_error("tensor periodic seam MMS has a non-finite solution");
          const int width = level == 0 ? n : 2 * n;
          ...
          if (level == 1) {
            ...
            result.maximum_error = std::max(result.maximum_error,
                std::abs(value - std::sin(pi * x) * std::cos(Real(2) * pi * y)));
          }
        }
    }
  }
...
inline Real tensor_seam_difference(const TensorSeamMeasurement& a, const TensorSeamMeasurement& b) {
  Real difference = 0;
