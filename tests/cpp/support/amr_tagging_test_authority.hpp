...
inline void install_prepared_threshold_decisions(
    ...
    pops::amr::TagEqualityPolicy equality_policy = pops::amr::TagEqualityPolicy::Hold) {
  using Program = AmrRuntime::TaggingProgram;
  ...
  runtime.set_tagging_program({}, std::move(leaves), std::move(refine_ops), std::move(refine_args),
                              std::move(coarsen_ops), std::move(coarsen_args), min_cycles,
                              static_cast<int>(equality_policy), 0, "test::prepared-tagging-clock",
