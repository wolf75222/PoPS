...
constexpr nd::HierarchyValidationBudget kHierarchyBudget{8, 4096};

template <int Dim>
nd::LevelLayout<Dim> make_level(int level, const Box<Dim>& domain,
...
TEST(test_nd_hierarchy_plan, one_dimensional_nonzero_origin_and_ratio_are_exact) {
  ...
  const Box<1> fine_patch = nd::refine_box(Box<1>{Index<1>{-3}, Index<1>{-1}}, ratio);
  ...
  const nd::HierarchyPlan<1> plan({coarse, fine}, kHierarchyBudget);
  ASSERT_EQ(plan.num_levels(), 2U);
  EXPECT_EQ(plan.level(1).domain(), (Box<1>{Index<1>{-9}, Index<1>{14}}));
  EXPECT_EQ(nd::coarsen_box(fine_patch, ratio), (Box<1>{Index<1>{-3}, Index<1>{-1}}));
  EXPECT_EQ(plan.exact_identity(),
            nd::HierarchyPlan<1>({coarse, fine}, kHierarchyBudget).exact_identity());
}
...
TEST(test_nd_hierarchy_plan, anisotropic_two_and_three_dimensional_levels_are_validated) {
  ...
  const auto plane_fine =
      make_level<2>(1, nd::refine_box(plane_domain, plane_ratio), plane_fine_patches, plane_ranks,
                    {Index<2>{6, -2}}, plane_ratio);
  const nd::HierarchyPlan<2> plane({plane_coarse, plane_fine}, kHierarchyBudget);
  ...
  const auto volume_fine =
      make_level<3>(1, nd::refine_box(volume_domain, volume_ratio), volume_fine_patches,
                    volume_ranks, {Index<3>{7, -3, 2}}, volume_ratio);
  const nd::HierarchyPlan<3> volume({volume_coarse, volume_fine}, kHierarchyBudget);
  EXPECT_EQ(volume.level(1).domain(), (Box<3>{Index<3>{-4, 3, -3}, Index<3>{3, 4, 5}}));
  EXPECT_EQ(nd::coarsen_box(volume_fine_patch, volume_ratio),
