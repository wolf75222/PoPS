...
TEST(test_nd_tag_mask, explicit_metadata_cell_byte_and_identity_budgets_fail_closed) {
  ...
  EXPECT_THROW(
      ...
      std::length_error);
  EXPECT_THROW(
      (void)nd::TagMask<1>(level, Index<1>{0}, nd::TagMaskBudget{2, 1, 4, 8, 8, kIdentityBudget}),
      std::length_error);
  EXPECT_THROW(
      (void)nd::TagMask<1>(level, Index<1>{0}, nd::TagMaskBudget{2, 2, 3, 8, 8, kIdentityBudget}),
      std::length_error);
  EXPECT_THROW(
      (void)nd::TagMask<1>(level, Index<1>{0}, nd::TagMaskBudget{2, 2, 4, 7, 8, kIdentityBudget}),
      std::length_error);
  EXPECT_THROW(
      (void)nd::TagMask<1>(level, Index<1>{0}, nd::TagMaskBudget{2, 2, 4, 8, 7, kIdentityBudget}),
      std::length_error);
  EXPECT_THROW((void)nd::TagMask<1>(level, Index<1>{0}, nd::TagMaskBudget{2, 2, 4, 8, 8, 1}),
               std::length_error);
  EXPECT_THROW((void)nd::TagMask<1>(level, Index<1>{2}, tag_budget(2, 2, 4, 8)), std::out_of_range);
}
...
TEST(test_nd_tag_mask, exact_identity_tracks_rank_patch_topology_and_tag_bits) {
  ...
  nd::TagMask<1> other_rank(level, Index<1>{5}, tag_budget(2, 1, 2, 2));
