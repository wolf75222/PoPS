...
template <int Dim>
nd::LevelLayout<Dim> make_partitioned_level(int level, const Box<Dim>& domain,
                                            const mesh::BoxArray<Dim>& patches,
                                            const mesh::RankSpace<Dim>& ranks,
                                            const std::vector<Index<Dim>>& owners,
                                            const nd::RefinementRatio<Dim>& ratio) {
  return nd::LevelLayout<Dim>(level, domain, patches,
                              mesh::Distribution<Dim>::partitioned(patches, ranks, owners), ratio,
                              kLayoutBudget);
}
...
TEST(test_nd_tag_mask, partitioned_storage_contains_only_owned_patches) {
  ...
  nd::TagMask<1> mask(level, Index<1>{10}, nd::TagMaskBudget{2, 2, 4, 4});
...
TEST(test_nd_tag_mask, all_storage_dimensions_honor_nonzero_origins_and_axis_zero_order) {
  ...
  nd::TagMask<2> plane_mask(plane_level, Index<2>{3, -1}, nd::TagMaskBudget{1, 6, 6, 6});
  ...
  EXPECT_EQ(plane_tags, (std::vector<Index<2>>{Index<2>{-1, 6}}));
  ...
  const auto volume_level = make_partitioned_level<3>(0, volume, volume_patches, volume_ranks,
                                                      {Index<3>{-3, 2, 1}}, {1, 1, 1});
  nd::TagMask<3> volume_mask(volume_level, Index<3>{-3, 2, 1}, nd::TagMaskBudget{1, 12, 12, 12});
  volume_mask.set(Index<3>{5, -1, 8});
  EXPECT_EQ(volume_mask.count(), 1U);
  EXPECT_TRUE(volume_mask.tagged(0, Index<3>{5, -1, 8}));
}

TEST(test_nd_tag_mask, explicit_patch_cell_total_and_byte_budgets_fail_before_allocation) {
  ...
  EXPECT_THROW((void)nd::TagMask<1>(level, Index<1>{0}, nd::TagMaskBudget{1, 4, 8, 8}),
               std::length_error);
  EXPECT_THROW((void)nd::TagMask<1>(level, Index<1>{0}, nd::TagMaskBudget{2, 3, 8, 8}),
               std::length_error);
  EXPECT_THROW((void)nd::TagMask<1>(level, Index<1>{0}, nd::TagMaskBudget{2, 4, 7, 8}),
               std::length_error);
  EXPECT_THROW((void)nd::TagMask<1>(level, Index<1>{0}, nd::TagMaskBudget{2, 4, 8, 7}),
               std::length_error);
  EXPECT_THROW((void)nd::TagMask<1>(level, Index<1>{2}, nd::TagMaskBudget{2, 4, 8, 8}),
               std::out_of_range);
}
...
TEST(test_nd_tag_mask, exact_identity_tracks_rank_patch_topology_and_tag_bits) {
  ...
  nd::TagMask<1> first(level, Index<1>{4}, nd::TagMaskBudget{1, 2, 2, 2});
  nd::TagMask<1> same(level, Index<1>{4}, nd::TagMaskBudget{1, 2, 2, 2});
  EXPECT_EQ(first.exact_identity(), same.exact_identity());
  first.set(Index<1>{0});
  EXPECT_NE(first.exact_identity(), same.exact_identity());

  nd::TagMask<1> other_rank(level, Index<1>{5}, nd::TagMaskBudget{1, 2, 2, 2});
