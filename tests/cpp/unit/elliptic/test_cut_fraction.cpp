...
TEST(test_cut_fraction, cut_face_matches_analytic_axial_crossing) {
  ...
  EXPECT_TRUE(std::fabs(cf.axp - Real(0.1)) < Real(1e-12)) << "axp_distance_analytique";
  EXPECT_TRUE(std::fabs(cf.alpha_xp - Real(0.5)) < Real(1e-12)) << "alpha_xp_analytique";
  // face -x : voisin (0.7, 0) ls = -0.3 < 0 -> interieur, face PLEINE = dx, aperture = 1.
  EXPECT_TRUE(std::fabs(cf.axm - dx) < Real(1e-12)) << "axm_face_pleine";
  EXPECT_TRUE(std::fabs(cf.alpha_xm - Real(1.0)) < Real(1e-12)) << "alpha_xm_pleine";
  // faces y : voisins (0.9, +-0.2) a hypot 0.9219 < 1 -> interieurs, faces pleines.
  EXPECT_TRUE(std::fabs(cf.aym - dy) < Real(1e-12)) << "aym_face_pleine";
  EXPECT_TRUE(std::fabs(cf.ayp - dy) < Real(1e-12)) << "ayp_face_pleine";
  // kappa = 0.5(alpha_xm + alpha_xp) * 0.5(alpha_ym + alpha_yp) = 0.5(1+0.5)*0.5(1+1) = 0.75.
  EXPECT_TRUE(std::fabs(cf.kappa - Real(0.75)) < Real(1e-12)) << "kappa_cellule_coupee";
  ...
  EXPECT_TRUE(std::fabs(w.w_xp - Real(2) / (cf.axp * sx)) < Real(1e-12)) << "w_xp_formule";
  EXPECT_TRUE(w.w_xp > w.w_xm) << "petite_face_poids_plus_grand";  // axp < axm => w_xp > w_xm
}
...
TEST(test_cut_fraction, interior_cell_has_unit_apertures_and_uniform_stencil) {
  const DiscLS ls{0.0, 0.0, 1.0};
  const Real dx = Real(0.2), dy = Real(0.2);

  const detail::CutFraction cf = detail::cut_fraction(ls, Real(0.0), Real(0.0), dx, dy);
  EXPECT_TRUE(std::fabs(cf.alpha_xm - Real(1.0)) < Real(1e-12) &&
  ...
  EXPECT_TRUE(std::fabs(cf.kappa - Real(1.0)) < Real(1e-12)) << "interieur_kappa_unite";
  // loin du bord les poids degenerent au stencil uniforme : 1/dx^2 = 25, diag = 2/dx^2 + 2/dy^2 = 100.
  const detail::ShortleyWellerWeights w = detail::shortley_weller(cf);
  EXPECT_TRUE(std::fabs(w.w_xm - Real(25.0)) < Real(1e-9) &&
...
TEST(test_cut_fraction, theta_clamps_to_minimum_near_the_boundary) {
  const DiscLS ls{0.0, 0.0, 1.0};
  const Real dx = Real(0.2), dy = Real(0.2);
  ...
  EXPECT_TRUE(std::fabs(cf.alpha_xp - Real(1e-3)) < Real(1e-9)) << "clamp_theta_min";
}
...
// ---------------------------------------------------------------------------
TEST(test_cut_fraction, weights_are_bit_identical_to_historical_inline_formula) {
