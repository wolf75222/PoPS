...
// Nomme GridSetup (et non "Setup") : ::testing::Test declare une methode virtuelle Setup(), qui
// masquerait un type libre "Setup" dans le corps d'un TEST() (erreur de compilation).
struct GridSetup {
  Box2D dom;
  Geometry geom;
  BoxArray ba;
...
// ----------------------------------------------------------------------------------------------
TEST(test_schur_condensation, operator_coefficients_match_closed_form_analytic) {
  ...
  EXPECT_TRUE(max_gap(ex, S.dom, [=](int, int) { return exa; }) < EPS) << "A_eps_x_analytique";
  EXPECT_TRUE(max_gap(ey, S.dom, [=](int, int) { return eya; }) < EPS) << "A_eps_y_analytique";
  EXPECT_TRUE(max_gap(axy, S.dom, [=](int, int) { return axya; }) < EPS) << "A_a_xy_analytique";
  EXPECT_TRUE(max_gap(ayx, S.dom, [=](int, int) { return ayxa; }) < EPS) << "A_a_yx_analytique";
  // non symetrie attendue (B_z != 0) : a_xy = -a_yx != 0.
  EXPECT_TRUE(dabs(axya + ayxa) < EPS && dabs(axya) > 1e-6) << "A_non_symetrique_Bz_nonnul";
...
// ----------------------------------------------------------------------------------------------
TEST(test_schur_condensation, rhs_matches_analytic_flux_divergence_in_the_interior) {
  const VariableSet vars = fluid_vars();
  const int n = 48;
  GridSetup S(n);
  ...
  EXPECT_TRUE(gap < 1e-11) << "B_rhs_analytique_interieur";
  std::printf("(B) RHS analytique=%.6f (neg_lap=%.6f, g*divF=%.6f) | ecart interieur=%.3e\n", rhs_a,
              neg_lap, g * divF, gap);
}

// ----------------------------------------------------------------------------------------------
// (C1) B_z = 0, alpha != 0 : A diagonal symetrique (a_xy = a_yx = 0, eps_x = eps_y = 1 + c rho).
// ----------------------------------------------------------------------------------------------
TEST(test_schur_condensation, zero_bz_gives_a_symmetric_diagonal_operator) {
  const VariableSet vars = fluid_vars();
  const int n = 32;
  GridSetup S(n);
  ...
  EXPECT_TRUE(max_gap(ex, S.dom, [=](int, int) { return diag; }) < EPS) << "C1_eps_x_diag";
  EXPECT_TRUE(max_gap(ey, S.dom, [=](int, int) { return diag; }) < EPS) << "C1_eps_y_diag";
  EXPECT_TRUE(max_gap(axy, S.dom, [](int, int) { return 0.0; }) == 0.0) << "C1_a_xy_zero_bitident";
  EXPECT_TRUE(max_gap(ayx, S.dom, [](int, int) { return 0.0; }) == 0.0) << "C1_a_yx_zero_bitident";
...
// ----------------------------------------------------------------------------------------------
TEST(test_schur_condensation, zero_bz_and_zero_alpha_reduces_to_canonical_poisson_bit_identical) {
  const VariableSet vars = fluid_vars();
  const int n = 32;
  GridSetup S(n);
  const Real rho0 = 2.1, theta = 0.5, dt = 0.4;
  ...
  EXPECT_TRUE(max_gap(ex, S.dom, [](int, int) { return 1.0; }) == 0.0) << "C2_eps_x_eq_1_bitident";
  EXPECT_TRUE(max_gap(ey, S.dom, [](int, int) { return 1.0; }) == 0.0) << "C2_eps_y_eq_1_bitident";
  EXPECT_TRUE(max_gap(axy, S.dom, [](int, int) { return 0.0; }) == 0.0) << "C2_a_xy_zero_bitident";
  EXPECT_TRUE(max_gap(ayx, S.dom, [](int, int) { return 0.0; }) == 0.0) << "C2_a_yx_zero_bitident";
  ...
  EXPECT_TRUE(dmax == 0.0) << "C2_rhs_eq_neg_laplacien_bitident";
