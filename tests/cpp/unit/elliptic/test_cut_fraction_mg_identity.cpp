...
static int pops_run_test_cut_fraction_mg_identity() {
  int fails = 0;
  ...
  if (fails == 0)
    std::printf("OK test_cut_fraction_mg_identity\n");
  return fails == 0 ? 0 : 1;
}

TEST(test_cut_fraction_mg_identity, Runs) {
  EXPECT_EQ(pops::test::RunTestBody(&pops_run_test_cut_fraction_mg_identity, "test_cut_fraction_mg_identity"), 0);
}
