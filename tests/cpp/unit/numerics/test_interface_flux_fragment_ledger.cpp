...
TEST(test_interface_flux_fragment_ledger,
  ...
  ledger.begin();
  ledger.accumulate(coarse, {amr::Rational(1, 1), 0.5}, 4.0);
  ledger.accumulate(fine, {amr::Rational(1, 1), 0.5}, 4.0);
  ledger.commit();
...
TEST(test_interface_flux_fragment_ledger, ssprk2_stages_accumulate_with_exact_weights) {
  ...
  ledger.begin();
  ledger.accumulate(stage_0, {amr::Rational(1, 2), 0.5}, 2.0);
  ledger.accumulate(stage_1, {amr::Rational(1, 2), 0.5}, 6.0);
  ledger.commit();
...
TEST(test_interface_flux_fragment_ledger, rollback_never_publishes_pending_fragments) {
  amr::TransactionalInterfaceFluxLedger<double> ledger(9);
  ledger.begin();
  ledger.accumulate(fragment_key(), {amr::Rational(1, 2), 0.5}, 2.0);
  EXPECT_TRUE(ledger.aggregate(scalar_axpy).empty());

  ledger.begin();
  ledger.accumulate(fragment_key(), {amr::Rational(1, 2), 0.5}, 6.0);
  ledger.commit();
...
TEST(test_interface_flux_fragment_ledger, stale_topology_epoch_is_rejected_before_storage) {
  ...
  ledger.begin();
  EXPECT_THROW(ledger.accumulate(stale, {amr::Rational(1, 1), 0.5}, 4.0), std::invalid_argument);
  EXPECT_EQ(ledger.pending_size(), 0u);
  ledger.rollback();

  ledger.begin();
  ledger.accumulate(fragment_key(), {amr::Rational(1, 1), 0.5}, 4.0);
  ledger.commit();
  ...
  EXPECT_TRUE(ledger.empty());
  ledger.begin();
  EXPECT_THROW(ledger.accumulate(fragment_key(), {amr::Rational(1, 1), 0.5}, 4.0),
