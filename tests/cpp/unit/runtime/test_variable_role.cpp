...
static int pops_run_test_variable_role() {
  ...
  if (pops::IsothermalFlux::conservative_vars().index_of(R::MomentumY) != 2) {
    std::printf("FAIL : roles isotherme\n");
    return 1;
  }
  if (pops::ExBVelocity::conservative_vars().index_of(R::Density) != 0) {
...
TEST(test_variable_role, Runs) {
  EXPECT_EQ(pops::test::RunTestBody(&pops_run_test_variable_role, "test_variable_role"), 0);
}
