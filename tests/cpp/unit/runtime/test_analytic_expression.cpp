...
TEST(AnalyticExpression, DiocotronRingIsARegularTypedExpression) {
  ...
  EXPECT_NEAR(program.evaluate(Real(0.375) * std::cos(angle), Real(0.375) * std::sin(angle)),
              Real(1), 1e-14);
...
TEST(AnalyticExpression, InvalidIntermediatesCannotMasqueradeAsFiniteResults) {
  ...
  const auto inside = guarded.view().eval_checked(Real(2.71828182845904523536), Real(0));
...
TEST(AnalyticExpression, ProgramViewRunsInsideTheKokkosCellKernel) {
  ...
  for (const auto [i, j] : {std::pair{0, 0}, std::pair{20, 31}, std::pair{32, 32},
    ...
    const Real expected = radius >= Real(0.35) && radius <= Real(0.40)
...
TEST(AnalyticExpression, InitialMaterializersCompleteBeforeBorrowedInputsExpire) {
...

  {
    ...
    EXPECT_EQ(pops::analytic::materialize_cell_average(values, Real(0), Real(0), Real(1) / Real(65),
                                                       Real(1) / Real(65), programs),
              box.num_cells());
