...
struct AmrProgramHistoryRemapCollectiveTestAccess {
  ...
  static std::uint64_t allocated_attempt(const context_type& context) noexcept {
    return context.allocated_subcycling_attempt_;
  }
  static std::uint64_t accepted_attempt(const context_type& context) noexcept {
    return context.accepted_subcycling_attempt_;
  }
  static void import_accepted(context_type& context) { context.import_accepted_state_(true); }
...
struct AttemptCursorFixture {
  ...
  // 1 rejects a mutated candidate; 2 rejects after the inner accepted publication; 3 probes import.
  std::shared_ptr<int> failure = std::make_shared<int>(0);
};

template <int Dim>
AttemptCursorFixture<Dim> make_attempt_cursor_fixture(bool with_flux, bool mapping = false) {
  AttemptCursorFixture<Dim> fixture;
  pops::AmrSystemConfig<Dim> config;
  config.regrid_every = 0;
  for (int axis = 0; axis < Dim; ++axis) {
    config.shape[axis] = 16;
  ...
  fixture.system = std::make_unique<pops::AmrSystem<Dim>>(config);
  auto& system = *fixture.system;
  pops::test::install_amr_runtime_authority(system, "tests.generated-amr/attempt-cursor");
  system.set_temporal_relations({1}, {1}, {"integral_only"});
  system.install_block_state_route("tracer", "state/tracer");
  pops::add_compiled_model<Dim>(system, "tracer", advection_model<Dim>());
  std::vector<double> initial(cell_count(config.shape), 1.0);
  for (std::size_t cell = 0; cell < initial.size(); ++cell) {
  ...
  pops::test::install_prepared_refine_coarsen_threshold(
      system, {"tracer", "u", 1.5, pops::test::PreparedThresholdRelation::Above, "state/tracer"},
      {"tracer", "u", 1.5, pops::test::PreparedThresholdRelation::Below, "state/tracer"},
      "tests.generated-amr/attempt-cursor-tagging@1");
  ...
  fixture.context = pops::runtime::program::make_program_execution_provider(&system);
  auto context = fixture.context;
  context->configure_primary_clock("clock.macro");
  context->declare_clock_relation("clock.macro", "clock.level.1", 1);
  auto failure = fixture.failure;
  context->install([context, failure, with_flux, mapping](double dt) {
    const auto body = [context, failure, with_flux](double level_dt) {
      auto& state = context->state(0);
      if (*failure == 3) {
        using Access = pops::runtime::program::detail::AmrProgramHistoryRemapCollectiveTestAccess<Dim>;
        EXPECT_THROW(Access::import_accepted(*context), std::logic_error);
      }
      if (with_flux) {
  ...
  using FluxBudget = typename pops::AmrSystem<Dim>::PreparedAmrProgramFluxExpressionBlockBudget;
  system.install_prepared_amr_program_flux_expression_budget(
      with_flux ? "tests.attempt-cursor/flux@1" : "tests.attempt-cursor/source-only@1",
      std::vector<FluxBudget>(1, with_flux ? FluxBudget{1, 1} : FluxBudget{0, 0}), 0, 0);
  return fixture;
}

TEST(GeneratedAmrSystemBlock, AttemptCursorFreshInteriorRestartMatchesEntireAcceptedImage) {
  constexpr int Dim = pops::kNativeDimension;
  using Access = pops::runtime::program::detail::AmrProgramHistoryRemapCollectiveTestAccess<Dim>;
  for (const bool with_flux : {false, true}) {
    SCOPED_TRACE(with_flux);
    auto cold = make_attempt_cursor_fixture<Dim>(with_flux);
    ...
    EXPECT_EQ(Access::allocated_attempt(*cold.context), 0u);
    ...
    const auto decoded = pops::runtime::program::deserialize_amr_program_accepted_state<Dim>(middle);
    EXPECT_EQ(decoded.accepted_attempt, 2u);
    EXPECT_EQ(original.checkpoint_topology_epoch(), epoch);  // Strictly between regrids.
    const bool has_faces = std::any_of(decoded.accepted_face_flux.begin(),
                                     decoded.accepted_face_flux.end(),
                                     [](const auto& axis) { return !axis.empty(); });
    EXPECT_EQ(has_faces, with_flux);

    auto resumed = make_attempt_cursor_fixture<Dim>(with_flux);
    auto& restored = *resumed.system;
    EXPECT_EQ(Access::allocated_attempt(*resumed.context), 0u);
    ...
    EXPECT_EQ(Access::accepted_attempt(*resumed.context), 2u);
    EXPECT_EQ(Access::allocated_attempt(*resumed.context), 2u);
    ...
    EXPECT_EQ(Access::accepted_attempt(*cold.context), 3u);
    EXPECT_EQ(Access::accepted_attempt(*resumed.context), 3u);
  }
}

TEST(GeneratedAmrSystemBlock, AttemptCursorRollbackBurnsRejectedIdsAndRestoresCommittedBytes) {
  constexpr int Dim = pops::kNativeDimension;
  using Access = pops::runtime::program::detail::AmrProgramHistoryRemapCollectiveTestAccess<Dim>;
  for (const bool mapping : {false, true}) {
    SCOPED_TRACE(mapping);
    auto fixture = make_attempt_cursor_fixture<Dim>(true, mapping);
    auto& system = *fixture.system;
    ...
    for (const int failure : {1, 2}) {
      *fixture.failure = failure;
      ...
      EXPECT_EQ(Access::accepted_attempt(*fixture.context), 1u);
      EXPECT_EQ(Access::allocated_attempt(*fixture.context), 1u + failure);
    }
    ...
    EXPECT_EQ(Access::allocated_attempt(*fixture.context), 3u);
    *fixture.failure = 3;
    ASSERT_NO_THROW(system.step(.01));
    EXPECT_EQ(Access::accepted_attempt(*fixture.context), 4u);
...
TEST(GeneratedAmrSystemBlock, AttemptCursorRemainsMonotoneThroughRegridAndRegridOnRestart) {
  constexpr int Dim = pops::kNativeDimension;
  using Access = pops::runtime::program::detail::AmrProgramHistoryRemapCollectiveTestAccess<Dim>;
  auto fixture = make_attempt_cursor_fixture<Dim>(false);
  auto& system = *fixture.system;
  ASSERT_NO_THROW(system.step(.01));
  *fixture.failure = 1;
  EXPECT_THROW(system.step(.01), std::runtime_error);
  *fixture.failure = 0;
  ...
  EXPECT_EQ(Access::accepted_attempt(*fixture.context), 1u);
  EXPECT_EQ(Access::allocated_attempt(*fixture.context), 2u);
  EXPECT_EQ(pops::runtime::program::deserialize_amr_program_accepted_state<Dim>(
                system.program_accepted_state()).accepted_attempt, 1u);
  ASSERT_NO_THROW(system.step(.01));
  EXPECT_EQ(Access::accepted_attempt(*fixture.context), 3u);
...
TEST(GeneratedAmrSystemBlock, AttemptCursorInvalidRestorePreservesFacadeAndLiveAuthority) {
  constexpr int Dim = pops::kNativeDimension;
  using Access = pops::runtime::program::detail::AmrProgramHistoryRemapCollectiveTestAccess<Dim>;
  auto fixture = make_attempt_cursor_fixture<Dim>(true);
  auto& system = *fixture.system;
  ASSERT_NO_THROW(system.step(.01));
  const auto accepted = system.program_accepted_state();
  const auto decoded = pops::runtime::program::deserialize_amr_program_accepted_state<Dim>(accepted);
  ...
  const auto unchanged = [&] {
    ...
    EXPECT_EQ(system.macro_step(), 1);
    EXPECT_EQ(Access::accepted_attempt(*fixture.context), 1u);
    EXPECT_EQ(Access::allocated_attempt(*fixture.context), 1u);
  };
  ...
  EXPECT_EQ(Access::allocated_attempt(*fixture.context), 1u);
  ASSERT_NO_THROW(system.step(.01));
  EXPECT_EQ(Access::accepted_attempt(*fixture.context), 2u);
}
