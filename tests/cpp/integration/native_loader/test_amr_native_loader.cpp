...
int tag_batch(void*, const PopsTaggerRequestV1* request,
              PopsComponentStatusV1* status) {
  ++tag_call_count;
  if (request->program.non_finite_policy != POPS_TAGGING_NON_FINITE_REJECT_V1) {
    *status = {sizeof(PopsComponentStatusV1), 21,
               POPS_COMPONENT_ABORT_RUN_V1, "unsupported non-finite policy"};
    return 21;
  }
  ...
  const auto evaluate = [&](const int32_t* opcodes, const int32_t* arguments,
    ...
    const std::size_t nx = reference.extents[0] - reference.ghost_lower[0] -
                           reference.ghost_upper[0];
    for (std::size_t point = 0; point < points; ++point) {
      ...
      for (std::size_t instruction = 0; instruction < instruction_count; ++instruction) {
        ...
        if (opcode >= 1 && opcode <= 5) {
          ...
          const auto read = [&](std::ptrdiff_t x, std::ptrdiff_t y) {
            const auto offset =
                (x + static_cast<std::ptrdiff_t>(view.ghost_lower[0])) *
                    view.axis_strides[0] +
                (y + static_cast<std::ptrdiff_t>(view.ghost_lower[1])) *
                    view.axis_strides[1] +
                static_cast<std::ptrdiff_t>(leaf.component) * view.component_stride;
            return values[offset];
          };
          double sample = read(static_cast<std::ptrdiff_t>(i),
                               static_cast<std::ptrdiff_t>(j));
          if (opcode == 3) sample = sample < 0.0 ? -sample : sample;
          if (opcode == 4 || opcode == 5) {
            const auto& stencil = request->program.stencils[leaf.stencil_index];
            double squared_norm = 0.0;
            for (std::size_t axis_index = 0; axis_index < stencil.axis_count;
                 ++axis_index) {
              const auto& axis = stencil.axes[axis_index];
              double derivative = 0.0;
              for (std::size_t term = 0; term < axis.term_count; ++term) {
                const auto x = static_cast<std::ptrdiff_t>(i) +
                    (axis.axis == 0 ? axis.offsets[term] : 0);
                const auto y = static_cast<std::ptrdiff_t>(j) +
                    (axis.axis == 1 ? axis.offsets[term] : 0);
          ...
          if (!std::isfinite(sample)) {
            *status = {sizeof(PopsComponentStatusV1), 22,
                       POPS_COMPONENT_ABORT_RUN_V1,
          ...
          equality[begin] = opcode == 16 ? !any_true && any_unknown
                                         : !any_false && any_unknown;
        }
      }
  ...
  if (!evaluate(request->program.refine_opcodes, request->program.refine_arguments,
                request->program.refine_instruction_count,
                request->refine_candidates, request->refine_equalities))
    return 22;
  if (request->program.coarsen_instruction_count != 0) {
    if (!evaluate(request->program.coarsen_opcodes,
                  request->program.coarsen_arguments,
                  request->program.coarsen_instruction_count,
                  request->coarsen_candidates, request->coarsen_equalities))
      return 22;
  }
...
int cluster(void*, const PopsClusteringRequestV1* request,
            PopsComponentStatusV1* status) {
  if (request->dimension != 2 || request->box_capacity < 1) return 2;
...
const PopsTaggerApiV1 tagger{
    {sizeof(PopsTaggerApiV1), POPS_COMPONENT_PROTOCOL_ABI_V1,
...
const PopsComponentInterfaceEntryV1 interfaces[]{
    ...
    {POPS_NATIVE_INTERFACE_TAGGER_V1, 1,
     sizeof(PopsTaggerApiV1), &tagger},
    {POPS_NATIVE_INTERFACE_CLUSTERING_V1, 1,
     sizeof(PopsClusteringApiV1), &clustering}};
const PopsComponentApiV1 component{
    ...
    "manifest-final-flux", 4, interfaces};
}  // namespace
...
extern "C" int pops_test_tag_call_count() { return tag_call_count; }
extern "C" void pops_test_set_partial_tag_output(int value) { partial_tag_output = value; }
)CPP";
}
...
pops::component::ExpectedNativeComponent expected() {
  return {kComponentId, kSemanticIdentity, kManifestIdentity,
          ...
          {{POPS_NATIVE_INTERFACE_NUMERICAL_FLUX_V1, 1,
           ...
           {POPS_NATIVE_INTERFACE_TAGGER_V1, 1,
            sizeof(PopsTaggerApiV1)},
           {POPS_NATIVE_INTERFACE_CLUSTERING_V1, 1,
            sizeof(PopsClusteringApiV1)}}};
}

std::shared_ptr<const pops::component::PreparedExecutionContextV1>
prepared_execution() {
  ...
  return std::make_shared<const pops::component::PreparedExecutionContextV1>(
      ...
      execution.storage_precision, execution.compute_precision,
      execution.accumulation_precision, execution.reduction_precision,
      execution.stream_handle, execution.stream_identity,
...
TEST(test_amr_native_loader, PreparedAmrProvidersExecuteExactTablesAndProvenance) {
  ...
  const auto tag_call_count = reinterpret_cast<CounterFn>(
      pops::dynlib::sym(inspection, "pops_test_tag_call_count"));
  const auto set_partial_tag_output = reinterpret_cast<SetIntFn>(
      pops::dynlib::sym(inspection, "pops_test_set_partial_tag_output"));
  ...
  ASSERT_NE(set_partial_tag_output, nullptr);
  {
    ...
    pops::runtime::amr::PreparedTaggerSpec tagger_spec{
        "test::tagger-provider", kComponentId, kManifestIdentity,
        "case::layout", "case::clock", {1, 2, 3, 4, 5}, {16, 17, 18},
        {POPS_TAGGING_STENCIL_ROUTE_LINEAR_AXIS_STENCIL_L2_V1},
        POPS_TAGGING_MAXIMUM_STENCIL_TERMS_V1, 128,
        POPS_TAGGING_NON_FINITE_REJECT_V1, 1,
        execution};
    pops::runtime::amr::PreparedClusteringSpec clustering_spec{
        "test::clustering-provider", kComponentId, kManifestIdentity,
        "case::layout", 1, execution};
    pops::runtime::amr::PreparedTaggerComponent tagger(
        std::move(tagger_spec), component);
    pops::runtime::amr::PreparedClusteringComponent clustering(
        std::move(clustering_spec), component);
    const pops::runtime::amr::PreparedTaggingProgram program{
        {}, {{0, 0, 1, 0.5, POPS_TAGGING_NO_STENCIL_V1}},
        {1}, {0}, {}, {}, 0, 0, 0,
        POPS_TAGGING_NON_FINITE_REJECT_V1, "case::clock",
        "case::bound-tagging-program", true};

    const pops::Box2D domain{{0, 0}, {3, 2}};
    const std::vector<pops::Box2D> patches{
        pops::Box2D{{0, 0}, {1, 2}}, pops::Box2D{{2, 0}, {3, 2}}};
    pops::MultiFab state(pops::BoxArray(patches),
                         pops::DistributionMapping(2, pops::n_ranks()), 1, 1);
    state.set_val(pops::Real(0));
    for (int local = 0; local < state.local_size(); ++local) {
      auto values = state.fab(local).array();
      if (state.box(local).contains(1, 0)) values(1, 0, 0) = pops::Real(2);
      if (state.box(local).contains(2, 1)) values(2, 1, 0) = pops::Real(3);
    }
    const int calls_before = tag_call_count();
    auto candidates = tagger.tag(
        {{"case::tracer::U", &state}}, program, domain, 0, 7, 0.25,
    ...
    exact_gradient.stencils = {TagProgram::Stencil{
        "case::forward-gradient", POPS_TAGGING_STENCIL_ROUTE_LINEAR_AXIS_STENCIL_L2_V1,
    ...
    const auto gradient_candidates = tagger.tag(
        {{"case::tracer::U", &state}}, exact_gradient, domain, 0, 7, 0.25,
        0.25, 1.0 / 3.0, false, false, false);
    ...
    EXPECT_THROW(
        (void)tagger.tag(
            {{"case::tracer::U", &state}}, false_order, domain, 0, 7, 0.25,
            0.25, 1.0 / 3.0, false, false, false),
        std::runtime_error);
    ...
    EXPECT_THROW(
        (void)tagger.tag(
            {{"case::tracer::U", &state}}, insufficient_halo, domain, 0, 7, 0.25,
            0.25, 1.0 / 3.0, false, false, false),
        std::runtime_error);
    ...
    const auto not_candidates = tagger.tag(
        {{"case::tracer::U", &state}}, not_equality, domain, 0, 7, 0.25,
        0.25, 1.0 / 3.0, false, false, false);
    ...
    any_equality.leaves.push_back(
        {0, 0, POPS_TAGGING_BELOW_V1, -1.0, POPS_TAGGING_NO_STENCIL_V1});
    any_equality.refine_ops = {
        POPS_TAGGING_ABOVE_V1, POPS_TAGGING_BELOW_V1, POPS_TAGGING_ANY_OF_V1};
    any_equality.refine_args = {0, 1, 2};
    any_equality.provider_identity = "case::bound-tagging-program-any-equality";
    const auto any_candidates = tagger.tag(
        {{"case::tracer::U", &state}}, any_equality, domain, 0, 7, 0.25,
        0.25, 1.0 / 3.0, false, false, false);
    ...
    all_equality.leaves.push_back(
        {0, 0, POPS_TAGGING_BELOW_V1, 1.0, POPS_TAGGING_NO_STENCIL_V1});
    all_equality.refine_ops = {
        POPS_TAGGING_ABOVE_V1, POPS_TAGGING_BELOW_V1, POPS_TAGGING_ALL_OF_V1};
    all_equality.refine_args = {0, 1, 2};
    all_equality.provider_identity = "case::bound-tagging-program-all-equality";
    const auto all_candidates = tagger.tag(
        {{"case::tracer::U", &state}}, all_equality, domain, 0, 7, 0.25,
        0.25, 1.0 / 3.0, false, false, false);
    ...
    const auto changed = tagger.tag(
        {{"case::tracer::U", &state}}, changed_threshold, domain, 0, 7, 0.25,
        0.25, 1.0 / 3.0, false, false, false);
    EXPECT_EQ(changed.refine.count(), 1);
    EXPECT_TRUE(changed.refine.tagged(2, 1));

    auto refine_and_coarsen = program;
    refine_and_coarsen.leaves.push_back(
        {0, 0, 2, 1.0, POPS_TAGGING_NO_STENCIL_V1});
    ...
    const auto dual = tagger.tag(
        {{"case::tracer::U", &state}}, refine_and_coarsen, domain, 0, 7, 0.25,
        0.25, 1.0 / 3.0, false, false, false);
    ...
    EXPECT_THROW(
        (void)tagger.tag(
            {{"case::tracer::U", &state}}, unsupported_hysteresis, domain,
            0, 7, 0.25, 0.25, 1.0 / 3.0, false, false, false),
        std::runtime_error);
