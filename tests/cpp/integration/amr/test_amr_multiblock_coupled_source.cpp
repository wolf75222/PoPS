        ...
        [context = context](double macro_dt) {
          context->advance_hierarchy(macro_dt, [context](double level_dt) {
            ...
            for (int program_block = 0; program_block < 2; ++program_block) {
              ...
              pops::SolveOutcome source = context->solve_source_default(
                  program_block, candidate, pops::Real(level_dt), pops::NewtonOptions{});
              const pops::SolveReport report = source.consume(pops::SolveConsumption::kAccept);
              if (!report.solved())
                throw std::runtime_error("multi-block SSPRK2/IMEX source solve failed: " +
                                         report.reason);
