...
TEST(CapabilityReport, ReportsSchemaAbiAndRouteVocabulary) {
  ...
  for (const auto& row : report.routes) {
    ...
    } else if (row.route_id == "checkpoint:accepted_state_v4") {
      saw_checkpoint_v4 = true;
      EXPECT_TRUE(row.status == "available") << "checkpoint_v4_available";
      EXPECT_TRUE(row.layout == "uniform|amr") << "checkpoint_v4_layouts";
  ...
  EXPECT_TRUE(saw_checkpoint_v4) << "saw_checkpoint_v4";
