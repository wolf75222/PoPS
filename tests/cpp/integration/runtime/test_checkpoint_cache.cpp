...
TEST(CheckpointCache, FullRoundTripSerializesAndDeserializesToEqualState) {
  ...
  EXPECT_TRUE(std::fabs(dst.accumulated_dt(5)) < 1e-15) << "restore_accum5_zero";
  EXPECT_TRUE(std::fabs(dst.accumulated_dt(8) - 0.003) < 1e-12) << "restore_accum8_sum";
  // name preserved (named slot keeps its name; nameless slot falls back to node_<id>)
  EXPECT_TRUE(dst.name_of(5) == "fields_from_state") << "restore_name_named";
  EXPECT_TRUE(dst.name_of(8) == "node_8") << "restore_name_nameless_fallback";

  // a deserialized cache is_due exactly like a live one (cold-start gone, cadence resumes)
  EXPECT_TRUE(!dst.is_due(5, 31, 10)) << "restored_not_due_off_cadence";
  EXPECT_TRUE(dst.is_due(5, 40, 10)) << "restored_due_on_cadence";
}
...
TEST(CheckpointCache, OnlyValidSlotsSerializeColdAccumulatorSkipped) {
  ...
  const std::vector<SerializedSlot> blob = serialize(src);
  EXPECT_TRUE(blob.size() == 1 && blob[0].node_id == 1) << "serialize_skips_invalid_slot";
}
...
TEST(CheckpointCache, VerbatimRestartErrorMessages) {
  ...
  EXPECT_TRUE(miss_msg == "checkpoint missing cached value for scheduled node 'fields_from_state'")
