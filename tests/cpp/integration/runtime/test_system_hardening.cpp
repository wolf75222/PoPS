...
// 9.2 ChargeDensityRhs : defaut charge = 0 (neutre ne pollue pas Poisson) ; exige une entree par bloc.
TEST(SystemHardening, ChargeDensityRhsRequiresOneEntryPerBlockAndDefaultsNeutralToZero) {
  ...
  good(system, rhs);
  EXPECT_TRUE(std::fabs(sum(rhs) - (Real(-1) * sum(Ue, 0))) < Real(1e-12)) << "neutral_zero_charge";
  // defaut de SpeciesCharge bien a 0 (et non +1).
  EXPECT_TRUE(SpeciesCharge{}.charge == Real(0)) << "species_charge_default_zero";
}

// 9.3 AmrSystemCoupler : refuse une hierarchie mal formee (throw au ctor).
TEST(SystemHardening, AmrSystemCouplerRejectsWrongBlockLevelsCount) {
  const Box2D dom = Box2D::from_extents(4, 4);
  ...
  // AmrSystemCoupler refuse un block_levels mal dimensionne (1 pour 2 blocs).
  EXPECT_TRUE(throws_exception([&] {
