...
TEST(test_multiblock_interface_scheduler, AmrBoundaryRegistryUsesOtherBlocksProvisionalStageState) {
  ...
  blocks[0].boundary_plan = std::make_shared<PreparedBoundaryPlan>(
      "case::amr::a::boundary", 1,
      prepare_hyperbolic_boundary<2>(
          {"periodic", "periodic", "periodic", "periodic"}, std::vector<double>(4, 0.0),
          {"case::amr::a::xlo", "case::amr::a::xhi", "case::amr::a::ylo", "case::amr::a::yhi"},
          {"Scalar"}),
      std::vector<int>{}, a_state, PreparedBoundaryReadDependencies{{b_state}, {}});
