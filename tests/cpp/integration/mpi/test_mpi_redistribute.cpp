...
static int pops_run_test_mpi_redistribute(int argc, char** argv) {
  comm_init(&argc, &argv);
...
TEST(test_mpi_redistribute, Runs) {
  EXPECT_EQ(pops::test::RunTestBody(&pops_run_test_mpi_redistribute, "test_mpi_redistribute"), 0);
}
