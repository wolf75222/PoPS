...
// C'est le pendant C++/MPI de tests/python/integration/amr/test_dsl_hybrid_amr.py et
// tests/python/unit/codegen/test_dsl_hybrid_coupling.py : le composite hybride passe
...
//
// Pendant strict de tests/cpp/integration/mpi/test_mpi_mbox_parity.cpp (composite 100% natif) : seul le TYPE de brique change. La
...
static void residual_norms(const BoxArray& ba, const DistributionMapping& dm, const Box2D& dom,
  ...
  BlockClosures clo = make_block(model, "minmod", "rusanov", ctx, /*recon_prim=*/false);
...
static int pops_run_test_mpi_hybrid_mbox_parity(int argc, char** argv) {
  comm_init(&argc, &argv);
#if defined(POPS_HAS_KOKKOS)
