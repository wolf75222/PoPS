...
static int pops_run_test_mpi_array_reduce(int argc, char** argv) {
  comm_init(&argc, &argv);
...
TEST(test_mpi_array_reduce, Runs) {
  EXPECT_EQ(pops::test::RunTestBody(&pops_run_test_mpi_array_reduce, "test_mpi_array_reduce"), 0);
}
