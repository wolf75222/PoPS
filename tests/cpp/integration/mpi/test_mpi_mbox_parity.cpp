...
static int pops_run_test_mpi_mbox_parity(int argc, char** argv) {
  comm_init(&argc, &argv);
#if defined(POPS_HAS_KOKKOS)
...
TEST(test_mpi_mbox_parity, Runs) {
  EXPECT_EQ(pops::test::RunTestBody(&pops_run_test_mpi_mbox_parity, "test_mpi_mbox_parity"), 0);
}
