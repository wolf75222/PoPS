...
def test_bound_sim_change_selects_bind_and_freeze_tests():
    ...
    assert "python/tests/test_bind_adapters.py" in sel
    assert "python/tests/test_freeze_lifecycle.py" in sel
...
def test_waves_change_selects_wave_speed_providers_test():
    ...
    assert "python/tests/test_wave_speed_providers.py" in sel
...
def test_cross_test_forward_pulls_shared_brick_helper():
    ...
    assert edges.get("python/tests/test_dsl_block.py") == {"python/tests/test_dsl_brick.py"}
    selected = {"python/tests/test_dsl_block.py"}
    cic._close_cross_test(selected, edges)
    assert "python/tests/test_dsl_brick.py" in selected
...
def test_cross_test_reverse_pulls_dependents_of_shared_helper():
    ...
    _, edges = cic.test_imports(REPO_ROOT)
    selected = {"python/tests/test_dsl_brick.py"}
    cic._close_cross_test(selected, edges)
    for dependent in (
        "test_dsl_block",
        "test_dsl_cse",
        "test_dsl_dynamic",
        "test_dsl_recon",
        "test_block_names_dynamic",
    ):
        assert f"python/tests/{dependent}.py" in selected
...
def test_plan_python_leaf_change_is_a_strict_subset_with_smoke(tmp_path):
    ...
    assert "python/tests/test_diagnostics_typed.py" in selected
    for smoke in ("test_bindings", "test_capabilities"):
        assert f"python/tests/{smoke}.py" in selected
...
def test_plan_python_direct_test_edit_pulls_cross_test_family(tmp_path):
    ...
    outputs, selected = _run_plan_python(tmp_path, ["python/tests/test_dsl_block.py"])
    assert outputs["python_mode"] == "subset"
    assert "direct-test" in outputs["python_why"]
    assert "python/tests/test_dsl_block.py" in selected
    assert "python/tests/test_dsl_brick.py" in selected  # helper it imports
