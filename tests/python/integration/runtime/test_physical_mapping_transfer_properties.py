...
def _polynomial_cell_averages(nx, nv):
    ...
    x, x2, x3 = (
        _power_average(x_edges[:-1], x_edges[1:], power)
        for power in (1, 2, 3)
    )
    v, v2, v3, v4 = (
        _power_average(v_edges[:-1], v_edges[1:], power)[:, None]
...
def test_physical_property_oracle_uses_true_cell_averages(nx, nv):
    ...
    if (nx, nv) != REFINEMENTS[0]:
        ...
        np.testing.assert_allclose(_restrict_phase(distribution, nx // 2, nv // 2),
                                   coarse_f, rtol=0, atol=ATOL)
    ...
    for nx, nv in REFINEMENTS:
        ...
        try:
            ...
            for route in native._transfer_routes:
                ...
                native._authenticate_mapping_receipt(route, receipt,
...
def test_native_velocity_moment_commutes_with_reference_restriction(
    ...
    np.testing.assert_allclose(_restrict_phase(fine_f, coarse_nx, coarse_nv),
                               coarse_f, rtol=0, atol=ATOL)
    ...
    np.testing.assert_allclose(_restrict_x(fine_moment, coarse_nx),
                               coarse_moment, rtol=0, atol=ATOL)
...
def test_native_spatial_pullback_commutes_with_reference_restriction(
    ...
    np.testing.assert_allclose(_restrict_x(fine_field, coarse_nx),
                               coarse_field, rtol=0, atol=ATOL)
    np.testing.assert_allclose(fine_pullback,
        np.broadcast_to(fine_field, fine_pullback.shape), rtol=0, atol=ATOL)
    np.testing.assert_allclose(coarse_pullback,
        np.broadcast_to(coarse_field, coarse_pullback.shape), rtol=0, atol=ATOL)
    np.testing.assert_allclose(_restrict_phase(fine_pullback, coarse_nx, coarse_nv),
                               coarse_pullback, rtol=0, atol=ATOL)
...
def test_physical_commutation_refuses_changed_x_range(tmp_path, lower, upper):
    ...
    for row in plan.mappings:
        ...
        changed_target = SimpleNamespace(
            shape=target.shape, lower=(lower, target.lower[1]),
            upper=(upper, target.upper[1]), periodicity=target.periodicity,
