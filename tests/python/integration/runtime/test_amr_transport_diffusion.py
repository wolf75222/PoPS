"""Bounded two-level AMR transport-diffusion conformance through the generated Program."""
...
def _case_and_layout():
    frame = Rectangle("amr-transport-diffusion-domain", lower=(0.0, 0.0), upper=(1.0, 1.0)).frame(
        Cartesian2D()
    )
    ...
    diffusion = model.diffusive_flux("diffusion", state=state, value=0.05 * pops.math.grad(u))
    rate = model.rate(
    ...
    # Two evaluations of the same transport occurrence are a generated C++ compile/runtime
    # control for occurrence-local accepted-mask variables.
...
def _composite_mass(simulation):
    ...
    for level in range(simulation.n_levels()):
        active = composite_active_block_state(simulation, "heat", level, refinement_ratio=2)
        mass += float(active.sum()) / (CELLS * (2**level)) ** 2
    return mass


def test_bounded_generated_amr_transport_diffusion_reconciles_fluxes(
    ...
    manifest = tuple(
        tuple(map(str, row)) for row in simulation._executor.program_flux_ledger_manifest()
    )
    assert manifest
