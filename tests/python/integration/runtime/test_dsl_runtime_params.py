"""PARAMETRES RUNTIME du DSL (P7-b) : un ``pops.params.RuntimeParam`` lu via ``Model.value`` peut voir
...
def _build_iso(declaration):
    """Modele isotherme 2D dont l'unique parametre ``cs2`` est une declaration typee."""
    m = Model("iso")
    rho, mx, my = m.conservative_vars("rho", "rho_u", "rho_v")
    cs2 = m.value(m.param(declaration))
...
def _check_codegen_non_regression():
    ...
    const_src = _build_iso(ConstParam("cs2", 2.5))._m.emit_cpp_brick(name="IsoHyp")
    ...
    rt_src = _build_iso(RuntimeParam("cs2", default=2.5))._m.emit_cpp_brick(name="IsoHyp")
    assert "runtime_params.hpp" in rt_src, "param runtime : doit inclure runtime_params.hpp"
    assert "pops::RuntimeParams params{1, {pops::Real(0)}}" in rt_src, \
        "the generated carrier is neutral; BindSchema owns declaration defaults"
    assert "params.get(0)" in rt_src, "param runtime : doit lire params.get(0) (pas de valeur en dur)"
    print("OK  (1) param const INLINE, runtime -> params.get(0) + neutral carrier")
...
def main():
    ...
    try:
        ...
        # (2) RUNTIME : install cs2=1 puis change cs2=4, SANS recompiler.
        sys = build(cs2=1.0)
        ...
        try:
            ...
            assert "runtime" in message and "param" in message, "message inattendu : %s" % ex
