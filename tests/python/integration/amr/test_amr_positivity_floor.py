...
tests/cpp/unit/numerics/test_positivity_floor.cpp; that is out of scope of the floor):
  ...
  (1) FACADE: positivity_floor > 0 no longer raises on the AMR path and the floored run stays FINITE on
      ...
      System path by tests/cpp/unit/numerics/test_positivity_floor.cpp and on a refined AMR C/F interface by (3); it is not
...
tests/cpp/integration/amr/test_amr_positivity_floor.cpp; not re-tested here to keep this test compiler-free.
"""
...
# is covered on System by tests/cpp/unit/numerics/test_positivity_floor.cpp and on a refined AMR C/F interface by (3).
