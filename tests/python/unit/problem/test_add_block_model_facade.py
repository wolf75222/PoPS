#!/usr/bin/env python3
"""ADC-557 acceptance: add_block(model=facade_Model) resolves to a Module with no manual to_module.

The standard flow adds a physics facade ``Model`` directly (``problem.add_block(name, model=m)``)
...
from pops.codegen.orchestration import _resolve_problem_model  # noqa: E402


def _facade_model(name="gas"):
    m = Model(name)
    rho, mx = m.conservative_vars("rho", "mx")
    m.flux(x=[mx, mx * mx / rho], y=[rho, mx])
    return m
...
def test_facade_model_never_requires_manual_to_module():
    # The facade Model is added AS-IS; the standard flow does not call to_module / lower. The facade
