...
def _program():
    ...
    return program, iterate, operator


def _record(program, operator, iterate, r0, *, sources=None, field_coupled=False):
...
def test_rhs_jacvec_rejects_a_field_that_is_not_an_rhs_node():
    program, iterate, operator = _program()
...
def test_rhs_jacvec_rejects_an_rhs_computed_from_another_iterate():
    ...
    r0 = program._rhs_primitive(name="r0", state=other, flux=True, sources=None)
...
def test_rhs_jacvec_rejects_r0_at_another_temporal_point():
    program, iterate, operator = _program()
    r0 = program._rhs_primitive(name="r0", state=iterate, flux=True, sources=None)
...
def test_rhs_jacvec_rejects_a_different_default_source_selection():
    program, iterate, operator = _program()
    full_r0 = program._rhs_primitive(name="r0", state=iterate, flux=True, sources=None)
...
def test_rhs_jacvec_rejects_a_named_flux_base_residual():
    program, iterate, operator = _program()
    named_r0 = program._rhs_primitive(
        name="r0", state=iterate, flux=True, sources=None, fluxes=["named_flux"])
    with pytest.raises(ValueError, match="may not use a named flux"):
        _record(program, operator, iterate, named_r0)
...
def test_rhs_jacvec_accepts_one_exact_field_context_from_the_frozen_iterate():
    ...
    r0 = program._rhs_primitive(
        name="r0", state=iterate, fields=fields, flux=True, sources=None)
    completed = _record(
