...
def test_decorator_matches_inline_ir(t):
    ...
    inline = t.Program("fe")
    lt.forward_euler(inline, *state_refs(inline, "plasma"))

    deco = t.Program("fe")  # same name: _ir_hash includes it

    @deco.step
    def _build(P):
        lt.forward_euler(P, *state_refs(P, "plasma"))
...
def test_decorator_calls_fn_exactly_once_at_build(t):
    ...
    def _build(prog):
        calls.append(prog)
        lt.forward_euler(prog, *state_refs(prog, "plasma"))
    ...
    other = t.Program("fe")
    lt.forward_euler(other, *state_refs(other, "plasma"))
...
def test_decorator_works_for_a_multistage_body(t):
    ...
    def build(P):
        ...
        P.commit(endpoint, P.value("step", U + P.dt * k, at=endpoint.point))
