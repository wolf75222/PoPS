...
On verifie :
 ...
 (4) AMR : un package production target='amr_system' avec projection est accepte par add_native_block
     (AmrSystem) : la projection ponctuelle est cablee PAR NIVEAU apres le reflux (ADC-312) ;
 (5) PROJECTION LISANT L'AUX (production) : un plancher par cellule q1 <- max(q1, w) ou w est un
     ...
     q1==5 partout (preuve que l'aux est CONSOMMEE, pas un canal a vide) par la fermeture ``project``
     du loader natif.
"""
import os
import shutil
...
from pops._ir.ops import abs_, sign
...
def build_aux_model(tag):
    ...
    Exerce le transport du champ aux par le loader natif : sans aux lue, ces branches tournaient a
    vide."""
...
def run_states(so, nsteps=NSTEPS):
...
def reference_states(so_nohook, m_clamp, nsteps=NSTEPS):
...
def run_states_aux(so, w, nsteps=NSTEPS):
...
def reference_states_aux(so_nohook, m_aux, w, nsteps=NSTEPS):
...
def main():
    ...
    try:
        ...
        st_none = run_states(so_none)
        st_id = run_states(so_id)
        ...
        so_clamp = m_clamp.compile(os.path.join(tmp, "toy_clamp.so"), INCLUDE,
                                   backend="production")
        st_clamp = run_states(so_clamp)
        st_ref = reference_states(so_none, m_clamp)
        ...
        print("== (4) projection sur AMR production ==")
        ...
        print("== (5) projection LISANT l'aux : champ aux consomme (production) ==")
        ...
        so_aux = m_aux.compile(
            os.path.join(tmp, "toy_aux_production.so"), INCLUDE, backend="production"
        )
        st_aux = run_states_aux(so_aux, W)
        st_ref = reference_states_aux(so_none, m_aux, W)
        ...
        chk(all(np.allclose(a, b, rtol=0.0, atol=1e-15) for a, b in zip(st_aux, st_ref)),
            "production : etat post-pas == transport puis plancher(aux) numpy (ecart max %.2e)" % d)
        # Le plancher LIT reellement W : q1 est releve dans au moins une cellule vs le run sans hook.
        d_active = max(float(np.max(np.abs(a[1] - b[1]))) for a, b in zip(st_aux, st_none))
        chk(d_active > 1e-6, "production : le plancher aux modifie q1 (ecart %.2e)" % d_active)
        # ADVERSARIAL : un plancher 5.0 (>> q1) force q1 == 5 partout a chaque pas. Si l'aux etait
        # ignoree, q1 resterait le transport (~[0.5, 0.9]) et ce check tomberait.
        st_hi = run_states_aux(so_aux, W_hi)
        chk(all(np.allclose(s[1], 5.0, rtol=0.0, atol=1e-12) for s in st_hi),
            "production : plancher aux=5.0 -> q1==5 a chaque pas (aux lu, jamais ignore)")
        chk(all(float(np.max(np.abs(a[0] - b[0]))) == 0.0 for a, b in zip(st_hi, st_none)),
            "production : q0 inchange (projection identite sur q0, aux ne touche que q1)")
    finally:
