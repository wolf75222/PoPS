"""ADC-369 : halos/ghosts auxiliaires configurables par champ with ``pops.mesh.AuxHalo``.
...
On verifie (compilateur requis pour le bout-en-bout, auto-skip sinon) :
  (1) validation pure-Python de AuxHalo ;
...
def test_auxhalo_validation():
    """AuxHalo : kinds valides, valeur, rejet d'un kind inconnu. Pur Python."""
    assert AuxHalo("foextrap").bc_type == 1
    d = AuxHalo("dirichlet", value=2.5)
    assert d.bc_type == 2 and d.value == 2.5
    raised = False
    try:
        AuxHalo("periodic")  # pas une politique par champ (gardee par le domaine)
    except ValueError:
        raised = True
...
def test_system_cartesian_halo():
    ...
    try:
        ...
        R_foe = _cart_rhs(compiled, n, vx2d, AuxHalo("foextrap"))
        R_dir = _cart_rhs(compiled, n, vx2d, AuxHalo("dirichlet", value=0.0))
...
def test_amr_halo():
    ...
    try:
        ...
        D_dir = stepped_density(AuxHalo("dirichlet", value=0.0))
