...
def test_modelspec_is_the_explicit_native_real_boundary():
    model = engine.Model(
        state=engine.FluidState.isothermal(cs2=Fraction(7, 10)),
        transport=engine.IsothermalFlux(),
        source=engine.PotentialForce(charge=Fraction(-1, 3)),
        elliptic=engine.BackgroundDensity(alpha=Decimal("0.2"), n0=Fraction(1, 8)),
    )
...
def test_root_time_and_spatial_descriptors_retain_exact_real_controls():
    ...
    spatial = engine.Spatial(positivity_floor=Fraction(1, 10**20))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda: engine.Explicit(stride=1.0),
        lambda: engine.Explicit(ssprk3=1),
        lambda: engine.IMEX(substeps=True),
        lambda: engine.IMEX(newton_max_iters=True),
        lambda: engine.IMEX(newton_diagnostics=1),
    ],
)
