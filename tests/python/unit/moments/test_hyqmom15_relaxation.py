...
from pops._ir.ops import eig_lmin, eig_lmin_bound
...
def test_eig_lmin_bound_matches_host_lmin_on_a_converged_block() -> None:
    rows = ((1.0, 0.2, 0.0), (0.2, 1.0, 0.1), (0.0, 0.1, 1.0))
    assert float(eig_lmin_bound(rows).eval({})) == pytest.approx(
        float(eig_lmin(rows).eval({}))
    )
...
def test_relaxation15_emits_a_bounded_native_program_kernel() -> None:
    ...
    assert "pops_eig_real_status_3x3" not in source
    assert source.count("return bounds.lmin;") >= 1
