...
def _scalar(elliptic):
    ...
    return engine.Model(state=engine.Scalar(), transport=engine.ExB(B0=1.0),
                     source=engine.NoSource(), elliptic=elliptic)
...
def _solve_single(elliptic, dens, n, rhs="composite"):
    ...
    sim.block("blk", model=_scalar(elliptic), spatial=engine.Spatial(none=True))
...
def background_is_generic():
    ...
    phi_bg = _solve_single(engine.BackgroundDensity(alpha=alpha, n0=n0), dens, n)
    ...
    f_ref = alpha * (dens - n0)
    phi_ref = _solve_single(engine.ChargeDensity(charge=1.0), f_ref, n, rhs="charge_density")
    ...
    phi_charge = _solve_single(engine.ChargeDensity(charge=1.0), dens, n, rhs="charge_density")
...
def mixed_bricks_sum():
    ...
    sim = System(n=n, L=1.0, periodic=True)
    sim.block("a", model=_scalar(engine.ChargeDensity(charge=q0)), spatial=engine.Spatial(none=True))
    sim.block("b", model=_scalar(engine.BackgroundDensity(alpha=alpha, n0=n0bg)),
...
def epm_facade_roundtrip():
    ...
    alpha, n0 = 0.9, 0.3
    dens = _density(n)
    phi_setpoisson = _solve_single(engine.BackgroundDensity(alpha=alpha, n0=n0), dens, n)

    sim = System(n=n, L=1.0, periodic=True)
    sim.block("blk", model=_scalar(engine.BackgroundDensity(alpha=alpha, n0=n0)),
                  spatial=engine.Spatial(none=True))
    sim.add_elliptic_model("poisson",
                           engine.elliptic(operator=engine.div_eps_grad(1.0), rhs=engine.composite_rhs(),
...
def token_alias():
    ...
    dens = _density(n)
    phi_charge = _solve_single(engine.ChargeDensity(charge=1.0), dens, n, rhs="charge_density")
    phi_comp = _solve_single(engine.ChargeDensity(charge=1.0), dens, n, rhs="composite")
    ...
    sim = System(n=n, L=1.0, periodic=True)
    sim.block("blk", model=_scalar(engine.ChargeDensity(charge=1.0)), spatial=engine.Spatial(none=True))
    try:
