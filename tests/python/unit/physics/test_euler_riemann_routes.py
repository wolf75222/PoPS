...
def test_generic_hllc_refuses_p_only_euler_model():
    ...
    with pytest.raises(RuntimeError, match="hllc_star_state"):
        _validate(m, HLLC())


def test_generic_roe_refuses_p_only_euler_model():
    m = _compiled(n_vars=4, prim_names=("rho", "u", "v", "p"), roe=False)
    with pytest.raises(RuntimeError, match="roe_dissipation"):
        _validate(m, Roe())
...
def test_euler_hllc_refuses_non_euler_layout():
    ...
    with pytest.raises(RuntimeError, match="4-variable Euler"):
