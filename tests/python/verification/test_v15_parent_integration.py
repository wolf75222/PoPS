...
def _authenticated_tr02_series(tmp_path: Path, monkeypatch, *, n_cells: int = 16):
    ...
    job = run_dir / "n016"
    _emit_tr02_job(job, identity, n_cells=n_cells)
    write_series_json(run_dir, "TR-02", ["n016"])
    return run_dir


def test_live_module_never_reloads_result_npy():
    ...
    assert loads == [], "live.py must not call np.load / Path.load"
    assert "result.npy" not in source
    assert "np.load" not in source
    assert "numpy.load" not in source


def test_live_visuals_use_authenticated_records_not_replaced_npy(
    tmp_path: Path, monkeypatch
):
    from verification.pops_verify.evidence_bundle import EvidenceBundle
    from verification.pops_verify.visualization.live import write_live_visual_data

    run_dir = _authenticated_tr02_series(tmp_path, monkeypatch)
    bundle = EvidenceBundle(run_dir)
    original = np.asarray(bundle.records[-1]["result"], dtype=np.float64)
    assert np.allclose(original, 1.0)

    npy = run_dir / "n016" / "result.npy"
    npy.chmod(0o644)
    np.save(npy, np.full(16, 99.0, dtype=np.float64))
    npy.chmod(0o444)

    write_live_visual_data(run_dir, bundle, case_id="TR-02")
    profile = json.loads(
        (run_dir / "analysis" / "visual_data" / "reference_profile.json").read_text(
            encoding="utf-8"
        )
    )
    ys = profile["series"][0]["y"]
    assert all(abs(value - 1.0) < 1e-12 for value in ys)
    assert not any(abs(value - 99.0) < 1e-12 for value in ys)


def test_live_visuals_survive_toctou_replace_after_bundle_auth(
    tmp_path: Path, monkeypatch
):
    from verification.pops_verify.evidence_bundle import EvidenceBundle
    from verification.pops_verify.visualization.live import write_live_visual_data
    ...
    run_dir = _authenticated_tr02_series(tmp_path, monkeypatch)
    real_init = EvidenceBundle.__init__

    def replace_after_auth(self, path, *args, **kwargs):
        real_init(self, path, *args, **kwargs)
        npy = Path(path) / "n016" / "result.npy"
        if npy.is_file():
            npy.chmod(0o644)
            np.save(npy, np.full(16, 99.0, dtype=np.float64))
            npy.chmod(0o444)

    monkeypatch.setattr(EvidenceBundle, "__init__", replace_after_auth)
    bundle = EvidenceBundle(run_dir)
    write_live_visual_data(run_dir, bundle, case_id="TR-02")
    profile = json.loads(
        (run_dir / "analysis" / "visual_data" / "reference_profile.json").read_text(
...
def test_status_authenticated_failing_bundle_is_not_scientific(tmp_path: Path):
    from verification.pops_verify.visualization.live import write_run_status

    run_dir = tmp_path / "failing-bundle"
    write_run_status(
        run_dir,
        case_id="TR-02",
        run_id="failing-bundle",
    ...
    assert status["scientific"] is False
    assert status["live"] is False


def test_status_passing_bundle_sets_scientific_and_live(tmp_path: Path):
    from verification.pops_verify.visualization.live import write_run_status

    run_dir = tmp_path / "passing-bundle"
    write_run_status(
        run_dir,
        case_id="TR-02",
        run_id="passing-bundle",
    ...
    assert status["scientific"] is True
    assert status["live"] is True


def test_status_invoke_only_cannot_imply_pass(tmp_path: Path):
    from verification.pops_verify.visualization.data import VisualsError
    from verification.pops_verify.visualization.live import write_run_status
    from verification.pops_verify.visualization.render import render_run

    run_dir = tmp_path / "invoke-only"
    write_run_status(
        run_dir,
        case_id="IF-08",
        run_id="invoke-only",
    ...
    status = json.loads((run_dir / "status.json").read_text(encoding="utf-8"))
    assert status["data_kind"] == "campaign"
    assert status["verdict"] == "not-run"
    assert status["scientific"] is False
    assert status["live"] is False
    with pytest.raises((VisualsError, FileNotFoundError, Exception)):
        render_run(run_dir, suite="pr", formats=("svg",), strict=True)


def test_status_rejects_pass_without_scientific_pass(tmp_path: Path):
    from verification.pops_verify.evidence_bundle import EvidenceError
    from verification.pops_verify.visualization.live import write_run_status

    with pytest.raises((EvidenceError, ValueError), match="scientific_pass|verdict"):
        write_run_status(
            tmp_path / "lie",
            case_id="TR-02",
            run_id="lie",
...
def test_runner_writes_live_phase8_status_after_scientific_analysis(
    ...
    scientific_pass = results[0].get("scientific_pass") is True
    assert status["data_kind"] == "campaign"
    assert status["verdict"] == ("pass" if scientific_pass else "fail")
    assert status["scientific"] is scientific_pass
    assert status["live"] is scientific_pass
...
def test_runner_order_fail_status_is_authenticated_not_scientific(
    tmp_path: Path, monkeypatch
):
    test_runner_execute_emits_evidence_and_splits_run_fields(tmp_path, monkeypatch)
    series_dir = tmp_path / "out" / "TR-02" / "dim1-KokkosSerial-off"
    results = json.loads((tmp_path / "out" / "results.json").read_text(encoding="utf-8"))
    assert results[0].get("scientific_pass") is not True
    status = json.loads((series_dir / "status.json").read_text(encoding="utf-8"))
    assert (series_dir / "series.json").is_file()
    assert (series_dir / "n016" / "result.npy").is_file()
    assert status["data_kind"] == "campaign"
    assert status["verdict"] == "fail"
    assert status["scientific"] is False
    assert status["live"] is False
