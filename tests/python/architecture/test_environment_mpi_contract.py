...
def test_environment_selects_one_exact_conda_mpi_implementation() -> None:
    ...
    assert "mpi=1.0=openmpi" not in environment
...
def test_setup_authenticates_the_solved_mpich_parallel_hdf5_stack() -> None:
    ...
    hdf5_variant = '"$hdf5_build" != mpi_mpich_*'
...
def test_macos_build_keeps_mpich_wrappers_on_the_pinned_compiler() -> None:
    build = (REPOSITORY / "scripts" / "build_python.sh").read_text(encoding="utf-8")

    activate = 'conda activate "$ENV_NAME"'
    platform_guard = 'if [[ "$(uname)" == Darwin && -x "$CONDA_PREFIX/bin/mpichversion" ]]; then'
    mpi_cc = 'export MPICH_CC="${MPICH_CC:-${CC:-/usr/bin/clang}}"'
    mpi_cxx = 'export MPICH_CXX="${MPICH_CXX:-${CXX:-/usr/bin/clang++}}"'
    build_command = 'python -m pip "${pip_args[@]}"'
