...
``ProgramExecutionServices`` is the backend-neutral seam included by every generated artifact.  Global
...
def test_program_execution_services_has_no_schur_tokens():
    """program_execution_services.hpp carries zero Schur/Lorentz/electrostatic tokens (ADC-587)."""
    assert PROGRAM_CONTEXT.exists(), "program_execution_services.hpp not found at %s" % PROGRAM_CONTEXT
    text = PROGRAM_CONTEXT.read_text(encoding="utf-8")
    lowered = text.lower()
    offenders = [tok for tok in _FORBIDDEN_TOKENS if tok in lowered]
    assert not offenders, (
        "program_execution_services.hpp must remain backend-neutral, but it still mentions retired "
        "Schur/Lorentz machinery %s; move provider-specific work behind the generic elliptic "
        "provider seam" % offenders
    )


def test_program_execution_services_drops_schur_includes():
    """program_execution_services.hpp no longer #includes the Schur / MG / Lorentz headers (ADC-587)."""
    text = PROGRAM_CONTEXT.read_text(encoding="utf-8")
    ...
    assert not leaked, (
        "program_execution_services.hpp must not include the Schur/MG/Lorentz headers after the split, but it "
...
if __name__ == "__main__":
    ...
    test_program_execution_services_has_no_schur_tokens()
    test_program_execution_services_drops_schur_includes()
