...
def test_program_loader_requires_the_complete_block_identity_table(path):
    ...
    assert '"pops_program_block_count"' in source
    assert '"pops_program_block_name"' in source
    assert "program_block_map.assign" in source
    assert "found < 0" in source or "found == runtime_blocks.end()" in source
    assert ".block_map_ = program_block_map" in source
...
def test_program_loader_has_no_empty_map_positional_fallback(path):
    ...
    assert "program_block_map.assign" in source
    assert ".block_map_ = std::move(program_block_map)" in source
