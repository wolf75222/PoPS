  ...
  for (int j = 0; j < nc; ++j)
    for (int i = 0; i < nc; ++i) {
      ...
      if (r2 < kR * kR) {  // interieur du disque
        ...
        s += e * e; mx = std::max(mx, e); ++cnt;
      }
    }
  l2 = std::sqrt(s / cnt); linf = mx;
}
...
int main() {
  ...
  solve_err(128, false, s128, si128); solve_err(256, false, s256, si256); solve_err(512, false, s512, si512);
  solve_err(128, true, c128, ci128); solve_err(256, true, c256, ci256); solve_err(512, true, c512, ci512);
  ...
  if (fails == 0) std::printf("OK test_cut_cell\n");
  return fails == 0 ? 0 : 1;
}
