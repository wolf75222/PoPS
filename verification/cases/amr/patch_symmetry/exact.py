"""AM-12 static coarse-fine geometry, reflection, and TR-01 sine reuse.

1-d: coarse left / fine right of x=0.5. The reflected mesh is the
image under x↦1-x: fine left / coarse right, interface still at 0.5.
...
def reflect_x(x) -> np.ndarray:
    """Return the unit-interval reflection x ↦ 1 − x."""
...
def reflected_cf_centers(
    n_coarse: int = N_COARSE, *, refinement_ratio: int = REFINEMENT_RATIO
):
    """Return the x↦1−x image: fine on [0, 0.5], coarse on [0.5, 1]."""
    centers, h_fine, h_coarse = static_cf_centers(
        n_coarse, refinement_ratio=refinement_ratio
    )
    return reflect_x(centers)[::-1], h_fine, h_coarse
