"""Public 1-d periodic AMR Case for AM-12 reflected static coarse-fine.

Leftover in-memory helpers stay. The live Case is the AM-01 counterpart with
the interface reflected: fine on the left half x<0.5. Optional native
...
def _band_errors(field, oracle, interface):
    bulk = np.logical_not(interface)
    e_cf = band_max_abs_error(field, oracle, interface)
    e_bulk = band_max_abs_error(field, oracle, bulk)
    return e_cf, e_bulk, bulk


def patch_sample(n_coarse=None, t=0.0, *, leftover: bool = True, reflected: bool = False) -> dict:
    """Return leftover interface-band errors on the original or reflected CF mesh."""
    count = _exact.N_COARSE if n_coarse is None else int(n_coarse)
    if reflected:
        centers, h_fine, h_coarse = _exact.reflected_cf_centers(count)
    else:
    ...
    e_cf, e_bulk, bulk = _band_errors(field, oracle, interface)
...
def reflect_leftover_and_oracle(sample) -> dict:
    """Spatially reverse leftover, oracle, and mask under x↦1−x."""
    centers = _exact.reflect_x(sample["centers"])[::-1]
    oracle = np.asarray(sample["oracle"], dtype=np.float64)[::-1]
    field = np.asarray(sample["field"], dtype=np.float64)[::-1]
    interface = np.asarray(sample["interface"], dtype=bool)[::-1]
    e_cf, e_bulk, bulk = _band_errors(field, oracle, interface)
    ratio = None if sample["ratio"] is None else interface_bulk_ratio(e_cf, e_bulk)
    return {
        "centers": centers,
        "h_fine": float(sample["h_fine"]),
        "h_coarse": float(sample["h_coarse"]),
        "oracle": oracle,
        "field": field,
        "interface": interface,
        "bulk": bulk,
...
def _frame():
    ...
    return CartesianDomain("am12_unit_interval", (0.0,), (1.0,)).frame(Cartesian1D())
...
def _left_half_marker(frame):
    """Spatial indicator: 1 on x<0.5, 0 on the coarse right half."""
...
def _author(n_coarse: int) -> _Authoring:
    ...
    tracer_model, tracer_state, tracer_flux, tracer_rate, velocity = _scalar_transport(
        "am12_tracer", frame, speed=A
    )
    marker_model, marker_state, marker_flux, marker_rate, _ = _scalar_transport(
        "am12_marker", frame, speed=0.0
    )

    case = pops.Case("am12_patch_symmetry")
...
def amr_periodic_layout(authored: _Authoring):
    """1-d periodic AMR layout: fine patch on the left half x<0.5."""
    ...
    case = authored.case
    threshold = case.param(RuntimeParam("am12_refine_x", default=REFINE_THRESHOLD))
...
def resolve_plan(n_coarse=None):
    ...
    try:
        ...
        raise AuthoringPending(
            f"AM-12 1-d periodic AMR resolve failed: {type(exc).__name__}: {exc}"
        ) from exc
...
def run_native(n_coarse=None, t_end=1.0):
    ...
    try:
        ...
        raise NativeUnavailable(
            f"AM-12 native compile/bind/run failed: {type(exc).__name__}: {exc}"
        ) from exc
