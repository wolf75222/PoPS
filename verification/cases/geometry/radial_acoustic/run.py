"""2-d Cartesian radial-acoustic authoring and native run.

Initial conditions come from ``primitives(..., t=0)``. ``build_case`` /
``resolve_plan`` author a public 2-d Euler Case (Rusanov, MUSCL/VanLeer,
SSPRK2). ``run_native`` compiles, binds, and advances the Case. Cartesian
only: no polar mesh. Does not call ROMEO.
"""
...
def _author(n_cells: int = N_CELLS) -> _Authoring:
    ...
    model = pops.Model("ge03-euler", frame=frame)
    state = model.state(
        "U",
    ...
    case = pops.Case("ge03-radial-acoustic")
...
def run_native(n_cells: int = N_CELLS, t_end: float = 0.05) -> np.ndarray:
    """Compile, bind, and run the 2-d radial acoustic wave.

    Returns a C-contiguous ``(4, n, n)`` conserved array in ``COMPONENT_ORDER``.
