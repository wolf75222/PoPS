"""PO-03 driver: mean-free manufactured series, norms, observed order, report."""
...
NOT_APPLICABLE = {
    "amr.*": "AMR not run in PO-03 1-d Neumann nullspace Poisson",
    "coupling.*": "coupling not run in PO-03",
    "parallel_invariance.*": "parallel invariance not run in PO-03",
    "performance.one_node": "performance not measured in PO-03",
    "performance.two_node": "performance not measured in PO-03",
}
ARTIFACTS = {
    "report_md": "REPORT.md",
...
def _manufactured_series():
    """Mean-free φ vs φ + C + α h² φ on N = 16,32,64,128."""
    error_scalars = []
    spacings = []
    finest = None
    field_finest = None
    ...
    for n_cells in RESOLUTIONS:
        ...
        manufactured = (
            sample["phi"]
            + GAUGE_SHIFT
            + MANUFACTURED_ERROR_SCALE * spacing**2 * sample["phi"]
        )
        potential = reference_errors(
            mean_free(manufactured, sample["volumes"]),
            mean_free(sample["phi"], sample["volumes"]),
            sample["volumes"],
        )
...
def _summary() -> dict:
    ...
    if not (
        ...
        raise ValueError("non-finite PO-03 diagnostics")
...
def write_po03_report(output_dir) -> dict:
    """Reduce the mean-free φ series and write the four Task 20 artifacts."""
