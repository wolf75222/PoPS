"""PO-01 driver: manufactured second-order series, norms, observed order, report."""
...
NOT_APPLICABLE = {
    "amr.*": "AMR not run in PO-01 1-d periodic trigonometric Poisson",
    "coupling.*": "coupling not run in PO-01",
    "parallel_invariance.*": "parallel invariance not run in PO-01",
    "performance.one_node": "performance not measured in PO-01",
    "performance.two_node": "performance not measured in PO-01",
}
ARTIFACTS = {
    "report_md": "REPORT.md",
...
def _summary() -> dict:
    ...
    if not (
        ...
        raise ValueError("non-finite PO-01 diagnostics")
...
def write_po01_report(output_dir) -> dict:
