"""PO-02 driver: manufactured second-order series, norms, observed order, report."""
...
NOT_APPLICABLE = {
    "amr.*": "AMR not run in PO-02 2-d Dirichlet MMS Poisson",
    "coupling.*": "coupling not run in PO-02",
    "parallel_invariance.*": "parallel invariance not run in PO-02",
    "performance.one_node": "performance not measured in PO-02",
    "performance.two_node": "performance not measured in PO-02",
}
ARTIFACTS = {
    "report_md": "REPORT.md",
...
def _manufactured_series():
    ...
    for n_cells in RESOLUTIONS:
        ...
        ex_num, ey_num = sample["e"]
        ex_ref, ey_ref = e_exact(sample["x"], sample["y"])
        field = reference_errors(
            np.hypot(ex_num, ey_num),
            np.hypot(ex_ref, ey_ref),
            sample["volumes"],
        )
        residual = reference_errors(
            sample["rhs"], rhs_exact(sample["x"], sample["y"]), sample["volumes"]
        )
...
def _summary() -> dict:
    ...
    if not (
        ...
        raise ValueError("non-finite PO-02 diagnostics")
...
def write_po02_report(output_dir) -> dict:
