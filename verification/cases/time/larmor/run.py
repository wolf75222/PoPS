...
def run_native(dt=None, t_end=1.0, *, n_cells: int = N_CELLS, request=None):
    ...
    n_cells = apply_campaign_request(
        n_cells, request, case_id='TM-04', allowed_dims=(1,), unavailable=NativeUnavailable
    )
    if dt is None:
    ...
    simulation = bind_public(artifact, initial_values={authored.instance: initial}, mpi_mode=require_bind_request(request, NativeUnavailable, 'TM-04'))
