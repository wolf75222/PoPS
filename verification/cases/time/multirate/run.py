...
def run_native(dt=None, r=1, t_end=_exact.DT, *, n_cells: int = N_CELLS, request=None):
    """Compile, bind, and run the Case. Raises NativeUnavailable without a compiler."""
    n_cells = apply_campaign_request(
        n_cells, request, case_id='TM-06', allowed_dims=(1,), unavailable=NativeUnavailable
    )
    if dt is None:
    ...
    simulation = bind_public(artifact, initial_values={authored.instance: initial}, mpi_mode=require_bind_request(request, NativeUnavailable, 'TM-06'))
