"""PF-05 driver: two-level leaf residual stand-in and campaign report writer."""
...
NOT_APPLICABLE = {
    ...
    "coupling.*": "coupling not run in PF-05",
    "parallel_invariance.*": "parallel invariance not run in PF-05",
...
def _leaf_diagnostics():
    ...
    if not (
        ...
        raise ValueError("non-finite PF-05 leaf diagnostics")
    ...
    if not (0.0 < fraction < 1.0):
        raise ValueError("refined fraction must be an interior observation")
    return sample, residual, potential, field, fraction


def _summary() -> dict:
    _sample, residual, potential, field, fraction = _leaf_diagnostics()
    return {
        ...
        "performance": {
            "one_node": {
                ...
                "notes": "refined fraction is an observation, not a timed PF run",
...
def write_pf05_report(output_dir) -> dict:
