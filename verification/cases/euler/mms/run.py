"""1-d periodic Euler MMS authoring and initial data.
...
SSPRK2). Manufactured sources live in ``exact.sources_1d``; they are not yet
injected into the Case. ``run_native`` is optional and raises
``NativeUnavailable`` when a compiler is missing.
"""
...
def _line_frame():
    ...
    return CartesianDomain("eu03-line", lower=(0.0,), upper=(1.0,)).frame(Cartesian1D())
...
def build_case(n_cells: int = N_CELLS):
    ...
    frame = _line_frame()
    (x_axis,) = frame.axes
    model = pops.Model("eu03-euler", frame=frame)
    state = model.state(
        "U",
