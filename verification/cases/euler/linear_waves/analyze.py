"""EU-01 driver: in-memory exact vs exact, one primitive component, campaign report."""
...
NOT_APPLICABLE = {
    ...
    "amr.*": "AMR not run for EU-01 in-memory path",
    "poisson.*": "Poisson not run for EU-01",
    "coupling.*": "coupling not run for EU-01",
    "parallel_invariance.*": "parallel invariance not run for EU-01",
    "performance.one_node": "performance not measured for EU-01 in-memory path",
    "performance.two_node": "performance not measured for EU-01 in-memory path",
}
ARTIFACTS = {
    "report_md": "REPORT.md",
...
def _summary() -> dict:
    ...
    primitives = exact_mode(centers, 0.0, mode="entropy")
...
def write_eu01_report(output_dir) -> dict:
