"""EU-05 driver: in-memory exact vs exact on density, campaign report."""
...
exact_gresho = _exact.exact_gresho
...
NOT_APPLICABLE = {
    "orders": "stationary vortex: no spatial-order campaign on the in-memory path",
    "amr.*": "AMR not run for EU-05 in-memory path",
    "poisson.*": "Poisson not run for EU-05",
    "coupling.*": "coupling not run for EU-05",
    "parallel_invariance.*": "parallel invariance not run for EU-05",
    "performance.one_node": "performance not measured for EU-05 in-memory path",
    "performance.two_node": "performance not measured for EU-05 in-memory path",
}
ARTIFACTS = {
    "report_md": "REPORT.md",
...
def _summary() -> dict:
    ...
    primitives = exact_gresho(x, y, 0.0)
...
def write_eu05_report(output_dir) -> dict:
