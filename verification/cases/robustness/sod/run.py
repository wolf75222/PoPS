"""1-d Sod shock-tube authoring and native run.
...
SSPRK2). The physical tube uses public ``Outflow`` faces on a non-periodic Uniform
layout. ``run_native`` compiles, binds, and advances the Case.
"""
...
def _line_frame():
    ...
    return CartesianDomain("rb01-line", lower=(0.0,), upper=(1.0,)).frame(Cartesian1D())
...
def _author(n_cells: int = N_CELLS) -> _Authoring:
    ...
    frame = _line_frame()
    (x_axis,) = frame.axes
    model = pops.Model("rb01-euler", frame=frame)
    state = model.state(
        "U",
...
def run_native(n_cells: int = N_CELLS, t_end: float = 0.2):
    """Compile, bind, and run the 1-d Sod tube. Raises NativeUnavailable without Kokkos."""
    import pops
