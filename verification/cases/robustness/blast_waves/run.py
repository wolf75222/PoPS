"""1-d Woodward–Colella blast-wave authoring and native run.
...
def _line_frame():
    ...
    return CartesianDomain("rb09-line", lower=(0.0,), upper=(1.0,)).frame(Cartesian1D())
...
def _author(n_cells: int = N_CELLS) -> _Authoring:
    ...
    frame = _line_frame()
    (x_axis,) = frame.axes
    model = pops.Model("rb09-euler", frame=frame)
    state = model.state(
        "U",
    ...
    case = pops.Case("rb09-blast-waves")
...
def run_native(n_cells: int = N_CELLS, t_end: float = 0.038):
    """Compile, bind, and run the 1-d blast-wave Case.

    Returns conserved ``(rho, rho_u, E)`` with shape ``(3, n)``. Raises
