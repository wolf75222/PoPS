"""Toro/Castro 1-d strong-shock exact Riemann oracle.

Left (rho, u, p) = (1, 0, 1000), right (1, 0, 0.01), gamma=1.4.
Diaphragm at x0=0.5 on the unit interval. Evaluation time t=0.012.

Star states and wave positions follow Toro, *Riemann Solvers and
Numerical Methods for Fluid Dynamics*, 3rd ed., Ch. 4 (same machinery
as RB-01):
...
This problem has a left rarefaction, contact, and a strong right-going
shock (p* >> p_R). Does not import pops or read a PoPS output.
"""
...
def wave_positions(t=T_END) -> dict:
    """Rarefaction head/tail, contact, and shock positions at time t."""
    time = float(t)
    star = star_states()
    return {
        ...
        "shock": X0 + star["shock_speed"] * time,
    }
...
def primitives_1d(x, t) -> np.ndarray:
    """Exact strong-shock primitives W=(rho, u, p). Shape (3, n)."""
