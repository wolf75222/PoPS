"""1-d Toro/Castro strong-shock authoring and native run.
...
def _line_frame():
    ...
    return CartesianDomain("rb03-line", lower=(0.0,), upper=(1.0,)).frame(Cartesian1D())
...
def _author(n_cells: int = N_CELLS) -> _Authoring:
    ...
    frame = _line_frame()
    (x_axis,) = frame.axes
    model = pops.Model("rb03-euler", frame=frame)
    state = model.state(
        "U",
    ...
    case = pops.Case("rb03-strong-shock")
...
def run_native(n_cells: int = N_CELLS, t_end: float = 0.012):
    """Compile, bind, and run the 1-d strong shock. Raises NativeUnavailable without Kokkos."""
    import pops
