"""Public 1-d Euler double-rarefaction authoring and native run.

Initial conditions come from ``primitives_1d(..., t=0)``. ``build_case`` /
``resolve_plan`` author a public 1-d Euler Case (Rusanov, FirstOrder,
SSPRK2, ``positivity_floor``). MUSCL at the u=±2 diaphragm reconstructs
p<0; ``sqrt(γ p / ρ)`` then publishes ``InvalidWaveSpeed`` (status=6).
The shared layout helper is periodic. ``run_native`` compiles, binds,
and advances the Case.
"""
...
CFL = 0.4
MAX_STEPS = 100_000
# Zhang-Shu density floor on reconstructed faces. FirstOrder faces are
# cell averages, so this is a backstop if a later cell undershoots.
...
def _line_frame():
    ...
    return CartesianDomain("rb02-line", lower=(0.0,), upper=(1.0,)).frame(Cartesian1D())
...
def _author(n_cells: int = N_CELLS) -> _Authoring:
    ...
    frame = _line_frame()
    (x_axis,) = frame.axes
    model = pops.Model("rb02-euler", frame=frame)
    state = model.state(
        "U",
    ...
    case = pops.Case("rb02-double-rarefaction")
...
def run_native(n_cells: int = N_CELLS, t_end: float = 0.15):
    """Compile, bind, and run the 1-d double rarefaction."""
    import pops
