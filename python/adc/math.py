...
It builds a tiny, numerics-free IR of *board nodes* that :mod:`adc.physics`
(model authoring) and :mod:`adc.time` (time programs) destructure when they
lower to the Spec 2 operator-first IR. A board node carries structure only; the
actual expressions over a state are :mod:`adc.dsl` ``Expr`` trees.

Design note -- why a separate node type. :mod:`adc.dsl` ``Expr`` deliberately
...
field are known). Pure standard library: it imports :mod:`adc.dsl` lazily, only
...
class OpApply(_BoardNode):
    ...
    :class:`Unknown`; :meth:`adc.time.Program.solve` destructures it.
    """
