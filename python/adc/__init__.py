...
class System:
    ...
    def install(self, compiled, *, instances=None, params=None, aux=None, solvers=None):
        ...
        @param solvers dict {field: <adc.lib.fields.GeometricMG(...)/adc.GeometricMG(...)>}: lowered to
    ...
    def _lower_spatial(self, spatial):
        """Lower a spatial selection to an adc.Spatial consumed by add_equation. Accepts an
        adc.Spatial / adc.FiniteVolume (returned as-is), an adc.lib.spatial.FiniteVolume(...)
        ...
        raise TypeError("install: spatial must be an adc.FiniteVolume / adc.Spatial or an "
                        "adc.lib.spatial.FiniteVolume(...) descriptor; got %r"
    ...
    def _validate_riemann_capability(self, model, spatial):
        ...
        a composed native adc.Model(...) carries the capability in its bricks (the C++ requires-gate
    ...
    def _solver_token(solver_brick):
        ...
        descriptor carrying ``scheme`` (adc.lib.fields.GeometricMG -> 'geometric_mg')."""
        if isinstance(solver_brick, str):
            return solver_brick
        token = getattr(solver_brick, "scheme", None) or getattr(solver_brick, "name", None)
        if token is None:
            raise TypeError("install: solver must be a token string or an adc.lib.fields.<Solver>(...) "
