...
The ``solvers`` :class:`types.SimpleNamespace` gathers the executable Krylov and nonlinear factories
under one attribute surface (``solvers.CG()`` / ``solvers.GMRES()`` / ``solvers.Newton()``); the
custom-solver GENERATION DSL (``@solver`` / ``SolverContext`` /
