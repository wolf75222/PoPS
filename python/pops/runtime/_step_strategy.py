    ...
    def prepare_attempts(
        ...
        if temporal is not None:
            if temporal.event_queue:
                event, queued_dt = temporal.queued_error_controlled_proposal(
                    time=native.time(), macro_step=native.macro_step())
                if proposal[0].hex() != queued_dt.hex():
                    event = temporal.queue_error_controlled_proposal(
                        dt=proposal[0],
                        time=native.time(),
                        macro_step=native.macro_step(),
                        replace=True,
                    )
                queued_event[0] = event
            else:
                queued_event[0] = temporal.queue_error_controlled_proposal(
                    dt=proposal[0],
                    time=native.time(),
                    macro_step=native.macro_step(),
                )
        ...
        def retry(_error: BaseException, attempts: int) -> bool:
            ...
            if reduced < self.strategy.dt_min:
                return False
            proposal[0] = reduced
            current_temporal = getattr(engine, "_temporal_restart_state", None)
            if current_temporal is not None:
                queued_event[0] = current_temporal.queue_error_controlled_proposal(
                    dt=proposal[0],
                    time=native.time(),
                    macro_step=native.macro_step(),
                    replace=True,
        ...
        def accept(attempts: int) -> None:
            ...
            current_temporal = getattr(engine, "_temporal_restart_state", None)
            if current_temporal is not None:
                current_temporal.queue_error_controlled_proposal(
                    dt=current.next_dt,
                    time=native.time(),
                    macro_step=native.macro_step(),
                )
