...
def _install_boundary_authorities(engine: Any, install_plan: Any) -> None:
    ...
    try:
        # Validate every external GhostBoundary descriptor and ABI table before the first route or
        # prepared boundary plan is published.  Its later package preparation remains inside the
        # native all-ranks package transaction, which restores the accepted pre-bind image on error.
        for _base_arguments, component_jobs in prepared:
            for preflight, _installer, _block, component, row, parameters_json, target_json, context in (
                component_jobs
            ):
