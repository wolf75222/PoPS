...
class RunManifest:
    ...
    def __init__(self, *, bind_identity: Any, start_time: Any, start_macro_step: Any,
        ...
        object.__setattr__(self, "start_macro_step", int(start_macro_step))
        object.__setattr__(self, "controls", MappingProxyType({
            "t_end": float(exact["t_end"]), "cfl": float(exact["cfl"]),
            "max_steps": int(exact["max_steps"]), "output_mode": str(exact["output_mode"]),
        }))
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        result["run_identity"] = self.run_identity.token
        return result
