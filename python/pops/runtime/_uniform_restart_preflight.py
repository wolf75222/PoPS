...
def _float_vector(payload: Any, key: str) -> tuple[float, ...]:
    import numpy as np

    value = np.asarray(payload[key])
    if value.ndim != 1 or value.dtype.kind != "f":
        raise TypeError("restart : %s must be an exact floating vector" % key)
...
def preflight_uniform_restart(payload: Any) -> None:
    ...
    required = {
        "t", "macro_step", "program_hash", "history_names", "cache_nodes", "cache_names",
    ...
    for name in history_names:
        keys = {
            ...
            "history_init_" + name, "history_fill_count_" + name,
        ...
        for label, slots in (("requested", requested), ("stored", stored)):
            if slots != tuple(sorted(set(slots))) \
                    or any(slot < 0 or slot >= depth for slot in slots):
                raise ValueError(
        ...
        slot_dt = _float_vector(payload, "history_slot_dt_" + name)
        if len(slot_dt) != depth:
            raise ValueError("restart : history '%s' dt index is truncated" % name)
        if any(not math.isfinite(dt) for dt in slot_dt):
            raise ValueError("restart : history '%s' slot dt values must be finite" % name)
        if initialized:
            if any(dt <= 0.0 for dt in slot_dt):
                raise ValueError(
                    "restart : initialized history '%s' requires positive slot dt values" % name)
        elif any(dt != 0.0 for dt in slot_dt):
            raise ValueError(
                "restart : uninitialized history '%s' must have zero slot dt values" % name)
        regrid_key = "history_regrid_steps_" + name
        if regrid_key in files:
            regrid_steps = _integer_vector(payload, regrid_key)
            if regrid_steps != tuple(sorted(set(regrid_steps))) \
                    or any(step < 0 for step in regrid_steps):
                raise ValueError(
    ...
    if cache_nodes != tuple(sorted(set(cache_nodes))) \
    ...
    for index, node in enumerate(cache_nodes):
        keys = {
        ...
        last_update = _integer_scalar(
            payload, "cache_last_update_%d" % node, minimum=0)
        if last_update >= macro_step:
            raise ValueError(
                "restart : scheduled cache node %d last update is not an accepted prior step"
                % node)
        _float_scalar(payload, "cache_accum_dt_%d" % node, minimum=0.0)
        if not cache_names[index]:
            raise ValueError(
                "restart : scheduled cache node %d has an empty name" % node)
