"""AMR native installation behind ``pops.bind``.

This mixin stages compiled blocks, resolved field plans, initial values and
auxiliary inputs on the hierarchy. Shared field-component authentication lives
in ``_field_provider_install``; this adapter owns AMR plan publication.
"""
...
class _PreparedAmrFieldSolverInstall(PreparedFieldSolverInstall):
