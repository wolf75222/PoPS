...
``route_program_params`` maps a compiled problem's declared runtime parameters to the per-block
``set_program_params`` vectors (ADC-510, Spec 5 C5); it is the mirror of ``_route_block_params`` (the
AOT-block ``set_block_params`` route, P7-b) and is re-exposed as ``System._route_program_params`` for
host tests.
"""


def route_program_params(routes, defaults, params):
    """Map a flat ``{param_name: value}`` to the per-block ``set_program_params`` vectors.
    ...
    else the declaration default); unknown lists the supplied names declared by no generated kernel
