"""Private typed profiling support for native execution engines.

A typed internal replacement for stringly ``enable_profiling()`` / ``profile_report()`` calls.
It is not an author-facing runtime API. Two pieces:
...
The off-by-default contract is unchanged: profiling adds no heavy timers unless a private execution
provider explicitly enables them. The native ``enable_profiling`` gate leaves ordinary runs
disabled. `RuntimeInstance` deliberately exposes no profiling or engine-delegation method.

This module is a pure typed/parsing wrapper: it imports neither ``_pops`` nor numpy. The native
extension is reached only through the internal engine selected behind `pops.bind` / `pops.run`.
"""
...
# POPS_PROFILE: select the default level for private engine profiling when enabled.
...
class Profile:
    ...
    The private engine uses named constructors rather than a string flag::

        from pops.runtime._profile import Profile
        with engine.profile(Profile.Basic()) as prof:
            engine.step_cfl(0.4)
        print(prof.summary())
    ...
    def from_env(cls, default: Any = None) -> Any:
        """Resolve a private engine profiling level from ``POPS_PROFILE``.
