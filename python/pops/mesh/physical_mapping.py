...
class AxisQuadrature:
    ...
    def __post_init__(self) -> None:
        ...
        weights = ((upper - lower) / self.cells,) * self.cells if self.weights is None else tuple(
            Fraction(value) for value in self.weights)
        if len(weights) != self.cells:
...
class PhysicalSupportMap:
    ...
    def __post_init__(self) -> None:
        ...
        dimension = max(len(source), len(target)) if self.native_dimension is None else self.native_dimension
        ...
        for name, support in (("source_axes", source), ("target_axes", target)):
            axes = getattr(self, name)
            axes = tuple(range(len(support))) if axes is None else tuple(axes)
            ...
            object.__setattr__(self, name, axes)
        ...
        if self.quadrature is not None:
            if type(self.quadrature) is not VelocityQuadrature:
                raise TypeError("moment requires an exact VelocityQuadrature")
            missing = tuple(axis for coordinate, axis in zip(source, self.source_axes, strict=True)
        ...
        required = {axis for coordinate, axis in zip(source, self.source_axes, strict=True)
