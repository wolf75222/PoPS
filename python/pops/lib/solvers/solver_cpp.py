"""pops.lib.solvers.solver_cpp -- lower a custom solver IR to a self-contained C++ kernel.

This is the ONLY file in ``pops.lib`` that emits C++ text. It builds the C++ source
with PURE Python string formatting -- it imports nothing from :mod:`pops.codegen` and
nothing from ``_pops``, so it does not break the ``lib/`` acyclic invariant (lib never
imports codegen / _pops at module scope). The authoring IR comes from
:mod:`pops.lib.solvers.dsl`.

The module is named ``solver_cpp`` (not ``codegen``) so the acyclic audit
(``grep -rnE '^(from |import )' lib/ | grep -E 'codegen|_pops'`` -> nothing) is literally
clean. The strict-invariant option (relocating this emitter to
``pops.codegen.solver_codegen`` and leaving a lazy delegator here) is deferred to PR-D2 per
the blueprint; for now the emitter lives here and stays codegen-free.
"""
import json
...
def generate_solver_cpp(solver_brick, func=None):
    ...
    solver IR with :func:`pops.lib.solvers.dsl.build_solver_ir` and walks the SSA, emitting a
    self-contained C++ free function that drives the solve entirely in C++ -- it calls the
    SHARED matrix-free HPC primitives (``pops::dot`` / ``pops::saxpy`` / ``pops::lincomb`` from
    ``mf_arith.hpp``) and runs the convergence loop as a REAL C++ ``for (;;)`` whose predicate
    re-evaluates each iteration. Returns the C++ source string (a header-only templated kernel).
...
class _SolverCppLowering:
    """Walk a :class:`pops.lib.solvers.dsl.SolverIR` and emit a standalone C++ solver kernel.
