...
_AXIS_ROLE = re.compile(r"^(momentum|velocity|axial):([0-9]+)$")
...
class RoleKey:
    ...
    def __post_init__(self) -> None:
        if self.family == "custom":
            ...
            if self.label is not None:
                _exact_role_text(self.label, where="custom StateSchema role")
        elif (
    ...
    def token(self) -> str:
        if self.family == "custom":
            return "custom" if self.label is None else self.label
