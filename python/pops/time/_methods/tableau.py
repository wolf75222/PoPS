...
from pops.time._methods.coefficients import (
    exact_coefficient as _exact_coefficient,
...
def _rows(A: Any, stages: int, *, diagonal: bool, where: str) -> tuple[tuple[Any, ...], ...]:
    ...
    for i, raw in enumerate(source):
        ...
        row = tuple(_exact_coefficient(item, "%s[%d][%d]" % (where, i, j))
                    for j, item in enumerate(raw))
        if any(_exact_fraction(item, where) != 0 for item in row[width:]):
...
def _weights(values: Any, where: str) -> tuple[Any, ...]:
    try:
        result = tuple(_exact_coefficient(item, "%s[%d]" % (where, i))
...
def _nodes(rows: tuple[tuple[Any, ...], ...], c: Any, where: str) -> tuple[Any, ...]:
    sums = tuple(sum((_exact_fraction(item, where) for item in row), Fraction()) for row in rows)
    if c is None:
        return sums
    ...
    for i, (actual, expected) in enumerate(zip(result, sums, strict=True)):
        if _exact_fraction(actual, where) != expected:
...
class RungeKuttaTableau:
    ...
    def __init__(self, A: Any, b: Any, c: Any = None, name: Any = None) -> None:
        ...
        if sum((_exact_fraction(x, "RungeKuttaTableau.b") for x in weights), Fraction()) != 1:
    ...
    def properties(self) -> Any:
        from pops.time._methods.properties import analyze_runge_kutta
        return analyze_runge_kutta(self)

    @property
    def certificate(self) -> Any:
        from pops.time._methods.properties import certify_runge_kutta
...
class AdditiveRungeKuttaTableau:
    ...
    def __init__(self, explicit: RungeKuttaTableau, implicit_A: Any,
        ...
        if sum((_exact_fraction(x, "ARK implicit_b") for x in weights), Fraction()) != 1:
    ...
    @property
    def certificate(self) -> Any:
        from pops.time._methods.properties import certify_additive_runge_kutta
