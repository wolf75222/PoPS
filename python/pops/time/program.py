...
class CompiledTime:
    ...
    A NUMERIC `cfl` (e.g. `cfl=0.5`) is wired: it is applied at RUNTIME by `sim.run(cfl=...)` /
    `sim.step_cfl(cfl)`, which computes the per-block CFL `dt` in adc_cpp and drives the installed
    Program at that `dt` (`System::step_cfl` routes through the installed program). A self-computed
    `cfl` SUB-PROGRAM (`cfl="program"`) is still deferred (the Program would export its own dt bound);
    it fails loud rather than being silently ignored."""
    ...
    def __init__(self, substeps=1, stride=1, cfl="default"):
        ...
        # A numeric cfl is wired (applied at runtime via sim.run(cfl=) / step_cfl); only a non-numeric,
