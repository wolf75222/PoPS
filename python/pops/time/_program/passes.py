...
from pops.time._program.serialization import _ProgramSerialization
...
class _ProgramPasses(_ProgramSerialization, _ProgramConstants, _ProgramBase):
    ...
    def _rebuild(
        ...
        *,
        ...
        """
        from pops.time._program.rebuild import rebuild_program
    ...
    def validate(self) -> Any:
        ...
        for v in self._values:
            ...
            elif v.op in ("range", "subcycle", "post_synchronization"):
                self._validate_block(v.attrs["body_block"], seen)
            elif v.op == "branch":
