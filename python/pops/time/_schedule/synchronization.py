...
class InterpolateHistory:
    """Use one typed history's declared interpolation or dense-output capability."""
    ...
    def validate_transfer(self, source: Any, target: Any) -> None:
        del target
        if source is not self.history.value:
            raise ValueError(
                "InterpolateHistory provider must be the same HistoryHandle as the "
                "transferred value"
            )
...
def validate_relation_data(data: Any) -> dict[str, Any]:
    ...
    return validate_synchronization_relation_data(data)
