"""pops.codegen.solvers.solver_cpp -- lower a custom solver IR to a self-contained C++ kernel.

INTERNAL / EXPERIMENTAL (Spec 5 criterion 19): this is the C++ lowering of the
:mod:`pops.codegen.solvers.dsl` solver-generation DSL, not a stable public API.

It builds the C++ source with PURE Python string formatting -- it imports nothing from
``_pops`` and nothing else in :mod:`pops.codegen`; its only dependency is the sibling
authoring IR :mod:`pops.codegen.solvers.dsl`. Lowering an authored IR to C++ text is a
...
def generate_solver_cpp(solver_brick: Any, func: Any = None) -> str:
    ...
    Builds the solver IR with :func:`pops.codegen.solvers.dsl.build_solver_ir` and walks the SSA, emitting a
...
class _SolverCppLowering:
    """Walk a :class:`pops.codegen.solvers.dsl.SolverIR` and emit a standalone C++ solver kernel.
    ...
    def __init__(self, ir: Any, func: Any) -> None:
        ...
        if self._iterate_id is None:
            ...
            # result upstream, so a well-formed @pops.codegen.solvers.solver never reaches this.
            raise ValueError(
...
// GENERATED by pops.codegen.solvers.generate_solver_cpp (epic ADC-462, Spec 3 section 20 / criterion 23).
