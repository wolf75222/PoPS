"""pops.codegen.compile : compile / .so-loader layer.

Free functions that receive a physical model object and drive the C++ compilation
pipeline (source emission -> compiler invocation -> .so on disk). Full brick
compilation still consumes the legacy hyperbolic-model protocol; compiled
``Program`` artifacts consume ``pops.model.Module`` directly through the
operator-first codegen view.
