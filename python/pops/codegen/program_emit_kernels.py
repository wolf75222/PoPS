...
// A combined problem.so installed across the stable .so ABI: it drives sim.step(dt) entirely in
// C++ via ProgramContext, reusing the adc_cpp runtime.
...
// pops_problem_dt_bound(ctx, cfl) returns the lowered scalar bound (min'd into the native CFL by
// step_cfl). When no bound was set, has_dt_bound() is false and dt_bound returns +inf (unreached).
extern "C" bool pops_problem_has_dt_bound() {{ return {has_dt_bound}; }}
extern "C" pops::Real pops_problem_dt_bound(pops::runtime::program::ProgramContext* ctxp, pops::Real cfl) {{
