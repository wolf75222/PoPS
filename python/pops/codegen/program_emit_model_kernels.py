...
def _emit_solve_coupled_implicit_kernel(components: Any, by_block: Any, var: Any,
    ...
    lines += [
        ...
        "    if (!solved_.solved()) {",
        "      %sA(i, j, 8) = pops::detail::encode_local_nonlinear_failure("
...
def _emit_solve_local_nonlinear_kernel(model: Any, v: Any, guess_var: Any, out_var: Any,
    ...
    Generated code contributes the residual functor, immutable controls and a transaction-local
    candidate scratch.  Convergence, finite-difference Jacobians, pivoted factorization, budgets and
    status diagnostics stay in ``solve_prepared_local_nonlinear`` rather than being emitted here.
    """
    impl = _model_impl(model)
    n = len(impl.cons_names)
    # ADC-617: the FD Jacobian relative step. A missing node value keeps the historic default.
    ...
    lambda_index = next(
        index for index, line in enumerate(body) if "pops::for_each_cell" in line)
    body[lambda_index:lambda_index] = [
        "  const pops::Array4 nonlinear_statusA = %s.fab(li).array();" % status_var,
    ]
    ...
    for c in range(n):
        body.append("    outA(i, j, %d) = solved_.value[%d];" % (c, c))
    body += [
        "    nonlinear_statusA(i, j, 0) = static_cast<pops::Real>("
        "pops::local_nonlinear_status_code(solved_.status));",
        "    nonlinear_statusA(i, j, 1) = static_cast<pops::Real>(solved_.iterations);",
        "    nonlinear_statusA(i, j, 2) = static_cast<pops::Real>(solved_.evaluations);",
        "    nonlinear_statusA(i, j, 3) = solved_.reference_residual_norm;",
        "    nonlinear_statusA(i, j, 4) = solved_.residual_norm;",
        "    nonlinear_statusA(i, j, 5) = solved_.step_norm;",
        "    nonlinear_statusA(i, j, 6) = solved_.condition_evidence;",
        "    nonlinear_statusA(i, j, 7) = "
        "static_cast<pops::Real>(solved_.safeguard_steps);",
        "    if (!solved_.solved()) {",
        "      nonlinear_statusA(i, j, 8) = "
        "pops::detail::encode_local_nonlinear_failure(i, j, solved_.failing_component);",
        "      nonlinear_statusA(i, j, 9) = pops::Real(1);",
        "    } else {",
        "      nonlinear_statusA(i, j, 8) = pops::Real(0);",
        "      nonlinear_statusA(i, j, 9) = pops::Real(0);",
        "    }",
    ]
    body += _kernel_close()
    return body
