...
def _emit_one_operator(model, op, operator_id):
    ...
    if op.kind == "field_operator":
        if len(op.signature.inputs) > 1:
            header.append(
                "static void %s(pops::runtime::program::ProgramContext& ctx, "
                "const std::vector<const pops::MultiFab*>& u_stages) {" % fn)
            body.append("ctx.solve_fields_from_blocks(u_stages);")
        else:
            header.append(
                "static void %s(pops::runtime::program::ProgramContext& ctx, int b, "
        ...
        header.append(
            "static const char* %s(pops::runtime::program::ProgramContext& ctx, int b, "
