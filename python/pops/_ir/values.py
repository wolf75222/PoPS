"""pops._ir.values -- EigWitness, StateRef, RuntimeParamRef, and their support dicts.
...
# imported lazily where used so pops._ir stays importable in a bare interpreter (e.g. the build-time
...
# nom DSL -> METHODE C++ const(im_tol) -> bool, abaissee en pops::Real(...). Verrouillee sur converged
...
class EigWitness(Expr):
    ...
    Codegen device-clean : l'emission est un FONCTEUR NOMME (methode statique POPS_HD de la brique
    generee) qui remplit un ``pops::Real M[k][k]`` puis appelle ``real_eig_minmax`` -- jamais de lambda
...
class RuntimeParamRef(Expr):
    ...
    READS the value from its pops::RuntimeParams member instead of having it baked in. The value can thus
    be CHANGED at runtime without recompiling the .so (cf. include/pops/runtime/runtime_params.hpp).
