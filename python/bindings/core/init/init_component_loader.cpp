...
py::dict report(const pops::component::LoadedComponent& loaded) {
  ...
  for (std::size_t index = 0; index < api.interface_count; ++index) {
    ...
    entry["table"] = pops::component::generated_native_interface_table_name(
        row.interface_id);
...
void init_component_loader(py::module_& m) {
  ...
  auto handle = py::class_<LoadedComponent, std::shared_ptr<LoadedComponent>>(
      m, "_NativeComponentHandle");
  ...
  m.def(
      ...
      [](const std::string& path, const std::string& component_id,
         ...
         const std::string& prepare_parameters_json,
         const std::string& prepare_target_json) {
        ExpectedNativeComponent expected{
            component_id, semantic_identity, manifest_identity, catalog_sha256, {},
        ...
        for (const auto& [raw_id, version, declared_table] : interfaces) {
          ...
          const auto* generated_table =
              pops::component::generated_native_interface_table_name(id);
          ...
          expected.interfaces.push_back(
              RequiredNativeInterface{id, version,
                                      pops::component::native_interface_table_size(id)});
        }
      ...
      py::arg("manifest_identity"), py::arg("catalog_sha256"), py::arg("interfaces"),
      py::arg("prepare_parameters_json"), py::arg("prepare_target_json"),
