...
if not hasattr(probe, "_install_program_so") or not hasattr(probe, "set_program_cadence"):
    _skip("_pops lacks _install_program_so / set_program_cadence (rebuild _pops) (A passed)")
...
def make_compiled(cadence=None):
    ...
    sim = make_sim(pops.Explicit.euler())
...
sim_a = make_compiled()
u0 = np.array(sim_a._get_state("ions"))
dt_cfl = sim_a.step_cfl(CFL)
u1 = np.array(sim_a._get_state("ions"))
...
sim_native = make_sim(pops.Explicit.euler())
...
sim_b = make_compiled()
sim_b.step(dt_cfl)
u_step = np.array(sim_b._get_state("ions"))
e_bx = float(np.abs(u1 - u_step).max())
chk(e_bx == 0.0, "step_cfl(cfl) == step(dt_cfl) bit-exact (max|d|=%.2e)" % e_bx)

# ----- (c) substeps=2 / stride=2 honored UNDER step_cfl (CompiledProgramCadence cadence applied) -----
...
# from substeps=1.
sim_c1 = make_compiled(CompiledProgramCadence(substeps=1))
dt_c1 = sim_c1.step_cfl(CFL)
u_c1 = np.array(sim_c1._get_state("ions"))

sim_c2 = make_compiled(CompiledProgramCadence(substeps=2))
dt_c2 = sim_c2.step_cfl(CFL)
u_c2 = np.array(sim_c2._get_state("ions"))
chk(abs(dt_c2 - dt_c1) < 1e-14, "step_cfl dt is cadence-independent at step 1 (%.10g vs %.10g)"
    % (dt_c2, dt_c1))

sim_c2_ref = make_compiled(CompiledProgramCadence(substeps=2))
sim_c2_ref.step(dt_c2)
u_c2_ref = np.array(sim_c2_ref._get_state("ions"))
...
K = 4
sim_s = make_compiled(CompiledProgramCadence(stride=2))
dts = []
for _ in range(K):
    dts.append(sim_s.step_cfl(CFL))
u_s = np.array(sim_s._get_state("ions"))
t_s = float(sim_s.time())

sim_s_ref = make_compiled(CompiledProgramCadence(stride=2))
for dt_k in dts:
    sim_s_ref.step(dt_k)
u_s_ref = np.array(sim_s_ref._get_state("ions"))
e_s = float(np.abs(u_s - u_s_ref).max())
chk(e_s == 0.0, "step_cfl stride=2 == step(dt_k) stride=2 over %d steps bit-exact (max|d|=%.2e)"
    % (K, e_s))

sim_s1 = make_compiled(CompiledProgramCadence(stride=1))
for dt_k in dts:
    sim_s1.step(dt_k)
u_s1 = np.array(sim_s1._get_state("ions"))
