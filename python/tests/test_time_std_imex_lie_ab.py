...
def test_imex_local_theta_guard(t):
    for bad in (0.0, -0.5, 1.5):
        try:
            t.std.imex_local(t.Program("x"), "plasma", linear_source="lorentz", theta=bad)
...
def test_imex_local_rejects_empty_linear_source(t):
    try:
        t.std.imex_local(t.Program("x"), "plasma", linear_source="")
    except ValueError as exc:
...
def test_lie_chains_two_stages(t):
    ...
    out = t.std.lie(P, "plasma", half_flow, source)
    P.validate()
    ...
    t.std.strang(strang, "plasma", half_flow, source)
...
def test_adams_bashforth_bad_order(t):
    for bad in (0, 4, 2.0):
        try:
            t.std.adams_bashforth(t.Program("x"), "plasma", bad)
        except ValueError as exc:
...
def test_ab3_lowers_with_two_history_reads(t):
    P = t.Program("ab3")
    t.std.adams_bashforth(P, "plasma", 3)
    try:
