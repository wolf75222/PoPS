...
# --- typed diagnostics / remaining macro scheme dedup (ADC-506) --------------------------
def test_norm_factory_is_removed_use_typed_norm():
    import pops.diagnostics as diag

    typed = Norm(L2())
    assert not hasattr(diag, "norm")
    assert typed.scheme == Norm.scheme
    assert typed.options()["scheme"] == Norm.scheme
...
def test_dedup_does_not_fabricate_a_runtime_path():
    ...
    typed = Norm(L2())
    assert typed.lower()["scheme"] == Norm.scheme
    for obj in (typed,):
