...
adc.IMEXRK(scheme="ars222") cable le schema d'Ascher-Ruuth-Spiteri (1997) : transport explicite
(L = -div F) couple a la source raide implicite (backward-Euler local par cellule), ORDRE 2. C'est
une famille DISTINCTE de adc.IMEX (backward-Euler local, ordre 1, INCHANGE).
...
Verifie :
  ...
  (c) DEFAUT INCHANGE / FAMILLE DISTINCTE : adc.IMEX.kind == "imex" != adc.IMEXRK.kind ==
...
def build(time_policy, q, B0, rho0=1.0, u0=1.0, v0=0.0, n=8):
    ...
    sim.add_block("e", cyclotron_model(q), spatial=adc.FiniteVolume(limiter="minmod"),
                  time=time_policy)
...
e1_rk, e2_rk, r_rk = err_ratio(lambda: adc.IMEXRK())
e1_be, e2_be, r_be = err_ratio(lambda: adc.IMEX())
...
m_stiff = momentum_after(adc.IMEXRK(), Q2, B2, DT2, 10)
...
chk(adc.IMEX().kind == "imex", "adc.IMEX.kind == 'imex' (backward-Euler local, defaut)")
chk(adc.IMEXRK().kind == "imexrk_ars222", "adc.IMEXRK.kind == 'imexrk_ars222' (famille distincte)")
chk(adc.IMEX().kind != adc.IMEXRK().kind, "kinds distincts -> chemins C++ distincts (pas un alias)")
chk(adc.IMEXRK().scheme == "ars222", "adc.IMEXRK.scheme == 'ars222'")
...
m_be = momentum_after(adc.IMEX(), Q, B, T / 10, 10)
m_rk = momentum_after(adc.IMEXRK(), Q, B, T / 10, 10)
...
try:
    amr.add_block("e", cyclotron_model(1.0), spatial=adc.FiniteVolume(limiter="minmod"),
                  time=adc.IMEXRK())
...
simp = adc.System(mesh=adc.PolarMesh(r_min=0.2, r_max=1.0, nr=16, ntheta=16))
try:
    simp.add_block("e",
                   adc.Model(state=adc.Scalar(), transport=adc.ExB(B0=1.0),
                             source=adc.NoSource(), elliptic=adc.BackgroundDensity()),
                   spatial=adc.FiniteVolume(), time=adc.IMEXRK())
    chk(False, "polaire + IMEXRK aurait du lever")
except (RuntimeError, ValueError, TypeError) as e:
    chk("imex" in str(e).lower(), f"polaire rejet explicite : {e}")

# (d3) Strang / Schur : l'etage hyperbolique doit etre un adc.Explicit (pas IMEXRK)
try:
    adc.Strang(hyperbolic=adc.IMEXRK(),
               source=adc.CondensedSchur(kind="electrostatic_lorentz", theta=0.5, alpha=1.0))
...
sim_mask = adc.System(n=8, L=1.0, periodic=True)
try:
    # adc.IMEXRK n'expose pas implicit_vars ; on force l'attribut pour exercer la garde C++.
    pol = adc.IMEXRK()
    pol.implicit_vars = ["rho_u"]
    sim_mask.add_block("e", cyclotron_model(1.0), spatial=adc.FiniteVolume(limiter="minmod"),
                       time=pol)
