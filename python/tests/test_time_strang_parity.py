#!/usr/bin/env python3
"""Compiled pops.lib.time.strang reproduces native pops.Strang on a simple case (ADC-410).

The Strang splitting macro H(dt/2); S(dt); H(dt/2) is expressed ONCE as Program IR via the
``pops.lib.time.strang`` combinator (no scheme-specific C++ stepper). This test demonstrates that the
