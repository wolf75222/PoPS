...
Couvre les deux chantiers du solde (docs/GENERICITY_2026-06.md, points NON generalises #1) :
  ...
  (c) NO-DEFAULT-CHANGE (mono-bloc) : les options Newton par DEFAUT explicites (newton_max_iters=2,
      ...
      sans options (adc.IMEX()). On compare deux runs de memes graines : dmax == 0 (vrai test de
...
def mono_imex(time):
    ...
    s = adc.AmrSystem(n=16, L=1.0, periodic=True, regrid_every=0)
    s.set_poisson(rhs="charge_density", solver="geometric_mg", bc="periodic")
    s.set_refinement(1e30)
    s.add_block("e", iso_model(), spatial=adc.FiniteVolume(limiter="minmod"), time=time)
...
d_a = mono_imex(adc.IMEX(newton_max_iters=5, newton_rel_tol=1e-12))
...
d_base = mono_imex(adc.IMEX())  # chemin historique (aucune option)
d_def = mono_imex(adc.IMEX(newton_max_iters=2, newton_rel_tol=0.0, newton_abs_tol=0.0,
...
sim_opt = adc.AmrSystem(n=16, periodic=True)
try:
    sim_opt.add_equation("gas", fake_production_amr(), spatial=adc.FiniteVolume(),
                         time=adc.IMEX(newton_max_iters=5))
...
sim_diag = adc.AmrSystem(n=16, periodic=True)
try:
    sim_diag.add_equation("gas", fake_production_amr(), spatial=adc.FiniteVolume(),
