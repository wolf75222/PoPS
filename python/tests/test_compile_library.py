...
def test_content_hash_is_sensitive_to_objects():
    ...
    more = pops.compile_library(
        "lib.so", objects=[lib.solvers.GMRES(max_iter=200), lib.riemann.HLLC()])
...
def test_content_hash_is_order_insensitive():
    fwd = pops.compile_library(
        "lib.so", objects=[lib.solvers.GMRES(max_iter=200), lib.riemann.HLLC()])
    rev = pops.compile_library(
        "lib.so", objects=[lib.riemann.HLLC(), lib.solvers.GMRES(max_iter=200)])
