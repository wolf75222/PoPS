"""Spec 5 (sec.5.11 / sec.8): the inert AMR authoring report.

These exercise the internal descriptor-sourced report used by
``CompiledProblem.inspect_amr()``. The helper names the levels / ratio envelope, the
attached regrid / refine / nesting / checkpoint / output policies, the runtime
requirements (reflux, tag reduction) and the explainable route limitations, for the
native envelope (``None``), a full AMR layout, an over-the-envelope (refused)
layout, and a Uniform layout. Pure Python: nothing computes on a grid; the report
reads descriptor metadata and imports no ``_pops`` / runtime / codegen.
"""
import sys
...
from pops._capabilities import inspect_amr  # noqa: E402
...
def test_inspect_amr_is_not_a_top_level_public_route():
    assert not hasattr(pops, "inspect_amr")
    assert "inspect_amr" not in pops.__all__
...
def test_amr_report_print_is_short_and_deterministic():
    ...
    assert str(inspect_amr(_full_amr())) == text
    assert len(text) < 1200


def test_over_envelope_layout_is_refused_with_reason():
    rep = inspect_amr(AMR(base=CartesianMesh(n=128), max_levels=4))
...
def test_uniform_layout_reports_single_level():
    rep = inspect_amr(Uniform(CartesianMesh()))
    d = rep.to_dict()
