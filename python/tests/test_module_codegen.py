...
A combined model+program .so carries GeneratedProgram (adc_install_program, the step) AND a
GeneratedModule descriptor: extern "C" adc_module_* accessors exposing the typed operator registry
...
def test_metadata_block_emitted():
    ...
    assert "adc_module_operator_kind(int i)" in src
    assert "adc_module_operator_signature(int i)" in src
...
def test_metadata_not_in_step_body():
    ...
    adctime.std.predictor_corrector_local_linear(
        ...
        explicit_rate_operator="explicit_rhs", implicit_operator="lorentz")
    src = P.emit_cpp_program(model=m)
    body = src.split("adc_install_program", 1)[1]
    assert "adc_module_" not in body, \
...
def test_no_model_empty_module():
    ...
    assert "adc_module_operator_count() { return 0; }" in src
