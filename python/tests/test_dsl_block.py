...
calcule par le bloc dynamique (host Rusanov via IModel) == celui d'adc.PythonFlux (meme schema), et que
le bloc tourne dans le System en conservant la masse. C'est l'aboutissement de l'item (a) : le DSL est
bout-en-bout depuis Python, a travers le runtime adc.
"""
import os
import shutil
import tempfile
...
def main():
    ...
    try:
        ...
        # (1) le residu du bloc dynamique == celui d'adc.PythonFlux (meme schema Rusanov global)
        R_sys = np.array(sim.eval_rhs("euler")).reshape(4, n, n)
        pf = adc.PythonFlux(lambda u, d: e.flux(u, {}, d),
        ...
        print("OK  eval_rhs(bloc dynamique) == adc.PythonFlux (ecart max %.1e)" % dres)
