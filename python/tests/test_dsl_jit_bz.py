"""Chantier Aux extensible, increment 7 : B_z pilotable 100% depuis Python via le chemin JIT.

Un modele DSL qui lit B_z (aux('B_z')) -> brique GenSrc avec n_aux=4 (increment 6) ->
CompositeModel::n_aux=4 (increment 4) -> .so JIT charge par add_dynamic_block : son IModel
expose n_aux()=4, le System elargit le canal aux partage (ensure_aux_width) et marshale 4
composantes vers le chemin hote (host_residual lit a.B_z). set_magnetic_field peuple B_z.
On verifie de bout en bout cote Python : eval_rhs du bloc dynamique = source = B_z * n.
"""
import os
import shutil
import tempfile
...
def main():
    ...
    try:
        ...
        print("OK  modele DSL B_z pilote 100%% depuis Python (compile_so + add_dynamic_block)")
        print("test_dsl_jit_bz : tout est vert")
    finally:
